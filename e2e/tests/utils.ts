...
export type PageWIds = Omit<PG, FuncNames | "getByTestId"> & {
  ...
  [funcName in FuncNames]: (...args: Parameters<PG[funcName]>) => Omit<ReturnType<PG[funcName]>, "getByTestId"> & Pick<PageWIds, "getByTestId">;
};

type LocatorOverridenFuncNames = "getByTestId" | "all" | "last";
export type LocatorWIds = Omit<Locator, LocatorFuncNames | LocatorOverridenFuncNames> & {
  getByTestId: (command: Command) => LocatorWIds;
  all: () => Promise<LocatorWIds[]>;
  first: () => LocatorWIds;
  last: () => LocatorWIds;
} & { 
  [funcName in LocatorFuncNames]: (...args: Parameters<Locator[funcName]>) => Omit<ReturnType<Locator[funcName]>, LocatorOverridenFuncNames> & Pick<LocatorWIds, LocatorOverridenFuncNames>;
};
export const getMonacoEditorBySelector = async (page: PageWIds, parentSelector: string) => {
...
export const runSql = async (page: PageWIds, query: string) => {
  ...
  await page.getByTestId("dashboard.window.runQuery").isEnabled({ timeout: 5e3 });
...
export const fillSmartFormAndInsert = async (page: PageWIds, tableName: string, values: Record<string, string>) => {
  for (const [key, value] of Object.entries(values)){
    ...
    if(await selectElem.isVisible()){
      ...
      await page.getByTestId("SearchList.List").locator(`[data-key=${JSON.stringify(value)}]`).click();
    }
  }
  await page.waitForTimeout(200);
...
export const clickInsertRow = async (page: PageWIds, tableName: string, useTopBtn = false) => {
  await page.getByTestId(useTopBtn? "dashboard.window.rowInsertTop" : "dashboard.window.rowInsert")
    .and(page.locator(`[data-key=${JSON.stringify(tableName)}]`)).click();
    await page.waitForTimeout(200);
}

export const insertRow = async (page: PageWIds, tableName: string, row: Record<string, string>, useTopBtn = false) => {
...
export const typeConfirmationCode = async (page: PageWIds) => {
  ...
  await page.waitForTimeout(200);
  await page.locator(`input[name="confirmation"]`).fill(code ?? "code not found on the page");
  await page.waitForTimeout(200);
}

export const forEachLocator = async (page: PageWIds, match: () => Promise<LocatorWIds> | LocatorWIds, onMatch: (locator: LocatorWIds) => Promise<void>) => {
...
export const closeWorkspaceWindows = async (page: PageWIds) => {
  ...
  await forEachLocator(
    ...
    async (closeBtn) => {
      ...
      const deleteSqlBtn = await page.getByRole("button", { name: "Delete", exact: true });
  ...
  const closeBtnsCount = await page.getByTestId("dashboard.window.close").count();
  if(closeBtnsCount){
...
const getTestId = (testid: Command) => `[data-command=${JSON.stringify(testid)}]`;
...
export const setTableRule = async (
  ...
  rule: { 
    ...
    update?: { excludedFields?: string[]; forcedFilter?: FFilter; forcedData?: FData; },
  ...
  const setForcedFilter = async (forcedFilter?: FFilter) => {
    ...
    for(const { fieldName, value } of forcedFilter){
      ...
      await page.getByTestId("FilterWrapper").locator(`input#search-all`).type(value);
      await page.waitForTimeout(500);
  ...
  const setForcedData = async (forcedData?: FData) => {
    ...
    for(const [fieldName, value] of Object.entries(forcedData)){
      await page.getByTestId("SmartAddFilter").click();
      await page.locator(getDataKey(fieldName)).click();
      await page.locator(`${getTestId("FilterWrapper")}${getDataKey(fieldName)} input`).type(value);
      await page.waitForTimeout(500);
      await page.keyboard.press("Enter");
      await page.waitForTimeout(200);
    }
...
export const openTable = async (page: PageWIds, namePartStart: string) => {
  ...
  const table = await page.locator(`[data-table-name^=${JSON.stringify(namePartStart)}]`)
  if(!table.isVisible()){
    const v_triggers = await runDbsSql(page, `SELECT * FROM prostgles.v_triggers;`, {}, { returnType: "rows" });
...
export const selectAndInsertFile = async (page: PageWIds, onOpenFileDialog: (page: PageWIds) => Promise<void>) => {
...
export const uploadFile = async (page: PageWIds) => {
  ...
  await page.waitForTimeout(200);
  await selectAndInsertFile(page, page => page.getByTestId("FileBtn").click());
}
...
export const getSearchListItem = (page: PageWIds | LocatorWIds, { dataKey }: { dataKey: string }) => {
  return page.getByTestId("SearchList.List").locator(`[data-key=${JSON.stringify(dataKey)}]`)
}
...
export const setWspColLayout = async (page: PageWIds) => {
  ...
  await page.getByTestId("dashboard.menu.settings.defaultLayoutType").locator(`[data-key="col"]`).click();
  await page.getByTestId("Popup.close").click();
}
export const disablePwdlessAdminAndCreateUser = async (page: PageWIds) => {

  await goTo(page);
  await page.getByRole('link', { name: 'Users' }).waitFor({ state: "visible", timeout: 60e3 });
  await page.getByRole('link', { name: 'Users' }).click();
  await expect(page as PG).toHaveURL(/.*users/);
  await page.goto('localhost:3004/users', { waitUntil: "networkidle", timeout: 10e3 });
...
export const createAccessRule = async (page: PageWIds, userType: "default" | "public") => {
  await login(page)
