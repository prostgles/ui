...
export type Method = DBSSchema["published_methods"] & { access_control_methods: DBSSchema["access_control_methods"][] };

type P = Pick<Prgl, "dbs" | "db" | "tables" | "dbsTables" | "dbsMethods" | "theme" | "dbKey"> & {
...
export const NewMethod = ({ dbKey, theme, db, dbs, methodId, connectionId, onClose, dbsTables, tables, dbsMethods, access_rule_id }: P) => {
  ...
  useAsyncEffectQueue(async () => {
    if(!methodId) return;
    const existingMethod = await dbs.published_methods.findOne({ id: methodId });
