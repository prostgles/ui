...
export const RestoreOptions = (props: RestoreOptionsProps) => {
  ...
  const restoreFile = async (popupClose: ()=>void) => {
    ...
    const streamId = await streamBackupFile("start", f.name, connectionId, null, f.size, restoreOpts);
  ...
  const backup = usePromise(async () => {
    ...
    if(bkp){
      ...
      const bkpFormat = bkp.options.command === "pg_dumpall"? "p" : bkp.options.format;
      if(format !== bkpFormat){
  ...
  let mainContent = <FlexCol style={{ maxHeight: "600px", overflow: "auto" }}>
    ...
    {(!fromFile || !!file) && (<>
      <Select 
        ...
        value={format}
        fullOptions={formats}
        onChange={format => { setRestoreOpts({ ...restoreOpts, format })  }} 
      />
      {format !== "p" && <>
        ...
        />
        <FormField value={create} label="Create" 
          ...
          asColumn={true} type="checkbox" onChange={create => { setRestoreOpts({ ...restoreOpts, create }) }} 
        />
        <FormField value={dataOnly} label="Data only" 
          ...
          asColumn={true} type="checkbox" onChange={dataOnly => { setRestoreOpts({ ...restoreOpts, dataOnly }) }} 
        />
        <FormField value={noOwner} label="No owner" 
          hint="Do not output commands to set ownership of objects to match the original database"
          asColumn={true} type="checkbox" onChange={noOwner => { setRestoreOpts({ ...restoreOpts, noOwner }) }} 
        />
        <FormField value={ifExists} label="If exists" 
          ...
          asColumn={true} type="checkbox" onChange={ifExists => { setRestoreOpts({ ...restoreOpts, ifExists }) }} 
        />
      </>}
  ...
  if(!fromFile){
    title = <InfoRow variant="naked" iconPath={mdiBackupRestore} color="action" className="text-medium font-20 " style={{ fontWeight: 400 }}>
      Restore {backup?.options.command === "pg_dumpall"? "server " : "database "} 
      using backup from <strong>{backup? (new Date(backup.created)).toLocaleString() : "???"}</strong>
    </InfoRow>
    mainContent = <>
      {title}
    ...
    title = <InfoRow variant="naked" iconPath={mdiBackupRestore} color="action" className="text-medium font-20 " style={{ fontWeight: 400 }}>
      Restore database from <strong>{!file? "local file" : sliceText(file.name, 44, undefined, true)} </strong>
    </InfoRow>
    mainContent = <>
      {title}
      <DumpRestoreAlerts {...{ dbsMethods, connectionId, dbProject: db } } />
