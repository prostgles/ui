...
import { type TokenInfo, getTokens } from "./SQLCompletion/completionUtils/getTokens";

const getNestingInfo = (tokens: TokenInfo[], nestingId: string, isStart: boolean) => {
  ...
  tokensOrdered.findIndex((_t, i) => {
    nestingContainedKeywords = nestingContainedKeywords || _t.type === "keyword.sql";
    const nextToken = tokensOrdered[i+1];
    nestingLength += _t.text.length;
    return nextToken?.text === (isStart? ")" : "(") && _t.nestingId === nestingId 
  });
...
export const getFormattedSql = async (model: editor.ITextModel) => {
  ...
  const newText = tokens.map((t, i) => {
    ...
    const newLineKeywords = [
      ...
      "with",
      "grant",
      "select", "from", "where", "group", "having", "order", "limit", "offset", "union", "intersect", "except", "on", 
      "join", "left", "right", "full", "cross", "natural", "inner"
    ]
    const newLineOperators = [ "and", "or", "exists"];
    const noPrevSpaceSymbols = ["(", ")", ",", "[", "]", "{", "}", "."];
    const currNestingSpaces = "  ".repeat(t.nestingId.length + t.funcNestingId.length);
    ...
    if(t.text === ")" && prevToken?.nestingId){
      const { nestingLength, nestingContainedKeywords } = getNestingInfo(tokens.slice(0, i+1), prevToken.nestingId, false);
      if(nestingLength > 40 || nestingContainedKeywords){
        return withNewline();
      }
    }

    const needsNewLine = t.textLC === "loop" && nextToken?.textLC === ";" ? undefined :
      (t.type === "keyword.sql" || t.type === "operator.sql") && newLineKeywords.includes(t.textLC)? "kwd" : 
      t.type === "operator.sql" && newLineOperators.includes(t.textLC)? "op" : undefined;
    if(needsNewLine) {
      if(needsNewLine === "kwd"){
        lastKwdToken = t;
      } else {
    ...
    if((prevToken?.type === "keyword.sql" || prevToken && ["in", "exists"].includes(prevToken.textLC)) && t.text === "("){
      addPrevSpace = true;
    }
    ...
    if(prevToken?.text === "("){
      const { nestingLength, nestingContainedKeywords } = getNestingInfo(tokens.slice(i-1), t.nestingId, true);
