...
import type { IDisposable, Monaco, languages } from "../W_SQL/monacoEditorTypes";


export type GetFuncs = (name: string, minArgs: number) => Promise<PG_Function[]>;

let sqlFunctionSuggestions: IDisposable | undefined;
export function registerFunctionSuggestions(monaco: Monaco, getFunc: GetFuncs, suggestions: SQLSuggestion[]){
  ...
  sqlFunctionSuggestions = monaco.languages.registerSignatureHelpProvider(LANG, {
    ...
    provideSignatureHelp: async function(model, position) {
      ...
      const numberOfDelimiters = parentFunction.prevDelimiters.reduce((a, v) => a + v.text.split(",").length - 1,0);
      ...
      const valuesIdx = previousToken.findIndex(t => t.textLC === "values" && t.type === "keyword.sql")
      ...
      if(!isInsertInto){
        ...
        value.activeSignature = funcs.findIndex(f => f.args.length >= activeParameter + 1);
