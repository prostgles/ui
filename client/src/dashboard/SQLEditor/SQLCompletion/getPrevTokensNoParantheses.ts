...
export const getTokenNesting = (rawTokens: TokenInfo[]): (TokenInfo & { nestingId: string; nestingFuncToken: TokenInfo | undefined; })[] => {
  const startNest = "(";
  const endNest = ")";
...
const getFuncNesting = (tokensWithNestingId: TokenInfo[]) => {
  ...
  const matchers = {
    ...
    loop: (t: TokenInfo, i: number) => {
      return t.textLC === "end" && tokensWithNestingId[i+1]?.textLC === "loop";
    }
  ...
  const nestMatcher = (t: TokenInfo, i: number): NestEndMatcher | undefined => {
    ...
    if(prevToken?.textLC === "begin"){
      return matchers.end;
    } else if(prevToken?.textLC === "loop" && prevPrevToken?.textLC !== "end"){
  ...
  return tokensWithNestingId.map((t, i) => {
    ...
    const isInsidefunc = withFuncsTokens.find(({ startIdx, endIdx }) => i >= startIdx + 2 && i <= endIdx - 1);
...
export const getDollarFunctions = (tokens: TokenInfo[]): { startIdx: number; endIdx: number; }[] => {
  const plpgPrecedingKeywords = ["function", "do", "procedure"];
  const result: { startIdx: number; endIdx: number; }[] = [];
  tokens.forEach((t, i) => {
    const prevToken = tokens[i-1];
    if(!t.nestingId && t.text.startsWith("$") && prevToken && plpgPrecedingKeywords.includes(prevToken.textLC)){
      const endIdx = tokens.findIndex(et => et.text === t.text && et.offset > t.offset);
      if(endIdx > -1){
