...
export const KwdPolicy = [
  { kwd: "POLICY", options: [{ label: "$polciy_name", docs: `The name of the policy to be created. This must be distinct from the name of any other policy for the table.` }] },
  { kwd: "ON", expects: "table", dependsOn: "POLICY", docs: `The name (optionally schema-qualified) of the table the policy applies to.` },
  {
    kwd: "AS",
    dependsOn: "ON", 
    optional: true,
    options: [
      { label: "PERMISSIVE", docs: `Specify that the policy is to be created as a permissive policy. All permissive policies which are applicable to a given query will be combined together using the Boolean “OR” operator. By creating permissive policies, administrators can add to the set of records which can be accessed. Policies are permissive by default.` },
      {
        label: "RESTRICTIVE", docs: `Specify that the policy is to be created as a restrictive policy. All restrictive policies which are applicable to a given query will be combined together using the Boolean “AND” operator. By creating restrictive policies, administrators can reduce the set of records which can be accessed as all restrictive policies must be passed for each record.
