...
A common use of an AFTER trigger is to populate an audit/history table with the changes.`,
        options: [{ label: actions.join(" OR ")}].concat(actions.map(label => ({ label }))),
        excludeIf: types,
      } satisfies KWD)
    ),
    { kwd: "ON", expects: "table" },
    { kwd: "OF", expects: "column", docs: "Only execute the function if the specified column was targeted" },
    { 
      kwd: "REFERENCING", 
      options: ["NEW TABLE AS new OLD TABLE AS old"].map(label => ({ label })),
      dependsOn: "AFTER",
      // excludeIf: ["BEFORE"],
    ...
    { 
      kwd: "WHEN", expects: "condition",  dependsOn: "ON",
    ...
    { 
      kwd: "EXECUTE FUNCTION", options: getTriggerFuncs,  dependsOn: "FOR EACH",
