...
export const matchCreateRule = async ({ cb, ss, getKind, sql, setS }: SQLMatchContext) => {
  ...
  if(cb.currNestingId){
    const startTkn = cb.tokens.findLast(t => t.offset <= cb.offset && t.nestingId === cb.currNestingId && t.text === "(");
    const endTkn = cb.tokens.find(t => t.offset >= cb.offset && t.nestingId === cb.currNestingId && t.text === ")");
