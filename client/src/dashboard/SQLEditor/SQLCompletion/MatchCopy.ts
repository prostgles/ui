...
const KWDOptions = [
  ...
  { kwd: "FORCE_QUOTE", options: ["*"], dependsOn: "CSV",  docs: "Forces quoting to be used for all non-NULL values in each specified column. NULL output is never quoted. If * is specified, non-NULL values will be quoted in all columns. This option is allowed only in COPY TO, and only when using CSV format." },
  { kwd: "FORCE_NOT_NULL", options: ["*"],dependsOn: "CSV", docs: "Do not match the specified columns' values against the null string. In the default case where the null string is empty, this means that empty values will be read as zero-length strings rather than nulls, even when they are not quoted. This option is allowed only in COPY FROM, and only when using CSV format." },
  { kwd: "FORCE_NULL", options: ["*"],dependsOn: "CSV", docs: "Match the specified columns' values against the null string, even if it has been quoted, and if a match is found set the value to NULL. In the default case where the null string is empty, this converts a quoted empty string into NULL. This option is allowed only in COPY FROM, and only when using CSV format." },
  { kwd: "ENCODING", options: ENCODINGS.map(label => ({ label })), docs: "Specifies that the file is encoded in the encoding_name. If this option is omitted, the current client encoding is used. See the Notes below for more details." },
  { kwd: "WHERE", docs: "where condition is any expression that evaluates to a result of type boolean. Any row that does not satisfy this condition will not be inserted to the table. A row satisfies the condition if it returns true when the actual row values are substituted for any variable references. \
...
export const MatchCopy: SQLMatcher = {
  ...
  result: async ({cb, ss, setS, sql }) => {
    ...
    if(prevTokens.some(t => t.type === "string.sql" || t.textLC.includes("$")) || prevText.endsWith("'")){
    ...
    if(prevLC.endsWith("(") || prevLC.endsWith(",")){
      ...
      return {
        suggestions: ss.filter(s => s.type === "column" && !columns.includes(s.name) && prevText.includes(s.escapedParentName as any)).map(s => ({
          ...s,
    ...
    if(cb.tokens.length === 2){
      ...
      const colSuggestion = { label: { label: "( columns... )" }, insertText: "( $0 )" }
      if(!prevText.includes("(")){
        if(tableName){
          const cols = ss.filter(s => s.type === "column" && s.escapedParentName?.includes(tableName));
          if(cols.length){
            colSuggestion.insertText = `( ${cols.map(c => c.insertText).join(", ")} )`
          }
        }
      }
      return suggestSnippets([{ label: "FROM" }, ...(prevText.includes("(")? [] : [colSuggestion])]);
    }

    return withKWDs([
      { kwd: "COPY", expects: "table" },
      { kwd: "FROM", options: [
        ...(cb.currToken?.type === "string.sql"? [] : [{ label: "PROGRAM", kind: getKind("keyword") }]),
          ...(cb.prevTokens.length > 2? await getPathSuggestions(cb, sql, getKind) : [])
        ] 
      },
      {
        kwd: "PROGRAM",
...
const getPathSuggestions = async ({ currToken }: CodeBlock, sql: SQLHandler | undefined, getKind: GetKind): Promise<ParsedSQLSuggestion[]> => {
  ...
  return suggestSnippets(dirOrfiles.map(d => ({
...
export const suggestDirsAndFiles = async (sql: SQLHandler, lastWord = ""): Promise<DirFilesResult[] | { label: string; hint?: string }> => {
  ...
  try {
    try {
      dirs = await sql("set statement_timeout to 200; SELECT pg_ls_dir(${baseDir}) as path", { baseDir }, { returnType: "rows" }) as any;
    } catch(err: any){
      dirs = [
        {
          path: "",
    ...
    dirs = await Promise.all(dirs
      ...
      .filter(d => !["swapfile.sys", "pagefile.sys", "DumpStack.log.tmp"].includes(d.path) && !d.path.endsWith(".sys"))
      .map(async d => {
        const res = {
          ...d,
        ...
        try {
          ...
          const f = await sql("set statement_timeout to 200; SELECT *, pg_size_pretty(size::bigint) as size_pretty from pg_stat_file(${baseDir})", { baseDir: fileOrDirPath }, { returnType: "row" }) as DirOrFile;
          const isdir = f.isdir;
          ...
          if(!isdir && nameParts.length > 1 && ext!.length < 4){
            ...
            if(ext?.toLowerCase() === "csv"){
              ...
              `;
              const head = await sql(q, { command: `head -n 1 ${baseDir}${d.path}` }, { returnType: "row" }) as { val?: string };
              if(head.val){
                firstRow = head.val
              }
            }
    ...
    await sql("set statement_timeout to DEFAULT")
    error = errRaw instanceof Error? errRaw.message : isObject(errRaw)? errRaw.err_msg : JSON.stringify(errRaw);
    return { label: error }
  }
