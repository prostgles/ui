...
const KWDS = [
  ...
  { kwd: "DEFAULT VALUES", justAfter: ["INTO"], expects: "column", excludeIf: ["("] },
  ...
  { kwd: "ON CONFLICT", justAfter: ["VALUES", "SELECT"], options: ["DO NOTHING", "DO UPDATE"], 
  ...
  result: async ({ cb, ss, setS, sql }) => {
    ...
    if (prevLC.trim().toLowerCase().endsWith("insert into")) {
      ...
      return {
        suggestions:
          (clone(tables).map(s => {
            s.label = { ...(isObject(s.label) && s.label), label: s.name + " (..." },
            s.insertText += ` (${s.cols?.filter(c => !(c.has_default && ["u", "p"].includes(c.cConstraint?.contype as any))).map(c => c.escaped_identifier).join(", ")})\nVALUES`;
