...
export const MatchGrant: SQLMatcher = {
  ...
  result: async ({ cb, ss, getKind, setS }) => {
    ...
    if(cb.prevTokens.length === 2 && cb.prevTokens[1]?.type !== "keyword.sql" && cb.prevTokens[1]?.type !== "operator.sql"){
      return withKWDs([
        {
    ...
    const addAll = (objects: readonly typeof PG_OBJECTS[number][]) => {
      const allEntities: typeof PG_OBJECTS[number][] = ["TABLE", "FUNCTION", "SEQUENCE"];
      const matching = allEntities.filter(e => objects.includes(e));
      if(matching.length){
        return [
          ...matching.map(obj => `ALL ${obj}S IN SCHEMA`),
          ...objects
        ]
      }
    ...
    const kwds = [
      ...
      ...Object.entries(PRIVILEGES).map(([kwd, { objects }]) => ({
        kwd: `${kwd} ON`,
        ...excludePriviledge,
      ...
      ...PG_OBJECTS.map(kwd => ({
        kwd,
        exactlyAfter: ["ON"],
        expects: kwd === "TABLE"? ["table", "view"] : kwd.toLowerCase() as any,
      })),
      { 
        kwd: "IN SCHEMA", 
...
const PRIVILEGES = {
  ...
  USAGE:   { 
    ...
    `,
    objects: ["DOMAIN", "FOREIGN DATA WRAPPER", "FOREIGN SERVER", "LANGUAGE", "SCHEMA", "SEQUENCE", "TYPE"],
  },
  SET:  { 
