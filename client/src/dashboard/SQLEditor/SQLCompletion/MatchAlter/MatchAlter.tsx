...
export const MatchAlter: SQLMatcher = {
  ...
  result: async (args) => {
    ...
    const identifiers = cb.tokens.filter(t => t.type === "identifier.sql").map(t => t.text);
    
    if(prevLC.includes("add constraint") && prevLC.includes("(")){
      return getExpected("column", cb, ss);
    }

    if((lltoken?.textLC === "alter" || lltoken?.textLC === "drop") && ltoken?.textLC === "constraint" || cb.prevLC.includes(" constraint ")){
      return {
        suggestions: ss.filter(s => s.type === "constraint" && identifiers.includes(s.constraintInfo!.escaped_table_name!))
      }
    }
    ...
    const addColColumnTokenIdx = cb.prevTokens.findIndex((t, i, arr) => t.textLC === "column" && arr[i-1]?.textLC === "add");
    if(addColColumnTokenIdx > -1 && !cb.thisLineLC.includes("references")){
      ...
      if(!tokensAfter.length){
        return suggestSnippets(getNewColumnDefinitions(ss).concat([{ label: "$new_column_name" }]).map(v => ({ ...v, kind: getKind("column") })));
      }
    ...
    if(prevLC.endsWith("set storage")){
      return suggestSnippets(["PLAIN", "EXTERNAL", "EXTENDED", "MAIN"].map(label => ({ label })))
    }

    if (prevTokens.some(t => ["reset", "set"].includes(t.textLC))) {
      if(["reset", "set"].includes(cb.ltoken?.textLC ?? "")){
        return { suggestions: setS.map(s => ({ 
            ...s, 
      ...
      if(cb.l2token?.textLC === "set"){
        ...
        if(setting){
          if(setting.settingInfo?.enumvals){
            return suggestSnippets(setting.settingInfo.enumvals.map(label => ({ label })))
          }

          return suggestSnippets([{ label: "$value", docs: setting.settingInfo?.description }])
        }
      }
    }
    ...
    if (prevLC.endsWith("alter database")) {
      ...
      if(prevLC.includes("set") && ltoken?.textLC !== "set"){
        if(!prevTokens.some(t => t.textLC === "to")) return suggestSnippets(["TO"].map(label => ({ label })))
        return suggestSnippets(["DEFAULT"].map(label => ({ label })))
      }
      return suggestSnippets(["SET", "RESET", "RESET ALL"].map(label => ({ label })))
    }

    if (prevLC.endsWith("alter")) {
      return suggestSnippets(PG_OBJECTS.flatMap(label => [" IF EXISTS", ""].map<MinimalSnippet>(ifEx => ({ label: `${label}${ifEx}`, kind: getKind(label.toLowerCase() as any) }))));
    } else if (cb.tokens.length === 2 || cb.textLC.endsWith("exists") && cb.tokens.at(-1)?.type === "operator.sql") {
    ...
    } else if (cb.prevText.endsWith(" ")  &&   ( lastToken?.type === "identifier.sql" || lastToken?.type === "delimiter.parenthesis.sql" || lastToken?.type === "identifier.quote.sql")) {

      if(cb.prevText.includes("(") && cb.prevLC.includes("constraint")){
        const cols = ss.filter(s => s.type === "column" && identifiers.includes(s.parentName!));
        return {
          suggestions: cols.length? cols : ss.filter(s => s.type === "column" || s.type === "operator")
        }
      }
      ...
      if (expect.includes("function") || expect.includes("table")) {
        r.suggestions = r.suggestions.concat(
          suggestSnippets([`RENAME TO $new_${expect}_name`, "OWNER TO ", `SET SCHEMA $1`, `SET search_path = $1`, `DEPENDS ON EXTENSION `].map(label => ({ label, kind: getKind("keyword") }))).suggestions
        );
      }
...
const USER_OPTIONS = [
  ...ROLE_OPTIONS.filter(r => r.params.some((p: typeof ROLE_OPTIONS[number]["params"][number]) => p === "CREATEDB" || p === "VALID UNTIL 'timestamp'" || p === "SYSID uid" || p === "ENCRYPTED PASSWORD 'password'" || p === "IN GROUP")),
  { 
...
export const getUserOpts = (prevLC: string, ss: ParsedSQLSuggestion[], kind: number) => {
  const prevWords = prevLC.split(" ");

  if(prevLC.endsWith("role") || prevLC.endsWith("admin")  || prevLC.endsWith("group")){
    return {
  ...
  const nonUsedOptions = ROLE_OPTIONS
    .filter(o => !prevWords.some(w => (o.params as readonly string[]).includes(w.toUpperCase())))
  const labels = nonUsedOptions.flatMap(o => o.params.map(p => ({ 
    label: prevWords.includes("with")? `${p} ` : `WITH ${p} `, 
    docs: o.docs, 
  }))).concat([{ label: "SET", docs: "Set user setting" as any }]);


  return suggestSnippets(labels.map((l, i)=> ({
    ...l,
    kind,
