...
export const suggestSnippets = (mSnippets: MinimalSnippet[], opts?: { range?: IRange; cb?: CodeBlock; }): { suggestions: ParsedSQLSuggestion[] } => {
  const { range } = opts ?? {}; 
  return {
    suggestions: mSnippets.map(({ label, insertText, insertTextRules = 4, docs, sortText, kind, commitCharacters }) => {
...
export const CREATE_OR_REPLACE = [
  "FUNCTION", "PROCEDURE", "LANGUAGE", "VIEW",
...
const getVariations = (variations: string[], end: string): MinimalSnippet[] => variations.flatMap(v => ({ label: v + " ...", insertText: v + " " + end }));

export const createStatements = {
  TABLE: "${1:table_name} (\n  $0\n);",
  VIEW: "${1:view_name} AS\n  SELECT ",
  FUNCTION: "${1:new_func_name} (${2:input_arguments})\n RETURNS VOID AS $$\nBEGIN\n\n $3\n\nEND;\n$$ LANGUAGE plpgsql;",
  TRIGGER: [
    "${1:trigger_name}",
...
export const CREATE_SNIPPETS = [
  ...
  ...getVariations([
    "VIEW", 
    "MATERIALIZED VIEW", 
  ], createStatements.VIEW),

  ...CREATE_OR_REPLACE
    .flatMap(what => `OR REPLACE ${what}`)
    .map(label => ({ label, insertText: label + " " })),

  ...getVariations([
    "FUNCTION"
  ], createStatements.FUNCTION),

  ...getVariations([
    "TRIGGER"
  ], createStatements.TRIGGER),

  ...getVariations([
    "EVENT TRIGGER"
  ], createStatements["EVENT TRIGGER"]),
  ...
  ...orReplace({
    label: "RULE",
    docs: `A rule causes additional commands to be executed when a given command on a given table is executed.\n\n` + asSQL(
...
`)
  ...
  ...PG_OBJECTS
    .filter(v => ![ "POLICY", "EXTENSION", "MATERIALIZED VIEW", "UNIQUE", "INDEX", "ROLE", "USER", ...getKeys(createStatements)].includes(v))
    .map(label => ({ label, insertText: label }))
