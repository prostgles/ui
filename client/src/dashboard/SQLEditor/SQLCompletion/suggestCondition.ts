...
import { KNDS, getKind, type SQLMatchContext, type SQLMatcherResultArgs, type SQLMatcherResultType } from "./registerSuggestions";
...
export const suggestCondition = async (
  ...
  const prevKwdTokens = [...prevTokens].reverse().filter((t, i, arr)=> {
    ...
    const isKwd = ["keyword.choice.sql", "keyword.sql"].includes(t.type) && nextToken?.textLC !== "::" || t.textLC === "join";
    if(cb.ftoken?.textLC === "select"){
      return isKwd && !keywordsThatActAsIdentifiersWhenInSelect.includes(t.textLC);
    }
    return isKwd
  });
  ...
  const conditionPrecedingKeywords = ["where", "on", "having", "when", "using", "check"];
  if(conditionPrecedingKeywords.includes(prevKwdToken?.textLC ?? "") && func){
    const funcSuggestions = await suggestFuncArgs({ cb, parentCb, ss, setS, sql });
    if(funcSuggestions){
  ...
  const getPrevCol = async (colName: string | undefined) => {
    ...
    const expr = await getTableExpressionSuggestions({ parentCb, cb, ss, sql }, "columns");
    const col = expr.columnsWithAliasInfo.find(({ alias, s }) => [s.escapedIdentifier, `${alias}.${s.escapedIdentifier}`].includes(colName));
    return col?.s;
  }
  const suggestColumnByType = async (prevColToken?: TokenInfo) => {
    const colLike = await suggestColumnLike({ cb, ss, parentCb, setS, sql });
    if(ltoken?.type === "operator.sql" && l1token?.type === "identifier.sql" || prevColToken){
      ...
      if(prevCol){
        return { 
          suggestions: colLike.suggestions.map(s => ({ 
            ...s, 
            sortText: s.colInfo? 
              (s.colInfo.data_type === prevCol.colInfo?.data_type? "a" : "b") : 
    ...
    if(prevCol?.colInfo){
      ...
      const leftColOperators = ops
        .filter(o => {
          ...
          return !o.operatorInfo ||  // No info means matches all
            leftTypes?.some(t => 
