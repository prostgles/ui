...
const getAliasToken = (tokens: SQLMatchContext["cb"]["tokens"], expressionLastTokenIdx: number) => {
...
export const getTabularExpressions = ({ cb: _cb, ss, parentCb }: Pick<GetTableExpressionSuggestionsArgs, "ss" | "cb" | "parentCb">, require: "columns" | "table") => {
  ...
  if(_cb.currNestingId){
    ...
    if(func && preSubQueryKwds.includes(func.func.textLC)){
      ...
      const startTokenIdx = _cb.tokens.findLastIndex((t, i) => {
        ...
        return t.offset <= _cb.currOffset && t.nestingId === _cb.currNestingId && prevToken && prevToken.nestingId !== _cb.currNestingId;
      });
      const startToken = _cb.tokens[startTokenIdx];
      if(startToken){
        const lastTokenIdx = _cb.tokens.findIndex((t, i) => {
          const nextToken = _cb.tokens[i-1];
          const r = t.offset > startToken.offset && t.nestingId === _cb.currNestingId && nextToken?.nestingId !== _cb.currNestingId;
          return r;
        });
        tokens = tokens
          .slice(startTokenIdx, lastTokenIdx)
          .map(t => ({ 
            ...t,
  ...
  expressions = [
    ...expressions,
    ...getExpressions(tokens, _cb, ss, parentCb),
  ];
...
const getExpressions = (tokens: TokenInfo[], cb: CodeBlock, ss: ParsedSQLSuggestion[], parentCb: CodeBlock | undefined) => {

  const getQuerySection = (firstToken: TokenInfo, lastToken: TokenInfo, includeEnd = false) => {
    const textOffset = cb.blockStartOffset
    return (parentCb ?? cb).text.slice(firstToken.offset - textOffset, (includeEnd? lastToken.end : lastToken.offset) - textOffset)
  }
  const getClosingParenthesis = (fromIdx: number) => {
    const closingParenthesesIdx = tokens.findIndex((nt, idx) => idx >= fromIdx && nt.text === ")" && !nt.nestingId);
    ...
    if(prevToken && !prevToken.nestingId && kwd){
      ...
      /** Function */
        ...
        if(func && !otherFuncs.length){
          ...
          if(!closing) return;
          const getQuery = (selectedColumns = "*") => `SELECT ${selectedColumns} FROM ${t.text}(${closing.query})`
          expressions.push({
            type: "function",
            kwd,
            func,
      ...
      /** Subquery */
        ...
        if(kwd === "lateral"){
          ...
          if(isWith && isWithAsSectionFinished){
            ...
            const withEndToken = tokens.findLast(t => selectKwd && t.textLC === ")" && !t.nestingId && t.offset < selectKwd.offset);
            isWithStart = !withEndToken? "" : getQuerySection(tokens[0]!, withEndToken, true) + "\n";
          }
      ...
      /** Table or view or CTE alias */
        ...
        if(matchingTable && !otherTables.length){
          const columns = ss.filter(s => s.type === "column" && s.escapedParentName === matchingTable.escapedIdentifier);
          if(columns.length){
            expressions.push({
              type: "tableOrView",
              kwd,
              ...
              getQuery: (selectedColumns = "*") => `SELECT ${selectedColumns} FROM ${matchingTable.escapedIdentifier} ${alias? alias : ""}`
            });
          }
        /** CTE alias */
        } else if (!matchingTable && isWith && isWithAsSectionFinished) {
          const matchingWith = expressions.find(e => e.alias === t.text && e.kwd === "as");
          if(matchingWith){
            expressions.push({
              ...matchingWith,
              kwd,
