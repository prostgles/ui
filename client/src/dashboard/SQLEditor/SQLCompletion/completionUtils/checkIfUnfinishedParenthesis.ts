...
export const checkIfUnfinishedParenthesis = ({ allLines, startLineNumber, endLineNumber }: { startLineNumber: number; endLineNumber: number; allLines: LineInfo[] }) => {
  const initialText = allLines.filter(l => l.n >= startLineNumber && l.n <= endLineNumber).map(l => l.v).join("\n");
  ...
  if(initialCounts.diff){
    ...
    const remainingLines = extendStart? allLines.slice(0, startLineNumber).reverse() : allLines.slice(endLineNumber);
