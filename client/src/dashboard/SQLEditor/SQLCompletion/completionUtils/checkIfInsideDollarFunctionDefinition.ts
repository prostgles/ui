...
export const checkIfInsideDollarFunctionDefinition = ({ allLines, lineNumber, currOffset, offsetLimits, editor, eol, ...limits }: Args) => {

  /** If inside function select function body so must disregard ";" */
  if(allLines.some(l => l.v.includes("$"))){
    let { tokens: allTokens } = getTokens({ editor, eol, lines: allLines.map(l => l.v), currOffset });
    if(offsetLimits){
    ...
    if(funcToken){
      ...
      const firstDollarQuote = allTokens.find(t => funcToken.offset <= t.offset && t.lineNumber <= limits.endLine && t.text.startsWith("$") && t.text.endsWith("$"));
      const secondDollarQuote = allTokens.find(t => firstDollarQuote && t.offset > firstDollarQuote.offset && t.text === firstDollarQuote.text);
      if(firstDollarQuote && secondDollarQuote && (secondDollarQuote.offset >= currOffset || secondDollarQuote.lineNumber === lineNumber)){
        return {
