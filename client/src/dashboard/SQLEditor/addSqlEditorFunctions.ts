...
export const addSqlEditorFunctions = async (editor: editor.IStandaloneCodeEditor, smallestBlock: boolean) => {
  ...
  editor.addAction({
    ...
    run: async (editor) => {
      ...
      if(!model || !position) return;
      let cb = await getCurrentCodeBlock(model, position, undefined, { smallestBlock });
      if(smallestBlock){
        const selection = editor.getSelection();
        if(selection){
          const selectedText = editor.getModel()?.getValueInRange(selection).trim();
          if(selectedText?.trim() === cb.text.trim()){
            cb = await getCurrentCodeBlock(
              model, 
              position, 
              undefined, 
              { smallestBlock: true, expandFrom: { startLine: selection.startLineNumber, endLine: selection.endLineNumber } }
            );
          }
        }
      }
...
export const getSelectedText = (editor: editor.ICodeEditor | editor.IStandaloneCodeEditor | undefined): string => {
  if(!editor) return "";
  const model = editor.getModel();
  const selection = editor.getSelection();
