...
function triggerCommand(attrPath: string[]) {
  const elem = document.querySelector<HTMLElement>(`[data-command="${attrPath[0]}"]`);
  if (elem) {
    elem.focus();
    setTimeout(() => {
      elem.click();
...
export type SearchAllProps = Pick<Prgl, "db" | "methods" | "tables"> & {
  ...
  onOpenDBObject: (suggestion: SearchAllSuggestion | undefined, method_name?: string) => void;
  style?: object;
  className?: string;
...
export class SearchAll extends RTComp<SearchAllProps, S> {
  ...
  constructor(props: SearchAllProps) {
    ...
    this.state = {
      searchTerm: "",
      sType: props.searchType ?? "rows" as typeof SEARCH_TYPES[number]["key"],
      matches: [],
      loading: false,
      rows: [],
      cols: [],
  ...
  static renderRow = (matchRow: Match["match"] | any, key: string | number) => {
    ...
    return <div key={key} className="flex-row ws-pre f-1 ">
      ...
      {matchRow.map((r, i) => {
        if (typeof r === "string") {
          ...
          const style: React.CSSProperties = i ? { flex: 1 } : { display: "flex", flex: noRightText ? undefined : 1, justifyContent: noRightText ? "start" : "end" }
          ...
          return <span key={i}
            ...
            className={"fs-1 min-w-0 text-1 text-ellipsis" + (!i ? " " : " ta-left")}
          >
            {r}
          </span>;
          ...
          return <strong key={i} className="f-0">{r[0]}</strong>;
    ...
    this.searching = setTimeout(async () => {
      ...
      if (sType === "rows" && db.sql) {
        ...
        for (const k of tablesToSearch) {
          ...
          if (tableHandler?.find && this.mounted && colsToSearch.length && term === this.searchTerm) {
            const { matchCase } = this.state;

            this.setState({ currentSearchedTable: k });
            const s = { $term_highlight: [colsToSearch, term, { edgeTruncate: 30, matchCase, returnType: "object" }] },
              filter = { $term_highlight: [colsToSearch, term, { matchCase, returnType: "boolean" }] },
              res = await tableHandler.find(filter, { select: { $rowhash: 1, s }, limit: 3 }); /** Search top 3 records per table */

            const _matches = res.filter(r => r.s).map(r => {
              const colName = Object.keys(r.s)[0]!
              return {
                $rowhash: r.$rowhash,
                table: k,
                colName,
                match: r.s[colName].map((v, i) => !i ? `${colName}: ${v}` : v)
              }
            })
  ...
  getSearchListProps() {
    ...
    if (sType === "views and queries") {
      ...
      items = searchItems.filter(s => s.type === "table" && s.schema === "public" && objTypesToSearch.includes("tables"))
        .map(s => ({
        ...
        })).concat((objTypesToSearch.includes("queries") ? (queries ?? []) : []).map(q => ({
          key: q.id,
          label: q.name,
          subLabel: q.sql || "",// sliceText(q.sql || "", 200) ,
          contentLeft: (<div className="f-0">
            <Icon className="text-1p5 p-p25" size={1.5} path={mdiScriptTextPlay} />
          </div>),
          onPress: (e, term) => {
            this.props.onClose();
            let extra = {}
            if (q.sql && term && q.sql.toLowerCase().includes(term.toLowerCase())) {
              const lines = q.sql.split("\n").map(l => l.toLowerCase())
              const lineNumber = lines.findIndex(s => s.includes(term.toLowerCase()));
          ...
          Object.entries(this.props.methods as Record<string, MethodFullDef>)
            ...
            .map(([methodKey, method]) => ({
              ...
              contentLeft: (<div className="f-0">
                <Icon className="text-1p5 p-p25" size={1.5} path={mdiFunction} />
              </div>),
              onPress: (e, term) => {
                this.props.onClose();
      ...
      items = SQL_SNIPPETS
        .map(c => {
          ...
          const res: Required<SearchListProps>["items"][number] = {
            ...
            label: c.label,
            subLabel: c.info + "\n" + c.sql,
            contentLeft: <Icon path={mdiFileCogOutline} size={1.5} className="mr-p5 text-1p5" />,
            // mdiFileCogOutline
  ...
  render() {
    ...
    const { loading = false, sType, tablesToSearch, allTablesToSearch, currentSearchedTable, matchCase, objTypesToSearch } = this.state;
    ...
    const content = <>
      <div className="flex-row aai-start w-full min-h-0" style={{ width: "hh550px", maxWidth: "88vw", alignSelf: "center" }}>
        ...
        <div className="flex-col min-w-0 min-h-0 f-1">
          ...
          {sType === "commands" ? null : (!loading || sType !== "rows") ? <div style={{ width: "34px", height: "34px" }}></div> :
    ...
    return (
      <Popup
        ...
        rootStyle={{
          minHeight: "50vh", top: margin, right: margin, left: margin,
