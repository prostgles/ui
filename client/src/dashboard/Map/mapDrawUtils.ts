import { mdiEllipseOutline, mdiMapMarker, mdiRectangleOutline, mdiShapePolygonPlus, mdiSquareOutline, mdiVectorPolyline } from "@mdi/js";
...
} as const satisfies Partial<Record<keyof AllDrawModes, {
  label: string;
  iconPath: string;
}>> ;
...
export const geometryToGeoEWKT = (f: GeoJSONFeature["geometry"], srid?: number) => {
  if(f.type === "GeometryCollection") {
    return { $ST_GeomFromEWKT: [`${srid? `SRID=${srid};`: ""}GEOMETRYCOLLECTION(${f.geometries.map(g => geometryToGeoEWKT(g, srid))})`] };
  }
