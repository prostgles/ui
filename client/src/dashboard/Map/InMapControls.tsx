...
type P = Pick<DecKGLMapProps, "tileAttribution" | "onOptionsChange" | "topLeftContent" | "options" | "edit"> & {
  deckGlLibs: DeckGlLibs;
  fitBounds: VoidFunction;
  deckW: DeckWrapped;
  onRenderLayer: (layer: GeoJsonLayer<any, {}> | undefined) => void
};
export const InMapControls = ({ tileAttribution, topLeftContent, onOptionsChange, options, deckGlLibs, edit, fitBounds, deckW, onRenderLayer }: P) => {
  ...
  useEffect(() => {
    ...
    const sub = deckW.currHoverRState.subscribe(hover => {
      ...
      refCursor.current.innerText = !hover?.coordinate? "" : `${hover.coordinate.map(v => v.toString().padStart(3, "0")).join("\n")}`;
    });
    return sub.unsubscribe;
  }, [showCursorCoords, deckW.currHoverRState]);

  const onRenderDrawnLayer = useCallback((editedFeaturesLayer: GeoJsonLayer<any, {}> | undefined) => {
