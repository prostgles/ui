...
export const CreateTable = ({ prgl:{db, tables}, onClose, suggestions }: DashboardMenuProps & { onClose: VoidFunction; }) => {
  ...
  const [tableOpts, setTableOpts] = useState<Partial<{
    name: string;
    cols: ColumnOptions[];
    finished: boolean;
  }>>({});
  ...
  const duplicateColumn = tableOpts.cols?.find((c, i) => tableOpts.cols?.some((_c, _i) => isDefined(c.name) && c.name === _c.name && i !== _i));
  const duplicateTable = tables.find(t => t.name === tableOpts.name);
  const error = success? undefined : duplicateTable? "A table with this name already exists" :
    duplicateColumn? `Cannot have two columns with the same name ${JSON.stringify(duplicateColumn.name)}` : "";
  ...
  const getColSQL = (c: ColumnOptions) => {
    ...
    return "  " + [
      ...
      firstRef? getReferencesQuery(firstRef).filter(v => v).join(" ") : "",
      // c.check?.trim() ? `CHECK(${c.check})` : ""
    ].filter(v => v).join(" ");
  }
  const query = `CREATE TABLE ${JSON.stringify(tableOpts.name)} ( \n${tableOpts.cols?.map(c => `  ${getColSQL(c)}`).join(", \n") ?? ""} \n);`;

  const updateColumn = (columnIndex: number, columnChanges: Partial<ColumnOptions> | undefined) => {
    ...
    setTableOpts(topts => 
      ({
        ...topts, 
        cols: (topts.cols ?? [])
          .map((c, i)=> i === columnIndex? (columnChanges && { ...existingColumn, ...columnChanges }) : c )
          .filter(isDefined)
      })
    )
  }

  const dataTypeSuggestions = useMemo(() => suggestions?.suggestions.map(s => s.dataTypeInfo).filter(isDefined), [suggestions]);

  const [activeColumnInfo, setActiveColumnInfo] = useState<{ index: number; anchor: HTMLElement; isAddingNew: boolean }>();
  const activeColumn = activeColumnInfo && tableOpts.cols?.[activeColumnInfo.index];
  const activeColumnIndex = activeColumnInfo?.index;
  const displayedColumns = tableOpts.cols?.filter((c, i)=> c.name && !(activeColumnInfo?.isAddingNew && activeColumnInfo.index === i));
  return <Popup 
  ...
  {activeColumn && isDefined(activeColumnIndex) && tableOpts.name &&
    <Popup
      title={!activeColumnInfo.isAddingNew? `Edit ${activeColumn.name}` : "Add new column"}
      ...
      footerButtons={[
        ...
        { 
          ...
          disabledInfo: !activeColumn.name? "Name required" : !activeColumn.dataType? "Data type required" : undefined, 
          onClick: () => {
            setActiveColumnInfo(undefined);
          }, 
        }
    ...
    >
      <ColumnEditor 
        ...
        onEditReference={(r, i) => {
          updateColumn(activeColumnIndex, {
            references: activeColumn.references!.map((_r, _i) => {
              if(i === _i){
                return r;
              }
  ...
  {success? <SuccessMessage message={`Created ${tableOpts.name}`} /> : <>
    <FlexCol className="flex-col gap-1 p-1 pr-2 pb-2">
      ...
      {tableOpts.name && !!displayedColumns?.length && 
        <div className="flex-col pl-p5 gap-1">
          ...
          {displayedColumns.map((colOpt, colI) => {
            const colDataType = dataTypeSuggestions?.find(dt => dt.udt_name.toLowerCase() === colOpt.dataType?.toLowerCase())
            return <Btn 
              key={colI}
              title="Press to edit"
              variant="faded" 
              iconPath={getColumnIconPath({ is_pkey: colOpt.isPkey, references: colOpt.references as any, udt_name: colOpt.dataType as any })} 
              onClick={({ currentTarget }) => setActiveColumnInfo({ index: colI, anchor: currentTarget, isAddingNew: false })}
            >
              {getColumnDefinitionQuery({ ...colOpt, dataType: colDataType?.udt_name ?? colOpt.dataType ?? "" })}
            </Btn>
          })}
      ...
      {tableOpts.name && <Btn 
        ...
        onClick={({ currentTarget }) => { 
          ...
          setActiveColumnInfo({ anchor: currentTarget, index: (tableOpts.cols?.length ?? 0), isAddingNew: true })
        }}
      >Add column</Btn>}
    ...
    <FlexRow className="p-1">
      ...
      {error? <InfoRow color="danger" className="mx-2 mb-1">{error}</InfoRow> : 
        tableOpts.finished? 
          <SQLSmartEditor
            asPopup={false}
            title=""
            onCancel={() => setTableOpts(topts => ({ ...topts, finished: false })) }
            key={query}
            sql={db.sql!} 
            query={query} 
            suggestions={suggestions}
