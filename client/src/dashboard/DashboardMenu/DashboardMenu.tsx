...
import type { CommonWindowProps, DashboardProps, DashboardState, _Dashboard } from "../Dashboard/Dashboard";
...
export const DashboardMenu = ({ menuAnchorState, ...props }: Omit<DashboardMenuProps, "localSettings" | "anchor"> & { menuAnchorState: ReactiveState<HTMLElement | undefined>; }) => {
  const { state: menuAnchor, setState } = useReactiveState(menuAnchorState);

  const [showSearchAll, setShowSearchAll] = React.useState<DashboardMenuState["showSearchAll"]>();
  const { 
    suggestions,tables, loadTable, workspace, prgl, 
  } = props;
  const { db, dbs, } = prgl;

  const filter = workspace.options.showAllMyQueries? {} : { workspace_id: workspace.id };
  const { data: windows } = dbs.windows.useSync!(filter, { handlesOnData: true, select: "*", patchText: false  });
