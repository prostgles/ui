  ...
  const tablesWithInfoNonSorted = usePromise(async () => {
    return (await Promise.all(tables
      .map(async t => {
        ...
        let count = 0;
        let sizeNum = 0;
        const countRequestedAndAllowed = tableListEndInfo === "count" && db[t.name]?.count;
        ...
        try {
          count = Number((shouldGetCount? await db[t.name]?.count?.() : 0) ?? 0);
          sizeNum = Number(tableListEndInfo === "size"? await db[t.name]?.size?.() : 0);
        ...
        (tableListEndInfo === "size"? 
          (sizeNum === 0? "0" : bytesToSize(sizeNum, 0)) : 
