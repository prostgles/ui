...
export const GeoFilter = ({ filter, db, tables, onChange,  }: BaseFilterProps) => {
  ...
  const updateValue = (newArgs: typeof args) => {
    ...
    const disabled = !newValue.distance || ![newValue.lat, newValue.lng, newValue.distance].every((v, i, arr) => arr.length && Number.isFinite(v));
    onChange({
      disabled,
      ...filter!,
      value: newValue,
  ...
  const [refPoint, setRefPoint] = useState<{ table?: DBSchemaTablesWJoins[number]; row?: AnyObject; }>({ table: firstGeoTable });
  ...
  return <div className="flex-row p-p5 gap-p5 ai-center">
    <FormFieldDebounced 
      placeholder="Km"
      style={{ padding: 0 }}
      inputProps={{
    ...
    {!!args.distance && <>
      <div>of</div>
      <Btn variant="faded"
        onClick={e => { setOpen(open? undefined : e.target as any) }}
      >
      ...
      >
        <FlexRow>
          <FormField type="number" value={args.lat} label={"Latitude"} onChange={lat => updateValue({ lat, name: "" })} />
          <FormField type="number" value={args.lng} label={"Longitude"} onChange={lng => updateValue({ lng, name: "" })} />
        </FlexRow>
        
        <ExpandSection 
          label="Find from data..."
          iconPath={mdiMagnify}
        >
          ...
          {refPoint?.table && 
            <SmartSearch 
              ...
              onChange={async (val) => {
                ...
                const rowFilter = { $and: filter.map(f => getFinalFilter(f as any)) };
