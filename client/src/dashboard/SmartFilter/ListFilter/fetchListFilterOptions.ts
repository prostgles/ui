...
type Args = Pick<BaseFilterProps, "db" | "column" | "tableName" | "tables" | "otherFilters"> & {
  term: string;
}
export const fetchListFilterOptions = async (args: Args) => {
  ...
  if(finalTableName && db[finalTableName]?.find){
    ...
    if(isTableColumn){
      const tableColumn = tables.find(t => t.name === finalTableName)?.columns.find(c => c.name === finalColumnName);
    ...
    if(isTableColumn && isReference && column.is_nullable && !rawOptions.includes(null)){
