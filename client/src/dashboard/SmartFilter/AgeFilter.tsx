...
import type { DetailedFilterBase, FilterType, SimpleFilter } from "../../../../commonTypes/filterUtils";
import { CORE_FILTER_TYPES, NUMERIC_FILTER_TYPES } from "../../../../commonTypes/filterUtils";
...
export const AgeFilter  = (props: FilterProps) => {
  ...
  if(filter.complexFilter?.type !== "controlled") return <>Something went wrong. Could not find complex filter</>;

  const complex: DetailedFilterBase["complexFilter"] = {
    ...filter.complexFilter,
  }

  const updateComplex = (newOpt: Partial<Extract<DetailedFilterBase["complexFilter"], { type: "controlled" }>>) => {
    onChange({
      ...filter,
      complexFilter: {
        ...complex,
