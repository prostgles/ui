...
import type { DetailedFilterBase, JoinedFilter, SimpleFilter, SmartGroupFilter } from "../../../../commonTypes/filterUtils";
import { isDetailedFilter, isJoinedFilter } from "../../../../commonTypes/filterUtils";
...
export const SmartFilter = (props: SmartFilterProps) => {
  ...
  const content = (!tableColumns?.length? null : 
    <FlexCol  
      key={"o"}  
      style={{
        ...style
    ...
    >
      {detailedFilter.map((filterItem, di)=> {
        ...
        let col: FilterColumn | undefined, fieldName, label, tName = tableName;
        ...
        if(isJoinedFilter(filterItem)){
          ...
          const lastTableName = isObject(lastPathItem)? lastPathItem.table : lastPathItem; 
          const nestedTableCol = tables.find(t => t.name === lastTableName)?.columns.find(c => c.name === fieldName);
          tableCol = nestedTableCol;
          tName = lastTableName;
          label = filterItem.path.map(p => isObject(p)? p.table : p).join(" > ") + "." + fieldName;
        } else {
          ({ fieldName } = filterItem);
          tableCol = tableColumns.find(c => c.name === fieldName);
          const selectedCol = selectedColumns?.find(c => c.name === fieldName);
        ...
        const otherFilters: (SimpleFilter | JoinedFilter | DetailedFilterBase)[] = props.detailedFilter?.filter((f, i)=> i !== di) ?? [];
        const onChangeFilter = (newFilterItem: typeof filterItem | undefined)=> {
        ...
        const filterProps = {
          ...
          otherFilters: isJoinedFilter(filterItem)? otherFilters.filter(isDetailedFilter)
            .map((f: DetailedFilterBase) => {
              const res: JoinedFilter = {
                type: filterItem.type,
                path: [...filterItem.path.slice(0).reverse().slice(1), props.tableName],
                filter: f
              }
              return res;
            }) : 
        ...
        const filterNode = <Filter 
          {...filterProps} 
          rootFilter={isJoinedFilter(filterItem) ? { value: filterItem, onChange: onChangeFilter } : undefined} 
          filter={{ ...filterProps.filter, minimised: props.minimised ?? filterProps.filter.minimised }} 
          tables={tables} 
        />
      ...
      {showAddFilter && 
        <SmartAddFilter 
          {...pickKeys(props, ["db", "tableName", "tables", "itemName", "selectedColumns"])}
          defaultType="="
          style={{ 
            boxShadow: "unset",
          }}
