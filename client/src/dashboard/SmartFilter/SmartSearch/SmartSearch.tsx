...
import type { SearchListItem, SearchListProps } from "../../../components/SearchList/SearchList";
...
export class SmartSearch extends RTComp<P, S> {
  ...
  get filterableCols(): Pick<ValidatedColumnInfo, "name" | "tsDataType" | "udt_name" | "is_pkey">[] {
    ...
    return getFilterableCols(tbl?.columns ?? []).filter(c => 
      columnName === "*" || (columnName && c.name === columnName || cols?.length && cols.includes(c.name) || !cols && !columnName)
    );
  }
  ...
  get column () {
    ...
    const column = isObject(colNameORColConfig)? (colNameORColConfig.computedConfig? colNameORColConfig.computedConfig.column : colNameORColConfig.name) : colNameORColConfig;
  ...
  render(){
    ...
    const { 
      ...
      noResultsComponent, wrapperStyle
    } = this.props;
