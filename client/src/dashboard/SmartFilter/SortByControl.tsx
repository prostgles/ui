...
type SortByControlProps = Pick<React.HTMLAttributes<HTMLDivElement>, "className" | "style"> & {
...
function SortByControl({ onChange, columns, value, fields, style = {}, className = "", buttonClassName = "", btnProps }: SortByControlProps){
  ...
  if(fields?.length){
    ...
    if(bad.length) console.warn("Bad fields provided for SortByControl: ", bad);
  }
  const orderableFields = columns.filter(c => c.filter && (!fields || fields.includes(c.name)));
  const orderAsc = !!value?.asc;
  const orderByKey = value?.key as string;

  return <div className={"Select flex-row gap-p25 min-h-0 f-0 relative ai-center " + className} style={style}>
    <Select
      id="orderbycomp"
