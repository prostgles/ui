...
export const AddTimeChartFilter = ({ onStart, onEnd, chartRef, filter, myActiveRow, activeRowColor = "blue", onCancelActiveRow }: P) => {
    ...
    const onPointerDown = (e: PointerEvent) => {
      ...
      if(newFilter.x1 === undefined){
        ...
        setNewFilter({ 
          ...
          dates: {
            min: new Date(Math.round(chartRef.data.xScale.invert(Math.min(x1, x2)))),
            max: new Date(Math.round(chartRef.data.xScale.invert(Math.max(x1, x2)))),
          }
        });
      }
    }
  ...
  return <>
    ...
    {newFilter && !newFilter.dates && <div ref={divRef} 
      style={{
        position: "absolute",
    ...
    />}
    {activeRow && chartRef.chart && <FlexCol
    ...
    {newFilter?.dates && newFilter.anchor && 
      <Popup 
        ...
        footerButtons={[
          ...
          !filter || newFilter.changed ? undefined : {
            label: "On chart edit",
            color: "action", 
            variant: "filled", 
            onClick: () => {
          ...
          !newFilter.changed && filter? undefined : { 
      ...
      >
        <FlexCol>
          <FormField type="text" value={newFilter.dates.min.toISOString()} onChange={v => setNewFilter({ ...newFilter, changed: true, dates: { ...newFilter.dates!, min: new Date(v) } })} />
          <FormField type="text" value={newFilter.dates.max.toISOString()} onChange={v => setNewFilter({ ...newFilter, changed: true, dates: { ...newFilter.dates!, max: new Date(v) } })}  />
        </FlexCol>
      </Popup>
    }
  </>
...
const TimestampFilter = ({ min, max, style, endIcon }: TimestampFilterProps) => {
  return <FlexRow className="TimestampFilter gap-1 p-p5 rounded ai-start" style={style}>
    ...
    {endIcon ?? <Btn size="small" iconPath={mdiClose} style={{ marginTop: "-8px", marginRight: "-8px" }} />}
  </FlexRow>
}