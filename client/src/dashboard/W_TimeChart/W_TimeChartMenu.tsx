...
const MissingBinsOptions = [
  ...
  { key: "show nearest", subLabel: "Each missing bin will show the nearest existing bin" },
] as const;
...
export type TimechartRenderStyle = typeof TimechartRenderStyles[number]["key"];
...
export const ProstglesTimeChartMenu = ({ w, autoBinSize }: P) => {
  ...
  const displayedBinSize = binSize === "auto" && autoBinSize !== undefined? 
    `Auto (${autoBinSize})` : 
  ...
  return (
    <FlexCol className="p-1">
      <Select
        className="w-fit"
        label="Bin size"
        value={binSize === "auto" && autoBinSize !== undefined? `Auto (${autoBinSize})` : binSize} 
        btnProps={{
      ...
      <FlexRow>
        ...
        <FormField 
          ...
          disabledInfo={showBinLabels === "off"? "Must enable 'Show value labels'" : undefined}
          nullable={true}
          style={{ maxWidth: "150px" }}
          inputProps={{ min: 0, max: 1e3, step: 1 }}
          onChange={v => {
            const binValueLabelMaxDecimals = v? +v : v;
            w.$update({ options: { binValueLabelMaxDecimals }}, { deepMerge: true });
          }}
        />
      </FlexRow>

      {renderStyle === "line" && <Select label="Missing bins" 
