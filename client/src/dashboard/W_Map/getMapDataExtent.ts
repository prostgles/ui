 ...
 */
export const getMapDataExtent: DecKGLMapProps["onGetFullExtent"] = async function(this: W_Map, fromUserClick = false){

  const { prgl: {db}, layerQueries = []} = this.props;
  ...
  for await (const layer of layerQueries){
    ...
    if("tableName" in layer){
      ...
      try {
        _xyExtent = await db.sql(q.sql, q.args, { returnType: "row" }) ?? undefined;
    ...
    const [[_minLat, _minLng], [_maxLat, _maxLng]] = _xyExtent.e?.slice(4, -1)?.split(",").map(v => v.split(" ").map(v => +v)) ?? [];
