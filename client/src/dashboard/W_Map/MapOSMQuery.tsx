...
const getQuery = (type: DataType, subTypeValues: string[] | undefined, limit: number) => {
  const query = predefinedOsmQueries[type];
  let mainQuery = (isObject(query)? `${query.nodeType}[${query.subTypeTag}]` : query) + `(\${bbox});`;
  if(subTypeValues?.length && isObject(query) && subTypeValues.length < query.subTypes.length){
    mainQuery = "(" + subTypeValues.map(v => `${query.nodeType}[${[query.subTypeTag, v].map(s => JSON.stringify(s)).join("=")}](\${bbox});`).join("") + ");";
  }
...
export const MapOSMQuery = ({ bbox, onData }: MapOSMQueryOSMQueryProps) => {
  ...
  const [selectedSubTypes, setSelectedSubTypes] = useState<string[] | undefined>();
    ...
    <FlexCol>  
      <FlexRow>
        ...
        {subTypes && <Select 
          label={"Subtypes"}
          multiSelect={true}
          fullOptions={subTypes.map(s => ({ key: s.value, subLabel: s.info }))}
