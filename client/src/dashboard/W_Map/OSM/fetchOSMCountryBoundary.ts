const fetchCountryBoundary = async (countryName: string) => {
  ...
  try {
    ...
    const relation = data.elements.find(element => element.type === "relation" && element.tags["name:en"]);
    ...
    const coordinates = relation.members
      ...
      .map(way => way.ref)
      .map(ref => data.elements.find(element => element.type === "way" && element.id === ref).nodes
