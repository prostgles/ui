...
import type { GeoJSONFeature, Polygon } from "../../Map/DeckGLMap";
...
type OSMNode = OSMElementBase & {
  type: "node";
  lat: number;
  lon: number;
}

type OSMWay = Omit<OSMElementBase, "type"> & {
  type: "way";
  nodes: number[]; // Array of node IDs
}

type OSMRelationMemberWay = {
  ref: number; // Reference ID of the member element
  type: "way";
  role: "outer" | "inner";
}
type OSMRelationMemberNode = {
  ref: number; // Reference ID of the member element
  type: "node";
  role: "admin_centre" | "label";
}
type OSMRelationMember = OSMRelationMemberWay | OSMRelationMemberNode;

type OSMRelation = OSMElementBase & {
  type: "relation";
...
const fetchElements = async <T extends "node" | "way">(type: T, ids: number[]): Promise<(T extends "node"? OSMNode : OSMWay)[]> => {
  ...
  do {
    batchNodeIds = nonCachedIds.splice(step * limit, limit);
    step++;
    if(!batchNodeIds.length) break;
    const nodeBatch = await fetch(OVERPASS_URL, {
      method: "POST",
      body: `[out:json];${type}(id:${batchNodeIds.join(",")});out;`,
...
export const getOSMDataAsGeoJson = async (responseData: { elements: OSMElement[]; }) => {
  const unique = <T,>(arr: T[]) => Array.from(new Set(arr));
  const wayIds = unique(responseData.elements.flatMap((e) => e.type === "relation"? e.members.flatMap(m => m.type === "way"? m.ref : undefined).filter(isDefined) : undefined).filter(isDefined));
  ...
  // const ways: OSMWay[] = !wayIds.length? [] : await fetch(OVERPASS_URL, {
  //   method: "POST",
  //   body: `[out:json];way(id:${wayIds.join(",")});out;`,
  // })
  // .then(res => res.json())
  ...
  const features: GeoJSONFeature[] = responseData.elements.map((element) => {
    ...
    if (element.type === "node") {
      ...
      const coordinates: [number, number][] = element.nodes.map((nodeId: number) => {
        const node = nodes.find((el: any) => el.id === nodeId);
      ...
      // Assuming multipolygon for simplicity
      ...
      // // Assuming a simple structure where each outer way is a separate polygon
      // // and inner ways are holes in these polygons. This logic will need to be
      // // more complex for real-world data handling.
      // outerWays.forEach(ow => {
      //   const outerCoordinates = [convertWayToCoordinates(ow)];
      //   innerWays.forEach(iw => {
      //     // Here you would typically check if the inner way is within the outer way
      //     // For simplicity, we're just adding all inners as holes to each outer
      //     outerCoordinates.push(convertWayToCoordinates(iw));
      //   });
      //   coordinates.push(outerCoordinates);
      // });

      const outerPolygon: Polygon["coordinates"] = outerWays.map(ow => convertWayToCoordinates(ow));
      // const innerPolygons
      return {
        ...commonProps,
        geometry: {
