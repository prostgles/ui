...
export const setMapLayerData = async function(this: W_Map, dataAge: number ){
    ...
    if(w.options.extent){
      ...
      try {
        ...
        await Promise.all(layerQueries.filter(q => !q.disabled).map(async (q, i) => {
          ...
          if(q.type === "osm"){
            ...
            if(!columns){
              this.setState({ error: "Could not find columns for table: " + tableName });
              return;
            }

            const tableHandler = db[tableName];
            if(!tableHandler?.find || !tableHandler.findOne || !tableHandler.size){
            ...
            if(
              ...
              !this.layerSubs.find(s => s.filter === subFilterStr && s.tableName === tableName)
            ){

              try {
                const sub = await tableHandler.subscribe(subFilter, { limit: 2, throttle: (w.options.refresh.throttleSeconds || 0) * 1000 }, () => {
            ...
            if(!oneRow.l?.type.endsWith("Point")){// && (oneRow?.c?.coordinates || []).flat().flat().flat().length > 30){
              const scale = zoom > 7? 
                d3.scaleLinear().range([0, 0.005]).domain([9, 7]).clamp(true) :
                d3.scaleLinear().range([0.005, 0.05]).domain([7, 1]).clamp(true);
              const size = scale(zoom);
              if(size > 0) {
                opts = { 
                  select: { 
                    ...pickKeys(select, ["i"]), 
                    [MAP_SELECT_COLUMNS.geoJson]: { $ST_Simplify: [geomColumn, size] } 
                  } as any, 
                  limit: AGG_LIMIT };
                }
            }

            const { signature, cachedLayer } = this.getDataSignature({ filter: f.finalFilter, ...opts }, dataAge, q, w.options.aggregationMode);
            ...
            if(oneRow.l?.type === "Point"){
              ...
              if(aggregationMode?.type === "limit"){
                const count = parseInt(await tableHandler.count?.(f.finalFilter) as any);
                willAggregate = (Number.isFinite(count) && count > (aggregationMode.limit || 1000))
              } else {
                // const count = await db[tableName].count(f.finalFilter);
                const size = await tableHandler.size(f.finalFilter, selectGeoJson);
            ...
            if(willAggregate){
              ...
              */
              ...
              const opts = { 
                select: { 
                  c: { "$countAll": [] }, 
                  [MAP_SELECT_COLUMNS.geoJson]: { $ST_SnapToGrid: [geomColumn, size] }
                }, 
                limit: AGG_LIMIT 
              } as const;
              
              const aggRows = await tableHandler.find(f.finalFilter, opts) as Record<keyof typeof opts["select"], any>[];
          ...
          const badRow = data.find(r => typeof r[MAP_SELECT_COLUMNS.geoJson] !== "object");
          if(badRow){
            console.error("Bad GeoJSON data. Expecting type object but got -> " + (typeof badRow.l), badRow);
            data = []
          }
