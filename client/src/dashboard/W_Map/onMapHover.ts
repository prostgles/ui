...
export async function onMapHover(this: W_Map, hoverObj?: AnyObject & HoveredObject, hoverCoords?: HoverCoords){
  const { prgl, tables } = this.props;
  if(!hoverObj && !this.hovering && !this.state.hoverObj){ //  || this.state.clickedItem
    return
  }
  ...
  /** If hovering then wait for the cursor to settle on an object before firing data request */
    ...
    this.hovering = {
      ...
      timeout: setTimeout(async () => {
         ...
         */
        if(this.hovering?.hoverObjStr === hoverObjStr && isObject(hoverObj.properties) && hoverObj.properties.layer){
