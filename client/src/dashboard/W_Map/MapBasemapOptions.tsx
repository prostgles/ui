...
export const MapBasemapOptions = ({ w, prgl, className, asPopup }: P) => {
  ...
  const content = <FlexCol className={className}>
    <ButtonGroup 
      ...
      {tileURLsOrDefaults.map((turl, i) => (
        <div key={i} className="flex-row ai-center py-p5 o-auto">
          <Btn iconPath={mdiDelete}  
            color="danger"
            onClick={() => {
              updateOptions({ tileURLs: tileURLsOrDefaults.filter(t => t !== turl) }) 
            }}
          />
      ...
      <FormField  
        ...
        rightIcons={
          <Btn 
            ...
            iconPath={mdiPlus} 
            onClick={() => {
              const urls = Array.from(new Set([...tileURLsOrDefaults, newTileUrl])).filter(isDefined)
  ...
  return <PopupMenu
    ...
    footerButtons={[
      ...
      },
      {
        ...
        variant: "filled",
        color: "action",
        disabledInfo: isEqual(w.options, localOptions)? "Nothing to update" : undefined,
