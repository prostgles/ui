...
type P = {
  ...
  table: DBSchemaTable;
  setData: (newData: Pick<ValidatedColumnInfo, "name" | "is_pkey" | "tsDataType">, files: (LocalMedia | Media)[]) => void;
 ...
 */
export const SmartFormFileSection = ({ db, table, newRow, defaultData, action, onSuccess, getThisRow, mediaTableName, row, setData, setNewRow }: P) => {
      ...
      if ("id" in media && media.id) {
        if (action.type === "update" && tableInfo.isFileTable) {
          ...
          if (db[mediaTableName]?.update) {
            const res = await db[mediaTableName]?.update!({ id: media.id }, { deleted: true }, onSuccess ? { returning: "*" } : {});
