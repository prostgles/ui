...
export const SmartFormFooterButtonsV2 = (p: P): JSX.Element => {
  const { props, state } = p;
  ...
  getErrors(async (row) => {
    ...
    try {
      ...
      if (tableInfo?.isFileTable) {
        dataForInsert = state.getThisRow()[tableName] ?? props.defaultData;
      } else {
  ...
  getErrors(async (row) => {
    ...
    setConfirmPopup({
      ...
      onAccept: async () => {
        ...
        let f;
        try {
          f = await state.getValidatedRowFilter();
...
const onClickDelete = async ({
  ...
  setConfirmPopup({
    ...
    onAccept: async () => {
      ...
      setAction({ ...action, loading: true });
      try {
        await tableHandler?.delete!(await state.getValidatedRowFilter());
        onSuccess?.("delete");
