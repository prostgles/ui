...
type P = Pick<SmartFormProps, "db" | "tables" | "methods" | "hideNullBtn" | "enableInsert" | "onSuccess" | "tableName" | "theme"> & {
   ...
   */
  setReferencedInsertData: (column: Pick<ValidatedColumnInfo, "is_pkey" | "name">, newVal: any) => void;
  setData: (column: Pick<ValidatedColumnInfo, "is_pkey" | "name" | "tsDataType">, newVal: any) => Promise<void>;
}
...
export const SmartFormFieldOptions = ({ 
  ...
  const ftable = ref?.ftable ?? (c.file && fileTableName? fileTableName : undefined );
  const fTableCols = ftable? tables.find(t => t.name === ftable)?.columns : undefined;
  const ftableHandler = ftable && c.references?.length? db[ftable] : undefined;
  const fcol = ref?.fcols[ref.cols.indexOf(c.name)];
  const canInsertFTableName = enableInsert && ftableHandler?.insert? ftable : undefined;
  ...
  if(canInsertFTableName){
    if(c.file && ["insert"].includes(action)){
      ...
      if("acceptedContent" in c.file && Array.isArray(c.file.acceptedContent) && c.file.acceptedContent.length){
        inputProps.accept = c.file.acceptedContent.map(c => `${c}/*`).join() + "," + c.file.acceptedContent.flatMap(type => getKeys(CONTENT_TYPE_TO_EXT).filter(k => k.startsWith(type)).flatMap(k => CONTENT_TYPE_TO_EXT[k]).flat()).map(type => `.${type}`).join(",")
      } else if("acceptedContentType" in c.file && Array.isArray(c.file.acceptedContentType) && c.file.acceptedContentType.length){
        inputProps.accept = c.file.acceptedContentType.flatMap(type => getKeys(CONTENT_TYPE_TO_EXT).filter(k => k === type).flatMap(k => CONTENT_TYPE_TO_EXT[k]).flat()).map(type => `.${type}`).join(",")
      } else if("acceptedFileTypes" in c.file && Array.isArray(c.file.acceptedFileTypes) && c.file.acceptedFileTypes.length){
        inputProps.accept = `${c.file.acceptedFileTypes.map(type => `.${type}`).join(",")}`
      }

      insertBtn = <div className="w-fit h-fit">
        <Btn iconPath={mdiFilePlusOutline}
          color="action"
          title="Attach file"
        ...
        <input { ...inputProps } 
          ...
          onChange={e => {
            ...
            setReferencedInsertData(c, file? { name: file.name, data: file } : undefined)
          }} />
      </div>
    } else {
      insertBtn = <Btn 
  ...
  if (action !== "view" && ftableHandler?.find && fTableCols) {
    ...
    searchBtn = !hasMultipleCols? null : <Btn iconPath={mdiSearchWeb}
      title="Find record"
  ...
  if(insertBtn || searchBtn){
    rightContent = <div className="flex-row">
      {searchBtn}
      {insertBtn}
    </div>
  }
  ...
  return <>
    {searchTablePopup}
    {rightContent}
    {showNestedInsertForm && <SmartForm
      ...
      isReferencedInsert={{
        tableName,
        columnName: c.name,
        pkeyColumns: tables.find(t => t.name === tableName)?.columns.filter(c => c.is_pkey).map(c => c.name) ?? [],
        row,
      }}
      onInserted={(newRowOrRows) => {
        const newRow = Array.isArray(newRowOrRows)? newRowOrRows[0] : newRowOrRows;
        if(newRow){
