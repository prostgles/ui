...
export const ChipArrayEditor = ({ inputType, elemTsType, elemUdtName, values, onChange }: P) => { 
  
  const removeValue = (idx: number) => {
    const newVals = [...values] as typeof values; newVals.splice(idx, 1);  onChange(newVals) 
  } 

  const renderedValues = values.map(value => ({ value, isLastNew: false })).concat([{ value: "", isLastNew: true }]);
  const [orderAge, setOrderAge] = useState(0)
   
  return <div className={"ChipArrayEditor flex-row-wrap gap-p5 ptd-p25 ai-center"}>
    ...
    >
      <InputChip
        ...
        {...(isLastNew? {
          ...
          onChange: (newValue) => onChange(values.map((_v, _i) => _i === idx? newValue : _v) as typeof values),
...
type InputChipProps = Pick<P, "elemTsType" | "elemUdtName" | "inputType"> & { value: any, onChange: (newVal) => void, onRemove?: VoidFunction; placeholder?: string; };
const InputChip = ({ onChange, value, onRemove, placeholder, inputType, elemTsType, elemUdtName }: InputChipProps) => {
  const numberOfChars = Math.max(2, ((((value as any)?.toString()) || placeholder) ?? "").length) + 2;
