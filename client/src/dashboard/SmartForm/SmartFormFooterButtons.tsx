...
export const SmartFormFooterButtons = (p: P): JSX.Element => {
  ...
  const { db, tableName, disabledActions, onClose, onBeforeInsert, confirmUpdates, onChange } = props;
  ...
  if (confirmPopup) {
    ...
    const showBtn = {
      update: !(!confirmUpdates || !tableHandler?.update || !hasDataToUpdate) && !disabledActions?.includes("update"),
      delete: !startedUpdatingData && !!tableHandler?.delete && !disabledActions?.includes("delete"),
      insert: !startedUpdatingData && !!tableHandler?.insert,
      clone: !startedUpdatingData && !!tableHandler?.insert && !disabledActions?.includes("clone"),
    };
    ...
    const footerExtraButtons =
      (action === "view") ? null :
        (action === "update" && (showBtn.update || showBtn.delete || showBtn.clone)) ? (
          <>
            {showBtn.update && 
              <Btn
    ...
    const showExtraBtns = !!(footerExtraButtons &&  !(state.action.success && !error));
...
type ButtonProps = P & {
  useConfirmPopup:  [ConfirmationPopup | undefined, React.Dispatch<React.SetStateAction<ConfirmationPopup | undefined>>];
  getIsMounted: () => boolean;
}

const onClickInsert = async ({ state, props, getThisRow, columns, getErrors, parseError, setAction, setError, getIsMounted }: ButtonProps) => {
  const { db, tableName, includeMedia = true, onInserted, onSuccess, onBeforeInsert  } = props;
  ...
  getErrors(async row => {
    ...
    try {
      ...
      if(tableInfo?.isFileTable){
        ...
        dataForInsert = pickKeys(getThisRow(), columns.filter(c => c.insert).map(c => c.name).concat(extraKeys))
        dataForInsert = {
          ...dataForInsert,
      ...
      if(onBeforeInsert){
        ...
        const doInsert = async () => {
          if(tableInfo?.isFileTable && props.isReferencedInsert?.pkeyColumns.length){
            const { tableName } = props.isReferencedInsert;
            const refTableHandler = db[tableName]
            if(refTableHandler?.update){
              const filter = pickKeys(props.isReferencedInsert.row, props.isReferencedInsert.pkeyColumns);
              if(Object.values(filter).some(v => [null, undefined].includes(v)) || isEmpty(filter)){
              ...
              return refTableHandler.update(filter, { [props.isReferencedInsert.columnName]: dataForInsert[0] })
            }
          }

          return tableHandler?.insert!(dataForInsert , (onInserted || onSuccess) ? { returning: "*" } : {});
        }
...
const onClickUpdate = async ({ 
  state, props, getErrors, 
  parseError, getValidatedRowFilter,
  useConfirmPopup, setAction, setError,  
}: ButtonProps) => {
...
const onClickClone = async ({ parseError, props, getRowFilter, setAction, setError }: ButtonProps) => {
  ...
  try {
    ...
    if(rows?.length === 1){
      ...
      setError("Could not clone row because the row filter refers to more than 1 row");
    }
...
const onClickDelete = async ({ props, parseError, getValidatedRowFilter, useConfirmPopup, state, setAction, setError, getIsMounted }: ButtonProps) => {
  const { db, tableName, onSuccess } = props;
  const tableHandler = db[tableName];
  const { action } = state;
