...
export const deleteCascade = async ({ db, tableName, filter, tables }: P, prevTables?: string[]) => {
  const refChain:  {
    table: string;
    count: any;
    delete: () => Promise<void>;
  ...
  for await(const t of (tables as TableSchema[]).filter(t => !prevTables?.includes(t.name))){
