...
type SmartTableProps = Pick<Prgl, "db" | "tables" | "methods" | "theme"> & {
  ...
  title?: React.ReactNode | ((dataCounts: { totalRows: number; filteredRows: number }) => React.ReactNode);
  titlePrefix?: string;
  showInsert?: boolean;
  allowEdit?: boolean;
  className?: string;
...
export default class SmartTable extends RTComp<SmartTableProps, S> {
  ...
  get columns(): ProstglesColumn[] {
    ...
    if(!tableCols){
      ...
      _tableCols = cols.filter(c => c.select).map(c => ({ 
          key: c.name, 
          sortable: true, 
          ...
          onRender: onRenderColumn({ c, table, tables, barchartVals: undefined }),
        }));
  ...
  onDelta(deltaP: Partial<SmartTableProps> | undefined): void {
    ...
    (async () => {
      ...
      if(
        ...
        (
          JSON.stringify(realtime) !== JSON.stringify(this.realtimeOpt?.realtime) || 
        ...
        this.realtime = realtime && {
          sub: await tableHandler.subscribe(filter, { select: "", limit: 0, throttle: this.props.realtime?.throttle ?? 100 }, () => {
            this.getData(); 
          }),
          filter,
        } 
  ...
  getData = async (
    ...
    try{
      ...
      const _filter = getSmartGroupFilter(filter, undefined, this.props.filterOperand);
      const totalRows = await tableHandler.count!();
      const filteredRows = await tableHandler.count!(_filter);
      const rows = await tableHandler.find!(
        _filter, 
        {
          limit: pageSize, 
          orderBy: sort as any,
      ...
      this.setState({ rows, filter, sort, page, pageSize, totalRows, filteredRows, loadedData: true, error: undefined })
  ...
  render(){
    ...
    const content = <FlexCol className={"gap-0 f-1 min-h-0 relative " + (onClosePopup? "" : className)}>
      {!onClosePopup && titleNode}
      {editRowFilter && <SmartForm
        asPopup={true}
      ...
      <Table rows={rows} 
        ...
        onColumnReorder={newCols => {
          ...
          this.setState({
            columns: tableCols.slice(0).sort((a, b) => nIdxes.indexOf(a.name) - nIdxes.indexOf(b.name))
          })
        }}
        pagination={{
          page,
