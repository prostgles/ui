import {
  mdiTableColumn,
  mdiTableRow,
  mdiUnfoldLessHorizontal, mdiUnfoldMoreHorizontal
} from "@mdi/js";
...
export const SmartFilterBar = (props: SmartFilterBarProps) => {
  ...
  const { db, tables, leftContent, filterFields, style, className, innerClassname } = props;

  const { filter: _fltr = [], having: _having = [] } = (("w" in props)? props.w : props);
  const { table_name } = ("w" in props)? props.w : props;
  const onHavingChange = ("w" in props)? (having) => {
    props.w.$update({ having }, { deepMerge: true });
  } : props.onHavingChange;
  const onFilterChange = (filter: SmartGroupFilter, addedFilter?: SimpleFilter, isAggregate?: boolean) => {
  ...
  const w: WindowSyncItem<"table"> | WindowSyncItem<"card"> | undefined = "w" in props? props.w : undefined;
  ...
  const havingFiltersNode = !!having.length && <FlexCol key="having" className={`gap-p5 min-h-0 f-0 relative ai-start my-p5 ${hasFilters? "bb b-color" : ""} pb-p5`}>
    <Label variant="normal" info={<>HAVING clause is used to filter aggregate expressions</>}>Having</Label>
    <SmartFilter
  ...
  const renderedFilters = hasFilters && <FlexCol key="where" className={"gap-p5 min-h-0 f-0 relative ai-start my-p5"}>
    {havingFiltersNode && 
      <Label variant="normal" info={<>WHERE clause is used to filter data before aggregate column filters are applied in the HAVING clause</>}>
        Where
      </Label>
    }
    <SmartFilter
  ...
  return (
    ...
    >
      ...
      <FlexRowWrap className={classOverride("min-h-0 f-0 relative ai-center gap-p5", innerClassname)} >
        <SmartAddFilter
          selectedColumns={selectedColumns ?? []}
          tableName={table_name}
        ...
        {Boolean(filter.length) && <>
            ...
            >
              {window.isMobileDevice? null : `${!someFiltersExpanded? "Expand" : "Collapse"} filters`}
            </Btn>
            <Btn 
              title="Filter layout"
              iconPath={colFilterLayout? mdiTableColumn : mdiTableRow} 
              onClick={() => { setColFilterLayout(!colFilterLayout) }}
            />
          </>
        }
