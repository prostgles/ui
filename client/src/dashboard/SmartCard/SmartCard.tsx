...
export type FieldConfigRender<T extends AnyObject = AnyObject> = (value: any, row: T) => React.ReactNode;

export type ParsedFieldConfig<T extends AnyObject = AnyObject> = FieldConfigBase<T> & {
...
export type FieldConfig<T extends AnyObject = AnyObject> = string | ParsedFieldConfig<T>;

export type SmartCardCommonProps = {

}

export type SmartCardProps<T extends AnyObject = any> = Pick<Prgl, "db" | "tables" | "methods" | "theme"> & Pick<SmartCardListProps<T>, "tableName"> & { 
...
type S = {
  editMode: boolean
  columns?: Pick<ValidatedColumnInfo, "name" | "label" | "udt_name" | "is_nullable" | "has_default" | "tsDataType" | "is_pkey" | "references" | "hint">[];
...
const getDefaultFieldConfig = (
  cols: Pick<ValidatedColumnInfo, "name" | "label" | "udt_name" | "tsDataType" | "references">[] = [],
  colNames?: string[]
  ...
  if (colNames) {
    ...
    if (allTablesIndex > -1) {
      _fieldConfigs.splice(allTablesIndex, 0, ...cols.filter(c => c.references?.length).flatMap(c => c.references!.map(r => ({
        name: r.ftable!,
        fieldConfigs: ["*"]
      }))));
    }
    _fieldConfigs = _fieldConfigs.filter(c => !["*", "**"].includes(c as string))
  }

  const getColConfig = (c: typeof cols[number]) => ({
    name: c.name,
    label: c.label,
    renderValue: ["postcode", "post_code"].includes(c.name) ? (addr: string) => (
      !addr ? SmartFormField.renderValue(c, addr) : <a className="flex-col" target="_blank" href={`https://www.google.com/maps/search/${addr}`} >
        <span>{SmartFormField.renderValue(c, addr)}</span>
      </a>
    ) : c.tsDataType === "boolean" ? (val: boolean | null) => (<Checkbox title={(val || "NULL").toString()} checked={!!val} className="no-pointer-events" readOnly={true} />) :
      undefined
  });

  const result = _fieldConfigs ? _fieldConfigs.map((fc: any) => {
    ...
    if (typeof fc !== "string") {
      ...
      if (!c) {
        console.error(`Could not find column ${fc}. Incorrect name or not allowed`);
  ...
  }).filter(c => c) : cols.map(c => getColConfig(c));

  return result;
}

export default class SmartCard<T extends AnyObject> extends RTComp<SmartCardProps<T>, S> {
  state: S = {
    editMode: false,
    columns: undefined,
    variant: undefined,
  ...
  static parseFieldConfigs = (configs?: FieldConfigNested[], cols?: ValidatedColumnInfo[]): undefined | ParsedFieldConfig[] | ParsedNestedFieldConfig[] => {
    ...
    if (configs) {
      ...
      const duplicate = result.find((f, i) => result.find((_f, _i) => f.name === _f.name && i !== _i));
      if (duplicate) {
  ...
  onDelta = async () => {
    ...
    if(typeof tableName === "string"){
      ...
      if (rowFilter && JSON.stringify(rowFilter !== this._rowFilter) && tableHandler?.find) {
  ...
  render() {
    ...
    const pkeyCols = columns.filter(c => (hasPkeys ? c.is_pkey : c.references?.length) && defaultData[c.name]);
    ...
    const tableHandler = typeof tableName === "string"? db[tableName] : undefined;
    const allowedActions = {
      view: showViewEditBtn && (Boolean(tableHandler?.find && rowFilter)),
      delete: showViewEditBtn && Boolean(tableHandler?.delete && rowFilter),
      update: showViewEditBtn && (Boolean(onChange) || Boolean(tableHandler?.update && rowFilter)),
      insert: Boolean(tableHandler?.insert)
    };

    const variantClass = variant === "row-wrap" ? "flex-row-wrap ai-start" :
    ...
    const displayedColumns = hideColumns? columns.filter(c => hideColumns.includes(c.name)) : columns;

    const fieldConfigs = SmartCard.parseFieldConfigs(_fieldConfigs) || getDefaultFieldConfig(displayedColumns);
      ...
      .map(({ name, fc, col: c }, i) => {
        ...
        return (
          <div 
            ...
            className={"SmartCardCol flex-col o-auto ai-start text-1 ta-left " + (fc?.className || "")} 
