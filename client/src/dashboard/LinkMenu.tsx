...
export default class LinkMenu extends RTComp<P, S> {
  ...
  static getMyLinks = (_links: Link[], w: WindowData, windows: WindowData[]) => {
    const getLinks = (links: Link[], allLinks: Link[]) => {
      return allLinks.filter(al => 
        [al.w1_id, al.w2_id].some(alid => 
          links.some(l => 
            [l.w1_id, l.w2_id].includes(alid) 
          )
        )
      )
    }
    const links = _links.filter(l => [l.w1_id, l.w2_id].every(wid => windows.some(w => wid === w.id && !w.closed && !w.deleted)))

    let currLinks = links.filter(l => [l.w1_id, l.w2_id].includes(w.id) );
    let prevLinks = currLinks;
    let myLinks = [...currLinks];
    do {
      currLinks = getLinks(prevLinks, links.filter(l => !myLinks.find(ml => ml.id === l.id)))
      prevLinks = currLinks;
  ...
  loadShapes = () => {
    ...
    if(w.table_name && this.rootRef){
      ...
      gridRef.querySelectorAll<HTMLDivElement>("[data-box-id][data-box-type='item']").forEach(n => {
      ...
      const { colorStr } = getLinkColorV2(myLinks.find(l => l.options.type !== "table"));
      ...
      myLinks.forEach(l => {
        ...
        if(box1 && box2){
          const getText = (w: WindowData | undefined, otherW: WindowData | undefined) => {
            if(!w) return ""
          ...
          const w1r = window.document.body.querySelector(`[data-box-id='${w1.id}']`)?.getBoundingClientRect();
          const w2r = window.document.body.querySelector(`[data-box-id='${w2.id}']`)?.getBoundingClientRect();

          if(!w1r || !w2r) return;

          const w1Above = Boolean(w1r.y < w2r.y);
          const w1Left = Boolean(w1r.x > w2r.x);
          const dominant = Math.abs(w1r.y - w2r.y) > Math.abs(w1r.x - w2r.x) ? "y" : "x";
          
          const parsedPath = l.options.type === "table"? l.options.tablePath : l.options.joinPath
          joinTextLines = parsedPath?.flatMap(p => [
            { text: `(${Object.entries(p.on[0]!).map(([l,r]) => `${l} = ${r}`).join(" AND ")})`, color: "blue" },
  ...
  render(){
    ...
    const { onClose, anchorEl, onLinkTable, tables, w, links, windows } = this.props;

    const currentJoinedTables = windows.filter(({ id, type, table_name }) => table_name !== w.table_name && type === "table" && links.some(l => [l.w1_id, l.w2_id].includes(id) )).map(w => w.table_name!);
    const canJoin = tables.some(t => t.name === w.table_name && t.joins.length);

    return (<div className="LinkMenu absolute inset-0"
      ref={e => {
        if(e) this.rootRef = e;
      }}
    >
      ...
      >
        ...
        {canJoin && <JoinPathSelectorV2
          // className="w-full"
          tableName={w.table_name!}
          value={undefined}
          tables={tables}
          getFullOption={path => path.length === 1 && currentJoinedTables.includes(path.at(-1)!.table)? ({ disabledInfo: "Already joined" }) : undefined}
          variant="expanded"
          onChange={targetPath => {
