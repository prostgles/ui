...
export const JoinPathItem = ({ tableName, on: onValue, tables, prevTableName, onChange }: JoinPathItemProps) => {
  const on = tables.find(t => t.name === prevTableName)?.joinsV2.find(j => j.tableName === tableName)?.on || [];
  ...
  return <>
    <FlexCol 
      className={"JoinPathItem relative gap-p25 f-0 "}
    >
      {canChooseConditions && 
        <Btn 
    ...
    {edit && 
      ...
      >
        {on.map((condition, i) => {
          const conditionStr = getJoinPathConditionStr({ tableName, on: [condition] });
          const checked = onValue.some((condValue) =>  getJoinPathConditionStr({ tableName, on: [condValue] }) === conditionStr);
          return <React.Fragment key={conditionStr} >
            {i > 0 && <div>OR</div>}
            <SwitchToggle              
              className="p-1" 
              label={conditionStr} 
              checked={checked}
              onChange={(newChecked) => {
                const newOn = newChecked? [...onValue, condition] : onValue.filter(condValue => getJoinPathConditionStr({ tableName, on: [condValue] }) !== conditionStr);
