...
export class CardView extends RTComp<CardViewProps, CardViewState> {
  ...
  render(){
    ...
    const { props, state, w, paginationProps, className = "", style, onEditClickRow, onDataChanged, cols } = this.props;
    ...
    const groupByColumn = columns.find(c => c.name === cardOpts.cardGroupBy && c.update)
    const moveItemsProps = groupByColumn? {
      groupByColumn,
      orderByColumn: columns.find(c => c.name === cardOpts.cardOrderBy),
    } : undefined;

    const { cardRows = 1, hideCardFieldNames, maxCardWidth = "100%", hideEmptyCardCells, maxCardRowHeight, cardCellMinWidth = "", cardGroupBy } = cardOpts;
    const marginRight = cardRows > 1? `.5em` : "auto";
    const marginTop = ".5em";
    const marginLeftRight = cardRows === 1 && maxCardWidth !== "100%"? "auto" : "";
    ...
    const getCardColumn = (indexedRows: IndexedRow[]) => {
      ...
      return <div className={"CardView_Column" + (cardRows > 1? " flex-row-wrap " : " flex-col ") + " p-p25 o-auto no-scroll-bar f-1 min-w-0 min-h-0 px-p5 pt-p5"}
          style={{
          ...
          return (
            ...
            >
              ...
              >
                ...
                }).onRender!({ value: "", renderedVal: "", row, prevRow: rows[ri-1], nextRow: rows[ri+1], rowIndex: ri })}
              </div>}
              ...
              {cols.filter(c => 
                ...
                >
                  ...
                  <div className=" o-auto font-18"
                    title={typeof row[c.name] === "string" && (_PG_date.some(v => v === c.udt_name) || c.tsDataType === "number" ) ? row[c.name] : ""} 
                    style={{
                      lineHeight: 1.33, 
                  ...
                  >
                    {c.onRender?.({ row: row, value: row[c.name], renderedVal: row[c.name], rowIndex: ri, nextRow: rows[ri+1], prevRow: rows[ri-1] }) ?? SmartFormField.renderValue(c, row[c.name]) }
                  </div>
                </div>
              ))}
            </FlexRowWrap>
    ...
    if(cardGroupBy){
      ...
      const columnGroups = Array.from(new Set( rows.map(({ data }) => data[groupByColumn]) ));
      content = <div className="flex-row f-1 min-s-0 mt-1 o-auto">
        {columnGroups.map(groupByValue => {
          let groupRows = rows.filter(({ data }) => data[groupByColumn] === groupByValue);
