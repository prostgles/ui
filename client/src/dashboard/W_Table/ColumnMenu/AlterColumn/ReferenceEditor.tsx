...
export const onDeleteOptions = REFERENCES_COL_OPTS.filter(d => d.kwd.startsWith("ON DELETE")).map(d => ({ key: d.kwd.split(" ").slice(2).join(" "), subLabel: d.docs }));
export const onUpdateOptions = REFERENCES_COL_OPTS.filter(d => d.kwd.startsWith("ON UPDATE")).map(d => ({ key: d.kwd.split(" ").slice(2).join(" "), subLabel: d.docs }));
...
export const References = ({ onChange, tables, onAdd, tableName, ...opts }: P) => {

  const references = opts.references?.map(r => {
    return {
      ...r,
      notNullErr: opts.notNull? (r.onDelete?.includes("SET NULL")? "d" as const : r.onUpdate?.includes("SET NULL")? "u" as const : undefined) : undefined
    }
  })
...
export const AddColumnReference = ({ tables, variant, onAdd, tableName, columnName, dataType, existingReferences }: AddColumnReferenceProps) => {

  const referenceableColumns = useMemo(() => tables.flatMap(table => {
    return table.columns.filter(c => c.is_pkey && !(table.name === tableName && columnName === c.name)).map(column => ({
      key: `${table.name}.${column.name}`,
      label: `${table.name} (${column.name})`,
      subLabel: column.udt_name,
      disabledInfo: existingReferences.some(r => r.ftable === table.name && r.fCol === column.name)? "Already referenced by this column" : 
        dataType && column.udt_name.toUpperCase() !== dataType.toUpperCase()? "Column is not of same data type" :
        undefined,
      table,
      column,
    }))
...
export const getReferencesQuery = ({ ftable, fCol, onDelete = "", onUpdate = "" }: ColumnReference) => {
  return [
...
export const getAlterFkeyQuery = (arg: ColumnReference & { col: string; tableName: string; }) => {
