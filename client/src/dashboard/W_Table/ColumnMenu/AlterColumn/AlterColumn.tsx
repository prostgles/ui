...
export class AlterColumn extends RTComp<AlterColumnProps, S> {
  ...
  async onDeltaCombined(delta: DeltaOf<AlterColumnProps> & DeltaOf<S>): Promise<void> {
  ...
  onNewDataType = async (args: Pick<Required<S>["edited"], "dataType" | "notNull" | "defaultValue">) => {
    const { 
      table,
      db
    } = this.props;
    ...
    const { dataType: newType, defaultValue, notNull } = { ...this.state, ...args };
    ...
    if(typeof newType === "string" && newType !== col?.data_type.toUpperCase()){
    
      if(newType.startsWith("TIMESTAMP") || newType === "DATE"){
        using = `USING to_timestamp(${usingCol}, 'YYYY-MM-DD HH:MI:SS')::${newType}`
  
        const value = await db.sql!(`SELECT ${field} FROM ${tableName} WHERE  ${field} IS NOT NULL LIMIT 1`, {}, { returnType: "value" })
    ...
    if(defaultValue !== undefined){
      query += `\n\n` + alterColQuery + `SET DEFAULT ${col?.tsDataType === "string"? `'${defaultValue}'` : defaultValue};`
    }
  ...
  render(){
    ...
    if(!col) return <ErrorComponent error={`Column ${this.props.field} not found`} />
    const pkeyCons = constraints?.find(c => c.constraint_type === "PRIMARY KEY");
    const fkeyCons = constraints?.filter(c => c.constraint_type === "FOREIGN KEY") ?? [];

    return (
      <FlexCol 
        className="f-1 flex-col p-1 o-auto"
      >
              ...
              if(k === "name"){
                ...
                if(typeof notNull === "boolean" && !notNull !== col.is_nullable){
                  this.setState({ 
                    notNull,
                    edited: { notNull }, 
                    query: alterColQuery + (notNull? "SET NOT NULL;" : "DROP NOT NULL;") 
                  });
                } else {
                  resetEdit()
                }
                ...
                if(defaultValue !== col.column_default){
                  const q = defaultValue === undefined? "DROP DEFAULT;" : `SET DEFAULT ${col.tsDataType === "string"? `'${defaultValue}'` : defaultValue};`
                  this.setState({
                    defaultValue,
                    edited: { defaultValue },
        ...
        {query && (!isCreate || showCreateQuery) && <SQLSmartEditor 
          query={query}
          sql={db.sql!}
          ...
          onSuccess={() => {
            this.props.onClose();
            this.setState({ query: "", queryAge: Date.now(), edited: undefined })
          }}
        />}
      </FlexCol> 
    )
