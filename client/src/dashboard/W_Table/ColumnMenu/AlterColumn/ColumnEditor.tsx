import {
  _PG_bool,
  _PG_date, _PG_geometric, _PG_interval,
  _PG_json, _PG_numbers, _PG_postgis, _PG_strings
...
type P = Pick<CommonWindowProps, "suggestions"> & ColumnOptions & {
  ...
  onAddReference: (c: ColumnOptions, r: Required<ColumnOptions>["references"][number]) => void;
...
export const ColumnEditor = ({ onChange, onAddReference, tables, onEditReference, isAlter, suggestions, tableName, ...colOpts }: P) => {
  ...
  return <FlexCol className="ColumnEditor gap-2">
    <FlexRowWrap className="ai-end">
      ...
      <FlexRow className={!name? "hidden" : ""}>
        ...
        {!!dataType && <>
          ...
          <SwitchToggle
            variant="col"
            label={{ label: "Not null", info: "If true then NULL values will not be accepted" }}
            disabledInfo={isPkey? "Primary key constraint overrides this" : undefined}
            checked={!!notNull}
            onChange={notNull => {
    ...
    <FlexCol className={!dataType? "hidden" : ""}>
      <FormFieldDebounced
        id={"defval"}
        label={{ label: "Default value", info: "Will be added to all new records if no other value is specified" }}
        type="text"
        autoComplete="off"
