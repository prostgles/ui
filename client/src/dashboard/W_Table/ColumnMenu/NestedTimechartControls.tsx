...
export const NestedTimechartControls = ({ tableName, chart, tables, onChange }: P) => {
        ...
        <FlexCol>
          ...
          {chart && numericCols.length > 0 && <>
            <Select 
              ...
              fullOptions={TIMECHART_STAT_TYPES.map(({ func: key, label }) => ({ key, label }))}
              value={chart.yAxis.isCountAll === true?  "$countAll" : chart.yAxis.funcName}
              onChange={funcName => {
                onChange({
                  ...chart,
                  yAxis: funcName === "$countAll"? { isCountAll: true } : {
            ...
            />
            {chart.yAxis.isCountAll !== true && <Select 
              label={"Aggregated column"}
              fullOptions={numericCols.map(c => ({ key: c.name, subLabel: c.udt_name }))}
            ...
            <Select label="Chart style" 
              ...
              fullOptions={TimechartRenderStyles.concat([{ key: "smooth-line", label: "Smooth line" } as any])}
