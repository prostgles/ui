...
import { LinkedColumnProps } from "./LinkedColumn";
...
export const LinkedColumnFooter = ({ onClose, column, w, localColumn, disabledInfo }: P) => {

  const [addTo, setPosition] = useState<typeof NEW_COL_POSITIONS[number]["key"]>("start");
  return <>
    {column?.nested && 
    ...
    <FlexRow className="mt-2">
      ...
      {column?.nested && 
        <Btn 
          color="danger"
          onClick={() => {
            updateWCols(w, w.$get().columns?.filter(c => c.name !== column.name));
            onClose?.();
          }}
        >
          Remove
      ...
      {localColumn && <Btn color="action" 
        ...
        onClickMessage={async (e, setM) => {
          ...
          const newColumns = !column? (addTo === "start"?
            [
              localColumn,
              ...w.columns,
            ] :
          ...
          ) : w.columns.map(c => c.name === column.name? localColumn : c);
