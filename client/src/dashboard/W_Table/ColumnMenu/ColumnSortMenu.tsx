...
export const ColumnSortMenu = ({ column, w, tables }: ColumnSortMenuProps) => {
  ...
  const nestedCols = nested?.chart? [{ name: "date", show: true, }, { name: "value", show: true, }] : nested?.columns;
  return <FlexCol>
    {nestedCols?.length && <Select 
      label={"Nested column"}
      fullOptions={nestedCols.map(nc => ({ 
        key: `${column.name}.${nc.name}`,
        disabledInfo: nc.show? undefined : "Must unhide column before sorting"
      }))}
      value={existingSort?.key}
      onChange={(key, e) => {
        const newSort = existingSort? 
          w.sort!.map(s => s.key === existingSort.key? { ...existingSort, key } : s) : 
          [{ key, asc: true }]
        w.$update({ sort: newSort });
      }}
    />}
    <Select label="Sort by" 
      value={sort.asc === true? "Ascending" : sort.asc === false? "Descending" : null} 
      variant="div"
      emptyLabel="None"
    ...
    {(typeof sort.asc !== "boolean")? 
      ...
      <>
        ...
        />
        {colIsText && <FormField type="checkbox" 
