...
const funcAcceptsColumn = (f: FuncDef, targetCols: ValidatedColumnInfo[]) => {
  ...
  return (!f.tsDataTypeCol && !f.udtDataTypeCol) || 
    f.tsDataTypeCol  === "any" || 
    f.udtDataTypeCol === "any" || 
    Array.isArray(f.udtDataTypeCol) && f.udtDataTypeCol.some(udt => targetCols.some(c => c.udt_name === udt)) ||
...
export const FunctionSelector = ({ column, tableColumns, onSelect, selectedFunction, wColumns: parentColumns, className }: P) => {
  ...
  const funcs = useMemo(() => { 
    ...
    const funcs = [
      ...
      ...f,
      isAllowedForColumn: funcAcceptsColumn(f, columnInfo? [columnInfo] : tableColumns)
    }))
    ...
    fullOptions={funcs
      .map(def => {
        ...
        return {
          ...
          disabledInfo: cannotUseAggs && def.isAggregate? "Cannot use aggregations with nested column" : 
            def.isAllowedForColumn? undefined : "Not suitable for the selected column data type",
        }
      })}
  />
...
function getFuncs( ): FuncDef[] {
  ...
  const DateFuncs = [
    { key: "$datetime", label: "Date and time", subLabel: "Timestamp formated as 'YYYY-MM-DD HH24:MI'" },
    { key: "$timedate", label: "Time and date", subLabel: "Timestamp formated as 'HH24:MI YYYY-MM-DD'" },
    // { key: "$DayNo",    label: "", subLabel: "Timestamp formated as 'DD" },
    // { key: "$dowUS",    label: "", subLabel: "Timestamp formated as 'D" },
    { key: "$D",        label: "Day of week number",      subLabel: "Timestamp formated as 'D'. Day of the week, Sunday (1) to Saturday (7)" },
    { key: "$dy",       label: "Day name short",          subLabel: "Timestamp formated as 'dy'. Abbreviated lower case day name (3 chars in English, localized lengths vary)" },
    { key: "$Dy",       label: "Day Name short",          subLabel: "Timestamp formated as 'Dy'. Abbreviated capitalized day name (3 chars in English, localized lengths vary)" },
    { key: "$DD",       label: "Day of month",            subLabel: "Timestamp formated as 'DD'. Day of month (01-31)" },
    { key: "$ID",       label: "ISO 8601 day of the week", subLabel: "Timestamp formated as 'ID'. Monday (1) to Sunday (7)" },
    { key: "$MM",       label: "Month number",            subLabel: "Timestamp formated as 'MM'. Month number (01-12)" },
    // { key: "$MonthNo",  label: "Month number", subLabel: "Timestamp formated as 'MM. Month number (01-12)" },
    { key: "$yy",       label: "Year short",              subLabel: "Timestamp formated as 'YY'. Last 2 digits of year" },
    // { key: "$yr",       label: "", subLabel: "Timestamp formated as 'yy" },
    { key: "$day",      label: "Day name", subLabel: "Timestamp formated as 'day'. Full lower case day name" },
    { key: "$Day",      label: "Day Name", subLabel: "Timestamp formated as 'Day'. Full capitalized day name" },
    // { key: "$dow",      label: "", subLabel: "Timestamp formated as 'ID" },
    { key: "$mon",      label: "Month name short",  subLabel: "Timestamp formated as 'mon'. Abbreviated lower case month name (3 chars in English, localized lengths vary)" },
    { key: "$Mon",      label: "Month Name short",  subLabel: "Timestamp formated as 'Mon'. Abbreviated capitalized month name (3 chars in English, localized lengths vary)" },
    { key: "$month",    label: "Month name",        subLabel: "Timestamp formated as 'month'. Full lower case month name " },
    { key: "$Month",    label: "Month Name",        subLabel: "Timestamp formated as 'Month'. Full capitalized month name " },
    { key: "$date",     label: "Date",              subLabel: "Timestamp formated as 'YYYY-MM-DD'" },
    { key: "$time",     label: "Time",              subLabel: "Timestamp formated as 'HH24:MI'. 24 Hour time format" },
    // { key: "$year",     label: "", subLabel: "Timestamp formated as 'yyyy" },
    { key: "$yyyy",     label: "Year full",         subLabel: "Timestamp formated as 'yyyy'. Year (4 or more digits)" },
    ...
    { key: "$age",      label: "Age at start of day",  subLabel: "Age of timestamp compared to start of today" },
  ...
  const StringFuncs = [
    ...
    { key: "$trim",         label: "TRIM", subLabel: "Remove spaces or set of characters from the leading or trailing or both side from a string" },
    { key: "$upper",        label: "Uppercase", subLabel: "Convert all characters to uppercase" },
    { key: "$lower",        label: "Lowercase", subLabel: "Convert all characters to lowercase" },
    { key: "$length",       label: "Length", subLabel: "Number of characters in the string" },
    { key: "$reverse",      label: "Reverse", subLabel: "Arrange a string in reverse order" },
    { key: "$initcap",      label: "Capitalise Initials", subLabel: "Convert the first letter of each word to upper case and the remaining to lower case" },
    { key: "$unnest_words", label: "Split into words", subLabel: "Split text from a column at spaces" },
  ];

  const NumberFuncs = [
    { key: "$ceil",  label: "Ceil", subLabel: "Smallest integer which is greater than, or equal to, the specified numeric expression" },
    { key: "$floor", label: "Floor",  subLabel: "Smallest integer which is lower than, or equal to, the specified numeric expression" },
    { key: "$sign",  label: "Sign", subLabel: "Returns -1 0 or 1 depending of the column value. Returns null if null is provided" },
  ...
  const AnyFuncs = [
    { key: "$template_string", label: "Template string", subLabel: "Create a string based on column values. E.g.: Dear {FirstName} {LastName}, ..." },
  ];
  ...
  const res: FuncDef[] = [
    ...
    ...DateFuncs.map(f => ({
      ...f,
      outType: (f.key.startsWith("$age") || f.key === "$duration")? infoTypes.interval : infoTypes.string, 
...
const aggFunctions = [
  ...
  { 
    ...
    label: "Non-null string/text values concatenated into a string, separated by delimiter", 
