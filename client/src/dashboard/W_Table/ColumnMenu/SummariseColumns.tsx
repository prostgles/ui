...
export const SummariseColumn = ({ column, columns, onChange, tableColumns }: SummariseColumnProps) => {
  const topFuncs = column.info?.udt_name.startsWith("timestamp") || column.info?.udt_name.startsWith("date")? [""] : [];
  const otherColumnsShown = columns.some(c => c.name !== column.name && c.show);
  ...
  return <>
    ...
    >
      {funcDef?.label ?? column.computedConfig?.funcDef.label ?? `${topFuncs.join(", ")}...`}
    </Btn>
    {showPopup && <Popup
      anchorEl={showPopup}
      ...
      footerButtons={!funcDef? undefined : [
        ...
        { 
          ...
          onClick: () => {
            ...
            onChange(columns.map(c => c.name === column.name? newCol : hideOthers? { ...c, show: false } : c));
    ...
    >
      <FlexCol className={"min-h-0"}>
        <FunctionSelector 
          selectedFunction={funcDef?.key ?? column.computedConfig?.funcDef.key}
        ...
        {funcDef && 
          <SwitchToggle 
            className="m-1"
            disabledInfo={otherColumnsShown? undefined : "No other columns are shown"}
