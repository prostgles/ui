...
export const ConditionalCellStyleControls = ({ column, style, tableName, tables, db, onUpdate }: P) => {
  ...
  const updateCondStyle = (newStyle: Partial<ConditionalStyle["conditions"][number]> | null, idx: number | undefined) => {
  ...
  return <FlexCol className="ConditionalCellStyleControls">
    {conditions.map((cs, condIdx) => (
      ...
      >
        ...
        <div className="flex-row-wrap gap-0 ai-center">
          <Btn color="action" variant="faded">{column.name}</Btn>
          <Select className="ml-p25"  
            value={cs.operator} 
            variant="div" 
        ...
        <PopupMenu 
          button={(
            <div className="flex-row ai-center gap-1 pointer noselect">
              <div>Chip style</div>
              <StyledCell 
                style={{ chipColor: cs.chipColor, textColor: cs.textColor ?? style.defaultStyle?.textColor, borderColor: cs.borderColor }} 
          ...
          footerButtons={[
            { label: "Done", variant: "filled", color: "action", onClickClose: true }
          ]}
          render={() => (
            <FlexCol>
              <FlexRow className="jc-start ai-center  gap-1 o-auto f-1">
                ...
                <ColorPicker label="Text:" 
                  value={cs.textColor ?? style.defaultStyle?.textColor ?? "black"} 
                  onChange={textColor => { 
                    updateStyle({ defaultStyle: { ...style.defaultStyle, textColor } });
                    updateCondStyle({ textColor }, condIdx)
                  }}
                />
                <ColorPicker label="Chip" 
                  value={cs.chipColor ?? "transparent"} 
                  onChange={chipColor => { updateCondStyle({ chipColor }, condIdx) }} 
                />
                <ColorPicker label="Border:" 
                  className="mr-auto"
                  value={cs.borderColor ?? "transparent"} 
                  onChange={borderColor => { updateCondStyle({ borderColor }, condIdx) }} 
                />
              ...
              <ChipStylePalette 
                onChange={chipStyle => {
                  updateCondStyle({ ...chipStyle, chipColor: chipStyle.color }, condIdx)  
                }}
              />
            </FlexCol>
          )}
