...
import type { ConditionalStyleIcons, StyleColumnProps } from "../ColumnStyleControls";
...
export const ConditionalCellIconStyleControls = ({ column, style, tableName, tables, db, onUpdate }: P) => {

  const updateStyle = (partialStyle: Partial<typeof style>) => {
    onUpdate({ style: { ...style, ...partialStyle } })
  }
  const updateCondStyle = (newStyle: ConditionalStyleIcons["valueToIconMap"], overWrite = false) => {
 
    updateStyle({ 
      ...style,
      valueToIconMap: overWrite? newStyle : {
  ...
  return <FlexCol className="ConditionalCellStyleControls">
    ...
    <Btn 
      ...
      iconPath={mdiPlus} 
      onClick={async () => {
        const firstNonNullValueRow = await db[tableName]?.findOne?.({ [column.name]: { $ne: null } }, { select: { [column.name]: 1 } });
