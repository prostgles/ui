...
export const ColumnList = ({ columns: columnsWithoutInfo, tableColumns, mainMenuProps, onChange, showToggle = true }: P) => {
  ...
  return <SearchList
    ...
    onReorder={async nc => {
      ...
      await onChange(nc.map(n => ({ ...(n.data as ColumnConfig), show: n.checked })));
    }}
    limit={200}
    className="f-1"
    onMultiToggle={!showToggle? undefined : (items => {
      const nc = columns.slice(0).map(_c => ({ ..._c, show: items.find(d => d.key === _c.name)?.checked ?? _c.show }));
      onChange(nc)
    })}
    placeholder={`Search ${columns.length} columns`}
    items={columns
      .sort((a,b) => (order[a.name] ?? Infinity) - (order[b.name] ?? Infinity))
      .map(c => {
        ...
        return {
          ...
          contentRight: (!mainMenuProps || !db?.sql && !c.computedConfig)? null : <FlexRow className="mr-p5" onClick={e => e.stopPropagation()}>
            {db?.sql && !c.computedConfig && !c.nested && <PopupMenu
              positioning="center"
              title={`Alter ${c.name}`}
              button={<Btn iconPath={mdiPencil} title="Alter column" color="action" className="show-on-trigger-hover" />}
              onClickClose={false}
            >
              <AlterColumn 
                {...mainMenuProps}
            ...
            {c.nested && 
              <PopupMenu 
                ...
                button={<Btn color="action" iconPath={mdiLink} title="Edit Linked Field" />}
                render={pClose => <LinkedColumn {...mainMenuProps} column={c} onClose={pClose} />}
              />
            }
            ...
            {!!computedRemove &&
              <Btn className="mr-1" 
                ...
                children={computedRemove === "Remove Function"? c.computedConfig?.funcDef.label : undefined}
                iconPath={computedRemove === "Remove Function"? mdiFunction : mdiDelete}
                onClick={e => {
                  e.stopPropagation()
                  let newCols;
                  if(computedRemove === "Remove formatting" || computedRemove === "Remove Function"){
                    newCols = columns.map(_c => {
                      if(_c.name === c.name){
                        const res = omitKeys({
                          ..._c,
                          computed: false,
                        }, computedRemove === "Remove formatting"? ["format"] : ["computedConfig"])
          ...
          onPress: () => {
            ...
            //@ts-ignore
            const nc = columns.slice(0).map(_c => ({ ..._c}))
              .map(_c => {
              if (_c.name === c.name) {
                _c.show = !c.show;
              }
