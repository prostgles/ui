...
import Popup, { FooterButton } from '../../../components/Popup/Popup';
...
import { CountAllFunc, FuncDef, FunctionSelector } from "./FunctionSelector";
import { NEW_COL_POSITIONS } from "./LinkedColumn/LinkedColumnFooter";
import { getNestedColumnTable } from "./getNestedColumnTable"; 
...
export class AddComputedColMenu extends RTComp<AddComputedColMenuP, AddComputedColMenuS> {
  ...
  get table(){
    const { w, tables, nestedColumnName } = this.props;
    return getNestedColumnTable(nestedColumnName, w, tables);
  }
  ...
  render(){
    ...
    let allowedColumnsForFunction: ValidatedColumnInfo[] = columns;
    if(funcDef && (funcDef.tsDataTypeCol || funcDef.udtDataTypeCol)){
      allowedColumnsForFunction = columns//.filter(c => !column || c.name === column)
      .filter(c => {
    ...
    const content = <>    
      <FlexRowWrap className="gap-2 f-0 min-h-0 mt-1 ai-start">
        {!column && <FlexRowWrap>
          <FlexCol className="gap-p25">
            <Btn variant="faded"
              ...
              <div className="text-gray-700 p-p25 ta-left font-14" style={{ maxWidth: "250px" }}>Will show total row counts grouped by the selected columns</div>
            </FlexCol>
          {!funcDef && <div>OR</div>}
        </FlexRowWrap>}
      ...
      {funcDef?.key === "$duration" && (args?.$duration?.otherColumn? 
        <div className="flex-col mt-1">
          <label className="noselect f-0 text-1p5 ta-left  mb-p5 ">Compare to</label>
          <Btn variant="filled"
            style={{
