...
type P = {
  ...
  getFullOption?: (path: ParsedJoinPath[]) => Pick<FullOption<string>, "disabledInfo"> | undefined;
  variant?: "expanded"
};
...
export const getRankingFunc = (searchTerm: string, labels: string[]) => {
  if(searchTerm){
    const matchedLabelRank = labels.map((l, i)=> {
      const idx = l.toLowerCase().indexOf(searchTerm.toLowerCase());
      const rank = idx === -1? undefined : Number(`${i}.${idx.toString().padStart(3, "0")}`);
      return rank
    }).filter(isDefined)[0];
...
export const JoinPathSelectorV2 = ({ tables, tableName, value, onChange, variant, getFullOption, btnProps }: P) => {
  ...
  const fullOptions = allJoins.map((j)=> {
    ...
    return {  
      ...
      ranking: searchTerm => getRankingFunc(searchTerm, j.labels.map(l => l.label)),
...
const hasMultiFkeys = (tables: DBSchemaTablesWJoins, t1: string, t2: string) => {
  const table = tables.find(t => t.name === t1);
  if(table){
    const join = table.joinsV2.find(j => j.tableName === t2 && j.on.length > 1);
    return join?.on;
  }
