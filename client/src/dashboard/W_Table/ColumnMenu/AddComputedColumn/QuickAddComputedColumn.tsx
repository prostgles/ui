...
export const QuickAddComputedColumn = ({ tableName, tables, onAddColumn }: P) => {
  ...
  const [allowedColumns, setAllowedColumns] = useState<ValidatedColumnInfo[] | undefined>();

  const addColumn = useCallback((c: ValidatedColumnInfo | undefined, funcDef: FuncDef, pClose: VoidFunction) => {
    ...
    render={pClose => (
      <FlexCol className="">
        <p className="m-0 p-1 ta-left">
          A computed column is a column that is calculated based on other data from the table.
          <br></br>
          It will not be stored in the database but will be calculated on the fly when queried.
        </p>
