...
export const getNestedColumnTable = (nestedColumnOpts: NestedColumnOpts | undefined, w: WindowSyncItem<"table">, tables: DBSchemaTablesWJoins): MaybeResult => {
  ...
  const nestedColumn = !nestedColumnOpts? undefined : nestedColumnOpts.type === "new"? getMinimalColumnInfo([nestedColumnOpts.config])[0] : columns.find(c => c.name === nestedColumnName);
  if(!nestedColumnOpts){
