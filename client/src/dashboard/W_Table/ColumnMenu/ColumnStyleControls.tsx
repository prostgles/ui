  ...
  return (
    <FlexCol className="ColumnStyleControls flex-col gap-1">
...

      {
        style.type === "Fixed"? <>
          <FlexRowWrap>
            <ColorPicker label="Text" className="m-p5" value={style.textColor || "black"} onChange={textColor => { updateStylePart({ textColor }) }} />
            <ColorPicker label="Chip" className="m-p5" value={style.chipColor || "red"} onChange={chipColor => { updateStylePart({ chipColor }) }} />
            <ColorPicker label="Cell" className="m-p5" value={style.cellColor || "white"} onChange={cellColor => { updateStylePart({ cellColor }) }} />
            <ColorPicker label="Border" className="m-p5" value={style.borderColor || "transparent"} onChange={borderColor => { updateStylePart({ borderColor }) }} />
          </FlexRowWrap>
          <ChipStylePalette 
            onChange={({ borderColor, color, textColor }) => updateStylePart({ borderColor, textColor, chipColor: color })}
          />
        </> :
        ...
        style.type === "Scale"? <FlexRowWrap>
          ...
          <ColorPicker 
            ...
            onChange={minColor => { updateStylePart({ minColor }) }} 
          />
          <ColorPicker 
            className="mr-p5" 
            label="Max color" 
            value={style.maxColor} 
            onChange={maxColor => { updateStylePart({ maxColor }) }} 
          />
          <ColorPicker 
            className="mr-p5" 
            label="Text" 
            value={style.textColor} 
            onChange={textColor => { updateStylePart({ textColor }) }} 
          />
          
        </FlexRowWrap> : 
        
        style.type === "Barchart"? <FlexRowWrap>
          <ColorPicker 
            label="Bar" 
            className="m-p5" 
            value={style.barColor} 
            onChange={barColor => { updateStylePart({ barColor }) }} 
          />
          <ColorPicker 
            label="Text" 
            className="m-p5" 
