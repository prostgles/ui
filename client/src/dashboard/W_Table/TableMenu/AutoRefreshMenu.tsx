...
export const AutoRefreshMenu = ({ w, db }: {w: WindowSyncItem<"sql"> | WindowSyncItem<"map"> | WindowSyncItem<"table">, db?: DBHandlerClient}) => {
  ...
  const REFRESH_OPTIONS = [
    { key: "Realtime", disabledInfo: w.type === "table" && !db?.[w.table_name!]?.subscribe ? "Cannot subscribe" : undefined },
    { key: "Interval" },
    { key: "None" },
  ] as const satisfies readonly { key: NonNullable<RefreshOptions["refresh"]>["type"]; label?: string; disabledInfo?: string; }[];
  ...
  return <div className="flex-col ai-start">
    ...
    <div className="text-1 noselect mt-0">{
      type === "Realtime" ? <>
        Changes shown as received from database. Needs super user to create triggers
        {w.table_name && db && db[w.table_name] && !db[w.table_name]?.subscribe && <InfoRow color="danger" className="my-1">Cannot subscribe</InfoRow>}
      </> :
