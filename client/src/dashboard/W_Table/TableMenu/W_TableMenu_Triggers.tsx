...
export const W_TableMenu_Triggers = ({ tableMeta, onSetQuery, w, prgl }: P) => {
  ...
  return (
    <FlexCol className="  ws-pre">
      ...
      {!!tableMeta.triggers.length && <div className="flex-row ai-center ">
        <Btn
          className="f-0 mr-1 "
          color="action"
          variant="faded"
          disabledInfo={tableMeta.triggers.every(t => t.disabled) ? "Already disabled" : undefined}
          onClick={() => {
            onSetQuery({ 
        ...
        <Btn
          // iconPath={mdiPlus} 
          className="  f-0"
          disabledInfo={!tableMeta.triggers.every(t => t.disabled) ? "Already enabled" : undefined}
          variant="outline"
          onClick={() => {
            onSetQuery({ 
