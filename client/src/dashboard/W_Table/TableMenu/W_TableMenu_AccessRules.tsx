...
export const W_TableMenu_AccessRules = ({ tableMeta, w, prgl }: W_TableMenuMetaProps) => {
  ...
  return <FlexCol>
    <ul className="flex-col gap-1">
      {accessRules.map(r => {
        ...
        const tableRule = dbRule.type === "Custom"? dbRule.customTables.find(t => t.tableName === w.table_name) : undefined;
        const href = getAccessControlHref({ connectionId: prgl.connectionId, selectedRuleId: r.id.toString() })
        return (
          <Link 
            key={r.id} 
            to={href}
          ...
          > 
            <div className=" active-shadow-hover pointer rounded bg-color-0 shadow b b-color p-p5 ">
              ...
              <LabeledRow 
                ...
                className="ai-center"
              >
                <span className="text-0 font-20 bold">{r.userTypes.join(", ")}</span>
              </LabeledRow>
              <FlexCol className="p-1 text-0">
                {dbRule.type === "Run SQL"? "Run SQL" : 
                  dbRule.type === "All views/tables"? dbRule.allowAllTables.join(", ") : 
                  ACCESS_RULE_METHODS.map(ruleName => {
                    const rule = tableRule?.[ruleName];
                    if(!rule) return null;
                    const ruleFields = rule === true? "*" : isObject(rule) && ("fields" in rule && rule.fields)? rule.fields : undefined;
                    const fieldsInfo = ruleFields === "*" ? "All columns" : 
                      Array.isArray(ruleFields)? `Columns: ${ruleFields.join(", ")}` : 
                      isObject(ruleFields)? `${Object.values(ruleFields).some(v => !v)? "Except columns:" : "Columns:"} ${Object.keys(ruleFields)}` : undefined;
                    return <FlexCol key={ruleName} className="ta-left">
                      <FlexCol>
                        <div className="font-22 text-0">{ruleName}</div>
                        <FlexCol className="pl-1">
                        {fieldsInfo}
                        {isObject(rule) && "forcedFilterDetailed" in rule && 
                        ...
                        {isObject(rule) && "checkFilterDetailed" in rule && 
