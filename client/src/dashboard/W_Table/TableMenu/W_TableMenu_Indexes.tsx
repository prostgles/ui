...
export const W_TableMenu_Indexes = ({ tableMeta, onSetQuery, w, cols, prgl }: P) => {
    ...
    <SmartCardList 
      ...
      fieldConfigs={[
        ...
        { 
          ...
          render: indexname => <FlexRow className="">
            <Btn 
              title="Drop index..."
              color="danger"
              variant="faded"
    ...
    <FlexRow className="ai-center ml-p5 f-0">
      <Select
        ...
        fullOptions={[
          {
            key: "B-Tree",
            subLabel: "Default. B-trees can handle equality and range queries on data that can be sorted into some ordering. Operators: \n<   <=   =   >=   >"
          },
          {
            key: "Hash",
            subLabel: "Hash indexes store a 32-bit hash code derived from the value of the indexed column. Operators: \n="
          },
          {
            key: "GiST",
            subLabel: "Has many different indexing strategies. Operators: \n<<   &<   &>   >>   <<|   &<|   |&>   |>>   @>   <@   ~=   &&"
          },
          {
            key: "SP-GiST",
            subLabel: "Has a wide range of different non-balanced disk-based data structures, such as quadtrees, k-d trees, and radix trees (tries). Operators: \n<<   >>   ~=   <@   <<|   |>>"
          },{
            key: "GIN",
      ...
      />
      <Btn iconPath={mdiDatabaseRefreshOutline} 
        variant="faded" 
        onClick={() => { 
          onSetQuery({
            title: "Reindex table",
