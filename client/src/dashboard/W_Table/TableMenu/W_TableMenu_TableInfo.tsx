...
export const W_TableMenu_TableInfo = ({ w, tableMeta, onSetQuery, prgl }: W_TableMenuMetaProps) => {
  ...
  return (
    <FlexCol className="f-1 ai-start o-auto ">
      ...
      <div className="f-1  flex-row mr-1 ai-center">
        <Chip className=" " variant="header" label={"Comment"} value={tableMeta.comment} />
        <Btn color="action"
          iconPath={mdiPencil} 
          onClick={() => { 
      ...
      <FlexRowWrap className="w-full  gap-0"> 
        <Chip className="f-1" variant="header" label={"OID"} value={w.table_oid} />
        <Chip className="f-1" variant="header" label={"Type"} value={tableMeta.type} />
        <Chip className="f-1 " variant="header" label={"Owner"} value={tableMeta.relowner_name} />
