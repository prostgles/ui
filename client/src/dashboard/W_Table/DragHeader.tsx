...
type DragHeaderProps = Pick<CardViewProps, "onDataChanged" | "onEditClickRow"> & {
  padding: string;
...
export class DragHeader extends RTComp<DragHeaderProps> {
  ...
  render(): React.ReactNode {
    ...
    const getNodes = (node: HTMLDivElement) => {
      ...
      const siblings = Array.from<HTMLDivElement>(rootView?.querySelectorAll(`[data-row-index]`) ?? [])
        .map(n => {
    ...
    return <Pan
      ...
      onPan={({ xDiff, yDiff, x, y, node }, e) => {
        ...
        const targetIndex = targetSibling ? parseInt(targetSibling.n.dataset.rowIndex!) : undefined;
        const targetIRow = this.props.allIndexedRows.find(d => d.index === targetIndex);
      ...
      onPanEnd={({ node }, e) => {
        ...
        (async () => {
          ...
          if (isMoving.target && isMoving.index !== isMoving.target.index - 1) {
            const { error, filter } = await getRowFilter(isMoving.data, this.props.columns, this.props.tableHandler);
            if (error) {
              alert(error)
            } else if (filter) {
              const noSiblingData = { nextRow: undefined, nextRowFilter: undefined, prevRow: undefined, prevRowFilter: undefined };
              const { groupByColumn, orderByColumn } = this.props;
              const sourceGroupValue = isMoving.data[groupByColumn.name];
              const targetGroupValue = isMoving.target.data[groupByColumn.name]
              const newGroupFilter = { [groupByColumn.name]: targetGroupValue };
              let newOrderValue = {};
              if(orderByColumn){
                const targetOrderValue = isMoving.target.data[orderByColumn.name]
                newOrderValue = { [orderByColumn.name]: targetOrderValue };
                const increment = 0.000001;
                try {
                  const targetSiblingFilter = await getRowFilter(isMoving.target.data, this.props.columns, this.props.tableHandler);
                  if(targetSiblingFilter.error) throw targetSiblingFilter.error;
                  const finalTargetFilter = getSmartGroupFilter(targetSiblingFilter.filter);
                  /** Move target order slightly below its current position which will be taken by the dragged item */
                  const prevTargetItem = await this.props.tableHandler.findOne?.({ 
                      $and: [
                        finalTargetFilter, 
                  ...
                  if(prevTargetItem){
                    newTargetOrderValue = (Number(prevTargetItem[orderByColumn.name]) + Number(targetOrderValue)) / 2;
                  }
                  await this.props.tableHandler.update?.(finalTargetFilter, { [orderByColumn.name]: newTargetOrderValue }, { returning: "*" });
                } catch (err) {
              ...
              if(!isEmpty(newGroupValue) || !isEmpty(newOrderValue)){
                try {
                  ...
                  await this.props.tableHandler.update!(finalFilter, {
                    ...newGroupValue,
                    ...newOrderValue,
                  }, { returning: "*" });
