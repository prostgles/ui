...
import { getRowSiblingData, RowSiblingData } from "./tableUtils/getEditColumn";
...
export const RowCard = ({ prgl, tableName, rows, tableHandler, onSuccess, onPrevOrNext, showR }: RowCardProps) => {
  
  const { state: show, setState } = useReactiveState(showR)
  if(!show) return null;

  const canPrevOrNext = onPrevOrNext && rows?.length && show.type === "update" && Object.values(show.siblingData).some(v => v);
  ...
  return <SmartForm
    ...
    onPrevOrNext={!canPrevOrNext? undefined : async increment => {

      const newRowIndex = show.rowIndex + increment;
      const newFilter = increment === 1? show.siblingData.nextRowFilter : show.siblingData.prevRowFilter;
      if(!newFilter){
        // alert("Reached end");
        return
      }
      ...
      const siblingData = await getRowSiblingData(rows, newRowIndex, cols, tableHandler);
