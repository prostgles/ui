...
export const flattenJoinPathsV2 = (tableName: string, tables: DBSchemaTablesWJoins, prevTables: string[] = [], maxDepth = 4): ParsedJoinPath[][] => {
  ...
  const _table = tables.find(t => t.name === tableName && !prevTables.includes(tableName));
  
  const depth = prevTables.length;
  if(!_table || depth > maxDepth) return [];

  const nextJoinsV2 = _table.joinsV2.filter(j => !prevTables.includes(j.tableName));
  const paths: ParsedJoinPath[][] = nextJoinsV2.flatMap(_j => {
    return _j.on.flatMap(singleOn => {
      ...
      const nestedJoins = flattenJoinPathsV2(_j.tableName, tables, [...prevTables, tableName]);
...
export const getJoinPaths = (tableName: string, tables: DBSchemaTablesWJoins): TargetPath[] => {
  ...
  const pathsWithInfo = paths.map((path, idx) => {
  ...
  }).sort((a, b) => a.path.length - b.path.length || a.table.name.localeCompare(b.table.name));
