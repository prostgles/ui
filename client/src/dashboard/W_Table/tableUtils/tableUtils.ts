  ...
  try {
    ...
    try {
      ...
      colsWInfo = getColWidth(
          ...
          "name", 
          windowWidth
        ).map(c => ({ ...c, width: c.info?.udt_name === "uuid"? 150 : c.width }))
    } catch(e){
      console.error(e)
    }
    ...
    if(!w.columns?.length && table?.info.isFileTable){
      ...
      const origNameColIdx = colsWInfo.findIndex(({ name }) => name === "original_name");
    ...
    if(!wcols) {
      ...
      return cols.some(c => {
        if(wcol?.computedConfig){
          ...
          return !wcol.computedConfig.column || wcol.computedConfig.column === c.name
        } else if(wcol) {
...
export const getJoinedTables = (
  ...
  const upsertJoinV2 = (tableName: string, condition: [string, string][]) => {
    ...
    if(tableJoinIdx > -1){
      const onIdx = joinsV2[tableJoinIdx]!.on.findIndex(cond => cond.every(cols => condition.some(newCols => cols.join() === newCols.join()) ));
      if(onIdx < 0){
  ...
  myCols?.filter(c => c.references).forEach(c => {
    c.references?.forEach(({ ftable, cols, fcols }) => {
      ...
      upsertJoinV2(ftable, cols.map((c, idx) => [c, fcols[idx]!]))
    })
  });
  ...
  tables.filter(t => t.name !== tableName).forEach(({ name, columns }) => {
    columns.forEach(c => {
      const matchingReferences = c.references?.filter(({ ftable }) => ftable === tableName)
      if (matchingReferences?.length) {
        matchingReferences.forEach(({ fcols, cols }) => {
          ...
          upsertJoinV2(name, fcols.map((c, idx) => [c, cols[idx]!]))
        })
      }
    })
  });
...
export const simplifyFilter = (f: AnyObject | undefined) => {
  ...
  if(result){
    while(
      result && "$and" in result && Array.isArray(result.$and) && result.$and.length < 2 || 
      result && "$or" in result && Array.isArray(result.$or) && result.$or.length < 2
    ){
