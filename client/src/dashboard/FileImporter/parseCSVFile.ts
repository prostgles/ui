...
export async function parseCSVFile(file: File, config: FileImporterState["config"]): Promise<{ 
  rows: AnyObject[];
  cols: Col[];
  header: boolean;
}> {
  ...
  const hasHeaders = async (dataPreview: string[][]) => {
    ...
    const header = r0.some((firstRowValue, i)=> {
      ...
      const f0 = getStringFormat(firstRowValue).map(f => f.type).join();
      const f1 = getStringFormat(secondRowValue).map(f => f.type).join();
  ...
  if(firstRow){
    ...
    if(badRows.length){
      console.error("Some rows have been ommited due to the number of columns not coinciding with first row:", badRows)
    }
