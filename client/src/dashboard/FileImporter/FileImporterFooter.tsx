...
export const FileImporterFooter = (props: P) => {
  const { db, selectedFile, onImport, onCancel, importing, openTable, error, customHeadersError } = props;
  
  const hideOpenTable = (!importing?.finished || !importing.tableName || !db[importing.tableName])
  return <>
    ...
    >
      ...
      {(!error && importing && !importing.progress)? <div className="flex-row ai-center ml-auto ta-right">
          Loading file ...
        </div> : null
      }
      ...
      {!importing || importing.finished || error? null : 
        <div className="ImportingProgress flex-row ai-center ml-auto ta-right">
          <div className={"mr-1 flex-col " + ((!importing.progress && !importing.importedRows)? " hidden " : "")}>
            <div style={{ fontSize: "1.5em" }}>{importing.progress?.toFixed(0)}%</div> 
          </div>
          <div className="flex-col ai-center"> 
            ...
            />
            <div className="text-1p5 mt-p5" style={{ fontSize: "1em" }}>{importing.timeElapsed}</div>
          </div>
        </div>}
