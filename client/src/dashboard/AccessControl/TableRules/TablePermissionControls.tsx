...
export const getBasicPermissions = (rules: TableRules = {}): BasicTablePermissions => {
  return getKeys(rules).reduce((a, ruleType) => ({
    ...a,
...
export const TablePermissionControls = (props: TablePermissionControlsProps) => {
  const { 
    tableRules, onChange, className = "", table,  
    errors, style, tablesWithRules: allRules
  } = props;
  ...
  const fileTableRefRules = useFileTableRefTableRules({ table, tablesWithRules: allRules });
  ...
  >
    ...
    .map(ruleType => {
      ...
      const fileRefRules = fileTableRefRules.refTables?.filter(rt => Object.values(rt.colRules).some((colRule) => colRule[ruleType]));
      const isOn = rule ? "on" : fileRefRules && fileRefRules.length? "file-table-with-refs" : undefined;

      const disabledInfo = table?.info.isView && ruleType !== "select"? "Can only select from a view" : undefined;

      return <div key={ruleType} className="flex-row ai-center gap-p25" >
        <Btn 
          data-command={`${ruleType}Rule`}
          disabledInfo={disabledInfo}
          size="small" 
          className={isOn? " " : " "}
          variant={isOn === "on"?  "filled" : isOn === "file-table-with-refs"? "faded" : undefined} 
          iconPath={fileRefRules? mdiCog : undefined}
          iconPosition={fileRefRules? "right" : undefined}
          color={isOn? "action" : undefined} 
          style={{ padding: ".5em .75em", ...(fileRefRules? { gap: "8px" } : {})}}
          onClick={() => {
            if(table?.info.isFileTable){
              setEditedRuleType(ruleType);
              return
            }
            const userIdField = table?.columns.find(c => c.udt_name === "uuid"  && (c.name === "user_id" || table.name === "users" && c.name === "id"));
            if(!isOn && userIdField){
        ...
        {!!table && !fileRefRules && <Btn
          data-command={`${ruleType}RuleAdvanced`}
          size="small" 
          className={isObject(rule) || error? "" : "show-on-parent-hover"}
          style={{ 
