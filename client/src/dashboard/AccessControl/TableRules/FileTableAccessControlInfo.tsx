...
export const FileTableAccessControlInfo = (props: FileTableAccessControlInfoProps) => {
  const { refTables } = useFileTableRefTableRules(props);
  if(!refTables) return null;

  const ruleCols = refTables.map(t => {    
    const cols = Object.entries(t.colRules).filter(([c, r])=> r[props.ruleType]);
  ...
  return <FlexCol>
    ...
    <FlexCol className="gap-p5">
      ...
      <div>
        Any allowed action on the referencing columns is permitted on the referenced file records
      </div>
      {ruleCols.length > 0 && <>
        <div className="mt-1">
          <strong>{props.ruleType.toUpperCase()}</strong> is allowed on files {props.ruleType !== "insert"? "referenced by" : "that will be referenced by"} the permitted records from following tables and columns:  
        </div>
        <ul className="pl-1">
          {ruleCols.map(t => 
...
export const useFileTableRefTableRules = ({ table, tablesWithRules }: {
  table: DBSchemaTablesWJoins[number] | undefined;
  tablesWithRules: TableInfoWithRules[];
}) => {
  const refTables = useMemo(() => !table?.info.isFileTable? undefined : tablesWithRules
    .map(t => {
      const refs = t.columns.filter(c => c.references?.some(r => r.ftable === t.info.fileTableName));
    ...
    .map(t => {
      ...
      const getCols = (fieldFilter: FieldFilter) => {
        const fields = parseFieldFilter({ columns: t.columns.map(c => c.name), fieldFilter });
        return t.refs.filter(c => fields.includes(c.name))
      }

      const selectRefs = r.select === true? t.refs : isObject(r.select)? getCols(r.select.fields) : undefined;
      const insertRefs = r.insert === true? t.refs : isObject(r.insert)? getCols(r.insert.fields) : undefined;
      const deleteRefs = r.delete? t.refs : undefined;
      const updateRefs = r.update === true? t.refs : isObject(r.update)? getCols(r.update.fields) : undefined;
      ...
      return {
        ...
        selectRefs, insertRefs, deleteRefs, updateRefs
      }
    })
    .filter(isDefined),
