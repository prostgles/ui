...
export const TableRulesPopup = ({ 
  ...
  const contextDataSchema = [{ name: "user", columns: prgl.dbsTables.find(t => t.name === "users")!.columns }];
  ...
  const commonProps = { 
    prgl, table, contextDataSchema, userTypes,
    tableRules: localRules, onChange: onChangeRule
  };
  const closePopup = () => {
    onChange(localRules);
    onClose();
  ...
  const { info } = table;
  const ruleWasChanged = JSON.stringify(tableRules[editedRuleType]) === JSON.stringify(localRules[editedRuleType])
  return <Popup 
    title={
      <FlexRow className="gap-p5 ai-start">
        <Icon className="text-2" path={info.isFileTable? mdiFile : info.isView? mdiTableEye : mdiTable} />
        <FlexCol className="gap-p5">
          <div>{table.name}</div>
          {table.info.isFileTable && 
            <div className="font-14 text-2">File metadata is stored in this table</div>
          }
        </FlexCol>
      </FlexRow>
    }
