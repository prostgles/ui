...
export const AccessRuleSummary = ({ rule: r, style, className = ""}: P): JSX.Element => {
  ...
  if(r.type === "Run SQL"){
    ...
    const tableRules: TableRuleSummary[] =  c.filter(t => t.select || t.delete || t.insert || t.update).map(t => {
        return {
    ...
    const chipWrapper = (tableRules: TableRuleSummary[]) => <div 
    ...
    >
      {tableRules.map((r, i) => <Chip key={i} >
        <div className="flex-row-wrap gap-p5" >
        {r.tableName} 
        <div className="flex-row gap-p25 ai-center">
          
          {r.i && <span title="Insert/add data" className={CLASSES.i}>{r.i}</span>}
          {r.s && <span title="Select/view data" className={CLASSES.s}>{r.s}</span>}
          {r.u && <span title="Update/edit data" className={CLASSES.u}>{r.u}</span>}
          {r.d && <span title="Delete/remove data" className={CLASSES.d}>{r.d}</span>}
        </div>
    ...
    return <LabeledRow
      className={"AccessRuleSummary "+ className} style={style}
