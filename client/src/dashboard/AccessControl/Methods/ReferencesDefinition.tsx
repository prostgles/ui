...
export const ReferencesDefinition = ({ onChange, tables, ...arg }: ArgumentDefinitionProps) => {
  ...
  const updateRef = (newRef: Partial<typeof arg["references"]>) => {
    if(!newRef) {
      ...
      onChange({ ...arg, references: { column: "", ...(arg.references || {}), ...newRef } as any })
    }
  }
  ...
  return <>
    ...
    >
      ...
      {refTable && <Select 
        ...
        fullOptions={refTable
          .columns.filter(c =>  ["string", "number", "Date"].includes(c.tsDataType))
          .map(t => ({ key: t.name, label: t.name, subLabel: t.data_type }))!}
        onChange={column => {
          const colInfo = refTable.columns.find(c => c.name === column)!;
          onChange({ 
            ...arg, 
        ...
        {!!arg.references.isFullRow && <>
          ...
          <SwitchToggle 
            label={{
              ...
              variant: "normal",
              info: <>
                After clicking on row view/edit the row panel footer actions will include this method
              </>
            }} 
            className="mr-auto"
          ...
          {!!arg.references.showInRowCard && 
            <FormField 
              ...
              type="text" 
              onChange={actionLabel => updateRef({ showInRowCard: { actionLabel } })} 
            />
          }
        </> }
      </>}
