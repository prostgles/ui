...
function FieldFilterControl({ 
  ...
  const fieldOpts = [
    ...
    { key: "all EXCEPT these fields", label: "All fields except", ["data-command"]: "FieldFilterControl.type.except" }, 
    { key: "all fields",              label: "All fields", disabledInfo: excluded?.fields?.length? excluded.message : undefined }, 
  ] as const;

  const isExcept = isObject(value) && Object.values(value).some(v => !v);
  const fieldOpt = value === "*"? fieldOpts[2].key : (isExcept? fieldOpts[1].key : fieldOpts[0].key);

  const setFields = useCallback((fields: string[] | true, include: boolean) => {
    const newFields = fields === true? "*" : 
      fields
  ...
  const fieldList = columns.map(c => ({ key: c.name, subLabel: c.udt_name, disabledInfo: excluded?.fields?.includes(c.name)? excluded.message : undefined  }));
  const fields = useMemo(() => Array.isArray(value)? value : isObject(value)? getKeys(value) : [], [value]);
  
  useEffect(() => {
    if(fieldList.some(f => f.disabledInfo && fields.includes(f.key))){
      setFields(fieldList.filter(f => !f.disabledInfo).map(f => f.key), true)
    }
  }, [fields, fieldList, setFields]);
  
  const error = getError(value, columns, expectAtLeastOne);

  return <FlexCol className="gap-0" data-command="FieldFilterControl">
    <FlexRow>
      ...
      <Select className="mr-p5" 
        ...
        onChange={opt => {
          if(opt === "all fields"){
            ...
            setFields(fields.length? fields : columns.map(c => c.name), opt === fieldOpts[0].key)
          }
        }}
      />
    </FlexRow>
...
const getError = (v: FieldFilter, columns: ValidatedColumnInfo[], expectAtLeastOne = true) => {
  if(!v) return;
  if(isObject(v)){

    const fields = v;
    const fieldKeys = getKeys(fields);
    if(!fieldKeys.length && expectAtLeastOne) return "Must select at least a field";

    const badCols = fieldKeys.filter(fieldName => !columns.some(c => c.name === fieldName))
    if(badCols.length){
