import { getFinalFilter, parseContextVal, type GroupedDetailedFilter, type SimpleFilter } from "../../../../../commonTypes/filterUtils";
import type { ForcedData } from "../../../../../commonTypes/publishUtils";
import type { SelectRuleControlProps } from "./SelectRuleControl";
type GetComparablePGPolicyArgs = Pick<SelectRuleControlProps, "table" | "userTypes" | "prgl"> & {
...
export const getComparablePGPolicy = async ({ forcedFilterDetailed, checkFilterDetailed, command, forcedDataDetail, userTypes, table, prgl, excludeRLSStatement }: GetComparablePGPolicyArgs) => {
  ...
  const getSingleFilterCondition = async (f: SimpleFilter) => {
    if("contextValue" in f){
      ...
      if(!col) return "";
      const contextVal: string = parseContextVal(f, undefined, { forInfoOnly: "pg" })
      return `${f.fieldName} ${f.type ?? "="} ${contextVal}::${col.udt_name}`;
    }
    const parsedFilter = getFinalFilter(f, undefined, { columns });
    try {
      const condition = (await prgl.db[table.name]?.find?.(parsedFilter, { returnType: "statement-where" })) as any as string;
      return condition.trim();
    } catch(err){
      return "";
    }
  }
  const getFilterCondition = async (filter: GroupedDetailedFilter | undefined) => {
    if(!filter) return "";
    const isAnd = "$and" in filter;
    const filters = isAnd? filter.$and : filter.$or;
    const conditions = await Promise.all(filters.map(f => getSingleFilterCondition(f as SimpleFilter)));
  ...
  const policyName = JSON.stringify(`prostgles_${table.name}_${finalAction}_${userTypes.sort().join("_")}`);
