...
export const DeleteRuleControl = ({ rule: rawRule, onChange, table, prgl, contextDataSchema: contextData, userTypes }: P) => {
  const rule: DeleteRule | undefined = rawRule === true? { filterFields: "*" } : isObject(rawRule)? rawRule : undefined
  const error = rule && isObject(rule.filterFields) && !getKeys(rule.filterFields).length? "Must select at least one field" : undefined;
      ...
      <FilterControl 
        ...
        info={
          <div className="flex-col gap-1">
            <div>If specified, a filter is added to each delete to ensure no other records can be deleted</div>
          </div>
        }
        db={prgl.db}
