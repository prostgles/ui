import { mdiDotsHorizontal, mdiFilter, mdiSortReverseVariant, mdiTextBoxSearchOutline } from "@mdi/js";
  ...
  return <FlexCol className="gap-2">
    ...
    {rule && <FlexCol className="gap-2">
      ...
      <FilterControl 
        ...
        info={
          <FlexCol>
            <div>Filter used in each select to ensure other records cannot be viewed</div>
          </FlexCol>
        }
        db={db}
        methods={dbMethods}
...
export const getSelectRuleError = ({ tableRules }: Pick<SelectRuleControlProps, "table" | "prgl" | "tableRules">): string | undefined => {
  if(tableRules.select){
    const rule = tableRules.select
    if(isObject(rule) && isObject(rule.fields) && !getKeys(rule.fields).length){
...
export const RuleExpandSection = ({ children, expanded }: { children: React.ReactNode; expanded?: boolean; }) => {
