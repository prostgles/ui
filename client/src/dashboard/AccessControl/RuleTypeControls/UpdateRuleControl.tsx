...
type P = Pick<Required<TablePermissionControlsProps>, "prgl" | "table" | "tableRules" | "userTypes"> & {
      ...
      <FilterControl 
        ...
        info={
          <div className="flex-col gap-1">
            <div>Filter added to each update to ensure other records cannot be updated</div>
          </div>
        }
        db={prgl.db}
...
export const getUpdateRuleError = ({ rule: rawRule, table, tableRules }: Pick<P, "rule" | "table" | "tableRules">): string | undefined => {
  const rule: UpdateRule = isObject(rawRule)? rawRule : { fields: "*" }
  
  let error = isObject(rule.fields) && !getKeys(rule.fields).length? "Must select at least one field" : undefined;
  
  if(!error){
    const pkeyCols = table.columns.filter(c => c.is_pkey).map(c => c.name);
    const notAllPkeysSelected = pkeyCols.length && (
      !tableRules.select || 
      isObject(tableRules.select) && 
      tableRules.select.fields !== "*" && 
      parseFieldFilter({ columns: pkeyCols, fieldFilter: tableRules.select.fields }).length !== pkeyCols.length
    );
      ...
      if(!tableRules.select || 
        ...
        tableRules.select.filterFields !== "*"
          ...
          if(tableRules.select && isObject(tableRules.select)){
            ...
            const selectFields = parseFieldFilter({ columns, fieldFilter: tableRules.select.fields });
            const selectFilterFields = parseFieldFilter({ columns, fieldFilter: tableRules.select.filterFields ?? "*" });
            const noFilterFields = !selectFilterFields.filter(f => selectFields.includes(f)).length;
            if(noFilterFields){
              error = err
            }
