...
export const ComparablePGPolicies = ({ prgl, rule }: { prgl: Prgl; rule: EditedAccessRule; }) => {
  const policies = usePromise(async () => {
    ...
    if(r.type === "All views/tables"){
      const table_policies = await Promise.all(tables.map(async table => {
        return await Promise.all(r.allowAllTables.map(command => {
      ...
      return await Promise.all(r.customTables.map(async tablePermission => {
        const table = tables.find(t => t.name === tablePermission.tableName);
        if(!table) return "";
        const commands = ACCESS_RULE_METHODS.filter(m => tablePermission[m]);
        const commandPolicies = await Promise.all(commands.map(command => {
  ...
  return <PopupMenu 
    ...
    button={<Btn color="action" data-command="ComparablePGPolicies">Comparable PG Policies</Btn>}
    render={(pClose) => (
      <FlexCol className="f-1">
        <InfoRow variant="naked">
          Some features like limiting read/write access to specific columns cannot be easily achieved with Postgres
          <br />
          The policies below limit which rows can be read/modified (USING clause) ensuring updated/inserted rows satisfy specific conditions (CHECK clause)
        </InfoRow>
        <CodeExample 
          language="sql" 
