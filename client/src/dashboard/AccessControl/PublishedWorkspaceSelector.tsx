...
export const PublishedWorkspaceSelector = ({ prgl: {dbs , connectionId}, dbsPermissions, onChange, onChangeRule, dbPermissions, className = "", style, onSetError, tables} : P) => {
  ...
  useEffectAsync(async () => {
    ...
    const workspaceIds = dbsPermissions?.viewPublishedWorkspaces?.workspaceIds ?? [];
    if(workspaceIds.length){
      const wsps = await dbs.workspaces.find({ "id.$in": workspaceIds });
      if(!getIsMounted()) return;
      const missingWorkspaceIds = workspaceIds.filter(wid => !wsps.some(w => w.id === wid ))
      if(missingWorkspaceIds.length){
        onSetError(`${missingWorkspaceIds.length} Published workspaces not found: \n  ${missingWorkspaceIds.join("\n ")}`);
        return;
      }
      const { msg } = await getWorkspaceTables(dbs, workspaceIds, dbPermissions, tables);
      if(!getIsMounted()) return; 
      if(msg){
        wspErrors = msg;
      }
    ...
    if(!wspErrors && (!dbsPermissions?.createWorkspaces && !dbsPermissions?.viewPublishedWorkspaces?.workspaceIds?.length)){
      wspErrors = "Must allow 'Create workspaces' or select at least one workspace within 'Access published workspaces' ";
    }
    ...
    <div className="relative bg-color-0 rounded flex-row gap-2 ml-2 " >
      ...
      <SmartSelect 
        ...
        onChange={async workspaceIds => {
          ...
          const { dbPermissionsCorrected } = await getWorkspaceTables(dbs, workspaceIds, dbPermissions, tables);
...
export const getWorkspaceTables = async (dbs: DBS, workspaceIds: string[], dbPermissions: DeepPartial<AccessRule["dbPermissions"]>, tables: DBSchemaTablesWJoins) => {
  ...
  if(dbPermissions.type === "Custom") {
    const workspaceWindows = await dbs.windows.find({ "workspace_id.$in": workspaceIds }); 
    const tableWindows = workspaceWindows.filter(w => (w.type === "table" && !!w.table_name)) as WindowData<"table">[];
    const { customTables } = dbPermissions;
    const _missingWindowTables = tableWindows
      .map(w => {
        const table = tables.find(t => t.name === w.table_name);
        const tableWindowColumns = table?.columns.filter(tc => w.columns?.some(c => tc.name === c.name && c.show && (!c.computedConfig || c.computedConfig.isColumn || tc.name === c.computedConfig.column)));
      ...
      .filter(t => !customTables?.some(ct => t && ct.tableName === t.table_name))
    ...
    dbPermissionsCorrected = {
      type: "Custom",
      customTables: validMissingWindowTables.map(({ table_name, tableWindowColumns, table }) => {
        const windowFields = tableWindowColumns!.map(c => c.name);
        const fields = windowFields.length === table.columns.length? "*" as const : windowFields;
        return {
        ...
        customTables?.filter(ct => !missingTableNames.includes(ct.tableName)) ?? [] as any
      )
    }
  }
