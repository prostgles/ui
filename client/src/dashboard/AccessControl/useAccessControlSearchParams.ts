...
const getActionFromSearchParams = (searchParams: URLSearchParams): AccessControlAction | undefined => {
  ...
  const action: AccessControlAction | undefined = selectedRuleId? {
    type: "edit",
...
export const useAccessControlSearchParams = () => {
  ...
  const setAction = (newAction: AccessControlAction | undefined ) => {
    ...
    const currentAction = getActionFromSearchParams(new URLSearchParams(window.location.search));
...
export const getAccessControlHref = ({ connectionId, ...otherOpts }: { connectionId: string; selectedRuleId?: string; tableName?: string; }) => {
  const sp = new URLSearchParams();
  sp.set("section", "access_control");
  if(otherOpts.selectedRuleId) sp.set(SEARCH_PARAMS.SELECTED_RULE_ID, otherOpts.selectedRuleId);
