...
export const CONTEXT_FILTER_OPERANDS = ["=", "<>", ">=", "<=", ">", "<"] as const;
...
export const ContextFilter = ({ columns, contextCols, onChange, filter }: ContextFilterProps) => {

  const [value, setValue] = useState<Partial<ContextFilterVal>>({ ...filter, type: "=" });
  const col = columns.find(c => c.name === value.fieldName);
  const ctxCol = contextCols.find(c => c.name === value.objectPropertyName);

  useEffect(() => {
    const isNewOrUpdated = !filter || JSON.stringify(filter) !== JSON.stringify(value);
    const finalFilter: ContextFilterVal | undefined = (isNewOrUpdated && col && ctxCol && col.tsDataType === ctxCol.tsDataType)? (value as ContextFilterVal) : undefined;
    if(finalFilter){
  ...
  return <div className="flex-col gap-1">
    <div className="flex-row gap-p5 ai-end">
      ...
      <Select
        ...
        fullOptions={columns.filter(c => contextCols.some(ctxCol => ctxCol.tsDataType === c.tsDataType)).map(c => ({
          key: c.name,
          label: c.label,
...
type AddContextFilterProps = Pick<ContextFilterProps, "columns" | "contextCols"> & {
  onAdd: (newFilter: SimpleFilter) => void;
}

export const AddContextFilter = ({ columns, contextCols, onAdd }: AddContextFilterProps) => {
