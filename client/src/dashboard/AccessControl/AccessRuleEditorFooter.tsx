...
export const AccessRuleEditorFooter = (props: P) => {
  ...
  const { dbs, action,  database_id, connectionId, onCancel, error: wspError, editedRule } = props;
  ...
  const { newRule, onChange, ruleWasEdited, type, ruleErrorMessage } = editedRule ?? {};
  const error = wspError || localError || ruleErrorMessage;

  return <FlexCol>
    <ErrorComponent error={error} variant="outlined" />
    {newRule && onChange && type !== "create" && <ButtonBar 
      error={localError}
      buttons={([
        { 
          children: "Cancel", 
        ...
        selectedRuleId? { 
          ...
          onClickPromise: async () => {
            try {
              await dbs.access_control_user_types.delete({ access_control_id: selectedRuleId });
        ...
        } satisfies BtnProps<void> :  undefined,
        { 
          children: (!selectedRuleId? `Create rule` : `Update rule`), 
          variant: "filled",
          color: "action",
          disabledInfo: wspError? "Must fix errors" :  ruleErrorMessage || localError || (ruleWasEdited? undefined : "Nothing to update"),
          "data-command": "config.ac.save",
          onClickPromise: async () => {
            try {
              await upsertRule({ action, newRule, database_id, connectionId, dbs });
              setLocalError(undefined);
              
              setSuccess(!selectedRuleId? `Rule created!` : `Rule updated!`);
  ...
  if(!connection || !connectionId){
    ...
    const insertRelatedData = async (access_control_id: number) => {
      ...
      if(userGroupNames.length){
        await dbs.access_control_user_types.insert(userGroupNames.map(user_type => ({ access_control_id, user_type })));
      }
    ...
    if(action.type === "edit"){
      const { selectedRuleId } = action;
      await dbs.access_control.update({ id: selectedRuleId }, newRuleWithoutSomeExtraKeys);
