...
export const PRunSQL = ({ dbPermissions, onChange, dbsConnection, prgl: {connection, db} }: DBPermissionEditorProps<"Run SQL">) => {
  ...
  const superAccessToSameServerAsDBS = areEqual(connection, dbsConnection, ["db_host", "db_port"]);
      ...
      <div className="flex-col gap-p5  my-2d" >
        {roleInfo?.rolsuper && superAccessToSameServerAsDBS? 
          <InfoRow color="warning">
            This rule will allow <strong>superuser</strong> access to state database server <strong>{dbsConnection.db_host}:{dbsConnection.db_port}</strong> 
            <br></br>
            <br></br>
            User may update their privilege to the same level of access as a type <strong>"admin"</strong> user
          </InfoRow> :
          roleInfo?.rolsuper? 
            <InfoRow color="warning">
              Superuser access (Bypasses all permission checks, except the right to log in. This is a dangerous privilege and should not be used carelessly)
            </InfoRow> :
          <LabeledRow label="PG Role Attributes" className="ws-wrap"  >
            {[
              ...
              roleInfo?.rolvaliduntil && ("Valid Until" + roleInfo.rolvaliduntil),
            ].filter(v => v)}
          </LabeledRow>
        }
      </div>
