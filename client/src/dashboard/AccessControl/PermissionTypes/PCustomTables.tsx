...
export type DBPermissionEditorProps<T extends EditedAccessRule["dbPermissions"]["type"]> = PermissionEditProps & { 
  dbPermissions:  DBPermissionCustomTables<T>
  onChange: (n: DBPermissionCustomTables<T>)=>void; 
  tablesWithRules: TableInfoWithRules[];
}

export const PCustomTables = ({ dbPermissions, onChange, contextData, prgl, userTypes, tablesWithRules, editedRule }: DBPermissionEditorProps<"Custom">) => {
  const { tables } = prgl;
  ...
  return <FlexCol className="PCustomTables gap-0 min-h-0 h-fit" style={{ maxHeight: "60vh"}}>
    <div className="f-1 jc-end ai-end flex-row">
      <SwitchToggle label="Show allowed only" 
        disabledInfo={dbPermissions.customTables.length? undefined : "No allowed tables to show" }
    ...
    <div className="flex-row-wrap gap-p5 ai-center p-p5 py-1 ">
      ...
      <TablePermissionControls
        ...
        onChange={newTableRules => {
          ...
          newDbPermissions.customTables = tables.map(t => {
            const existingRule = newDbPermissions.customTables.find(ct => ct.tableName === t.name);
            ...
            if(t.info.isView && toggledRuleName !== "select"){
              ...
              tableRules[toggledRuleName] = newTableRules[toggledRuleName]? true : undefined;
            }
            return tableRules;
      ...
      items={
        tablesWithRules
          ...
          .map(t => {
            ...
            const tableRules: TableRules & { tableName: string } = existingRule ?? { tableName: t.name };
            ...
            const setTableRules = (
              ...
              if(rules){
                ...
                if(!existingRule){
                  ...
                  newDbPermissions.customTables = newDbPermissions.customTables.map(ct => ct.tableName === t.name? tableRules : ct);
                }
            ...
            const icon = t.info.isFileTable? { path: mdiFile, title: "File table" } : t.info.isView? { title: "View", path: mdiTableEye } : { title: "Table", path: mdiTable }
            const isNotFromWorkspaceTables = !t.info.isFileTable && !editedRule?.newRule?.dbsPermissions?.createWorkspaces && editedRule?.worspaceTableAndColumns?.length && !editedRule.worspaceTableAndColumns.some(wt => wt.tableName === t.name)
            return {
              key: t.name,
              styles: {
                labelWrapper: {  
                ...
                rowInner: window.isLowWidthScreen? {
              ...
              disabledInfo: isNotFromWorkspaceTables? "Cannot allow non workspace tables" : undefined,
              title: t.name,
              rowStyle: { border: "1px solid var(--b-default)" },
              contentLeft: <Icon className="mr-p5 text-2" title={icon.title} path={icon.path} />,
              contentRight: <TablePermissionControls
                key={t.name}
