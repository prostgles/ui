...
export const PasswordlessSetup = ({ dbsMethods }: ExtraProps) => {
  ...
  const [{ username, password, passwordconfirm }, setNewUser] = useState<NewUser>({});

  const setU = (u: NewUser) => {
    setNewUser(user => ({...user, ...u }))
  }
  const issues = !username ? "Username not provided" :
    ...
    <PopupMenu
      ...
      render={(pClose) => 
        <div className="flex-col gap-1">
          <FormField id="username" label="Username" value={username ?? ""} onChange={username => setU({ username })} />
          <FormField id="new-password" autoComplete="off" label="Password" type="password" value={password ?? ""} onChange={password => setU({ password })} />
          <FormField id="confirm_password" autoComplete="false" label="Confirm password" type="password" value={passwordconfirm ?? ""} onChange={passwordconfirm => setU({ passwordconfirm })} />
        </div>
      }
      footerButtons={[
        { 
