...
export const  FileColumnConfigControls = (props: FileColumnConfigControlsProps) => {
  ...
  const linkedTables = tables.flatMap(t => {
    ...
    const droppedFileColumns = Object.keys(refsConfig[t.name]?.referenceColumns ?? {}).filter(colName => !t.columns.find(c => c.name === colName) && !fileColumns.find(c => c.name === colName));
    if((fileColumns.length || droppedFileColumns.length) && isNotLookupTable){
      return [
  ...
  return <FlexCol className="FileColumnConfigControls w-fit min-h-0 mt-1">
    ...
    <SearchList 
      ...
      items={linkedTables.map((linkedTable, i) => {
        ...
        return {
          key: linkedTable.key,
          rowStyle: !linkedTable.fileColumn? { border: "1px solid var(--danger)" } : !i? undefined : { borderTop: `1px solid var(--b-color)` },
          subLabel: linkedTable.fileColumn? getFileColumnConfigDescription(linkedTable.fileColumn.file!).full : "Missing from table",
          contentLeft: <Icon path={mdiLink} className="mr-p5" />,
          contentRight: linkedTable.fileColumn? <Btn 
            ...
            <Btn
              ...
              onClickPromise={async () => {
                ...
                delete newConfig[linkedTable.name]!.referenceColumns[linkedTable.columnName];
                if(isEmpty(newConfig[linkedTable.name]?.referenceColumns)){
