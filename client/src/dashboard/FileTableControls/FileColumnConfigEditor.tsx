...
export const FileColumnConfigEditor = ({ tableName, columnName, refsConfig, onChange, onSetError }: FileColumnConfigProps) => {
  ...
  if (getKeys(colConfig).every(k => (k as any) === "maxFileSizeMB") || "acceptedContent" in colConfig && colConfig.acceptedContent) {
    ...
    fullOptions = CONTENT_OPTIONS.map(key => ({
      key,
      checked: isChecked(key, "acceptedContent" in colConfig ? colConfig.acceptedContent : "*")
    }));
    onChangeOpts = (opts) => {
      //@ts-ignore
      updateMergeColConfig({ acceptedContent: opts.length === fullOptions.length ? "*" : opts, acceptedFileTypes: undefined, acceptedContentType: undefined })
    }
    ...
    onChangeOpts = opts => {
      const acceptedContentType: Extract<FileColumnConfig, { acceptedContentType: any }> = { acceptedContentType: opts.length === fullOptions.length ? "*" : opts as any }
      //@ts-ignore
      updateMergeColConfig({ acceptedContentType, acceptedContent: undefined, acceptedFileTypes: undefined })
    };
    ...
    onChangeOpts = opts => {
      //@ts-ignore
      updateMergeColConfig({ acceptedFileTypes: opts.length === fullOptions.length ? "*" : opts, acceptedContent: undefined, acceptedContentType: undefined })
    };
  }
  ...
  return <FlexCol className="f-1 min-h-0 gap-2">
    ...
    <ButtonGroup 
      ...
      onChange={newMode => {
        updateMergeColConfig(
          newMode === "By basic content type" ? { acceptedContent: "*", acceptedFileTypes: undefined, acceptedContentType: undefined } :
            newMode === "By content type" ? { acceptedContentType: "*", acceptedFileTypes: undefined, acceptedContent: undefined } : {
              acceptedFileTypes: "*", acceptedContent: undefined, acceptedContentType: undefined
            }
        )
      }}
    />
    <SearchList
      ...
      items={fullOptions.map(o => ({
        ...o,
        onPress: () => {
          const newItems = fullOptions.filter(d => d.key === o.key ? !d.checked : d.checked).map(d => d.key).filter(isDefined);
          onChangeOpts(newItems)
        }
      }))}
      onMultiToggle={items => {
        const newItems = items.filter(d => d.checked).map(d => d.key).filter(isDefined);
