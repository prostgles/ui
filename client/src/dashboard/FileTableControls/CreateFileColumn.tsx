...
export const CreateFileColumn = ({ db, tables, fileTable, tableName: _tableName, prgl, onClose }: CreateReferencedColumnProps) => {

  const [tableName, setTableName] = useState(_tableName);
  if(!fileTable) {
    return <Popup title="Create file column" clickCatchStyle={{ opacity: .5 }} onClickClose={true} positioning="top-center" onClose={onClose}>
      <InfoRow variant="naked">Must enable <Link to={`/connection-config/${prgl.connectionId}?section=file_storage`}>file storage</Link> first</InfoRow>
    </Popup>
  }
  if(tableName) {
    return <CreateFileColumnOptions 
      db={db} 
      tables={tables} 
      fileTable={fileTable} 
      tableName={tableName} 
...
const CreateFileColumnOptions = ({ db, prgl, fileTable, tableName, onDone }: Omit<CreateReferencedColumnProps, "fileTable"> & { fileTable: string; tableName: string; onDone: VoidFunction;}) => {
  const [colName, setColName] = useState<string>();
  const [optional, setOptional] = useState(true);
  const { refsConfig, setRefsConfig, updateRefsConfig, canUpdateRefColumns: canUpdate } = useFileTableConfigControls(prgl);
  ...
  return <Popup 
    ...
    footerButtons={[
      ...
      { 
        ...
        disabledInfo: error? "Must fix error" : !colName? "New column name missing" : undefined,
