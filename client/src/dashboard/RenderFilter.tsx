...
import type { GroupedDetailedFilter, SimpleFilter } from "../../../commonTypes/filterUtils";
import Btn from "../components/Btn";
import PopupMenu from "../components/PopupMenu";
import { pickKeys } from "../utils";
import type { ContextDataSchema, ForcedFilterControlProps, SingleGroupFilter } from "./AccessControl/OptionControllers/FilterControl";
...
export const RenderFilter = (props: RenderFilterProps) => {
  const { filter: f = { $and: [] }, onChange, contextData, mode, title = `Edit ${props.itemName}s`, itemName, ...otherProps } = props;
  ...
  const content = (minimised?: boolean, showAddFilter?: boolean) =>  (
    <>
      <SmartFilter
        ...
        filterClassName={(minimised ?? filters.some(f => f.minimised))? " " : " rounded  b b-action"}
        {...pickKeys(otherProps, ["db", "tableName", "tables", "selectedColumns"])}
  ...
  return <PopupMenu
    ...
    footerButtons={[
      { 
        ...
        disabledInfo: filters.some(f => f.disabled)? `Some ${itemName}s are incomplete/disabled` : undefined
      }
    ]}
  >
    {content(false, true)}
