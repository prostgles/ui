  ...
  el.addEventListener("touchmove", function(event) {
    ...
    if (event.touches.length === 2 && event.targetTouches.length === 2) {
      ...
      if (hypo === undefined) {
        ...
        const x = rect.x + (event.touches[0]!.pageX + event.touches[1]!.pageX)/2;
        const y = rect.y + (event.touches[0]!.pageY + event.touches[1]!.pageY)/2;
...
export const toDateStr = (date = new Date(), opts: Intl.DateTimeFormatOptions): string => {
...
export class Chart extends RTComp<{
  ...
  render(){
    ...
    return (
      ...
      >
        <canvas ref={e => { if(e) this.canv = e; }} className="f-1" />
      </div>
    )
  }
}
