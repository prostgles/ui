...
export function onRenderTimechart (this: TimeChart, delta: Partial<TimeChartProps & TimeChartD>, dd: DeepPartial<TimeChartD> | undefined) {
  ...
  if(delta.layers){
    ...
    layers.map((layer)=> {
      ...
      if(firstDataItem){
        ...
        const circlesXY = _data.map((c, idx)=> {
        ...
        const circles: Circle[] = displayedCircles
          .map((d, i)=> ({
            id: i,
            type: "circle",
            coords: [d.x, d.y],
        ...
        const showCircles = renderStyle === "scatter plot" || renderStyle === "line" && lineTooShort;
        ...
        this.mainShapes = this.mainShapes.concat([
          ...finalShapes,
          ...labels,
        ]);
      }
  ...
  if(reRender){
    ...
    const getShapes: GetShapes = () => {
      ...
      const { minDate, maxDate, dates, xScale, yScale, minVal, maxVal, layers } = this.data;
      ...
      const timeAxisTicks = this.props.showXAxis === false? [] : getTimeAxisTicks({
        ...
        height: h, 
        values: layers.flatMap(l => l.sortedParsedData?.map(d => new Date(d.date))).filter(isDefined),
      ...
      yTickValues.forEach((v, i) => {
        const yTick: ChartedText = {
          ...
          fillStyle: (compactYAxis && !i)?  getCssVariableValue("--color-ticks-1") : getCssVariableValue("--color-ticks-0"),
          // background: {
...
type GetBinValueLabelArgs = Pick<TimeChartProps, "renderStyle" | "binValueLabelMaxDecimals" | "showBinLabels"> & {
  circles: Circle[];
  showCircles: boolean;
}
const getBinValueLabels = ({ circles, renderStyle, binValueLabelMaxDecimals, showCircles, showBinLabels }: GetBinValueLabelArgs) => {

  const getLabel = (c: Circle, prevP: Point | undefined, nextP: Point | undefined) => {
    const [x, y] = c.coords;
    ...
    if(prevP && nextP){
      ...
      const lineIsVertical = [a1, a2].every(v => v > -120 && v < -70) || [a1, a2].every(v => v > 70 && v < 120);
      if(lineIsVertical) xOffset = -(textSize)
    }

    const showBelow = renderStyle === "line" && !showCircles && (angles.p || angles.n);
    const yOffset = showBelow? (textMargin + textSize) : -textMargin;
    const text = binValueLabelMaxDecimals? c.data.value.toFixed(binValueLabelMaxDecimals) : c.data.value;
...
function getYTickValues(args: { min: number; max: number; steps: number; }): number[] {
...
function getNicestVal(min: number, max: number): number {
  ...
  const roundVal = (val: string): number | undefined => {
    ...
    return [nv0, nv5, nv0Next].find(v => Number.isFinite(v) && v >= min && v <= max);
  }
...
export const getCssVariableValue = (varName: string, node: HTMLElement = document.body) => {
