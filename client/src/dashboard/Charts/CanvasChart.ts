...
export type Circle = ShapeBase & StrokeProps & FillProps & {
  type: "circle";
  r: number;
  coords: Point;
}

export type Rectangle = ShapeBase & StrokeProps & FillProps & {
  type: "rectangle";
  w: number;
  h: number;
  borderRadius?: number;
...
export type ChartedText = ShapeBase & FillProps & {
  type: "text";
  text: string;
  font?: string;
  textAlign?: CanvasTextAlign;
  coords: Point;
  background?: Partial<StrokeProps> & Partial<FillProps> & {
    padding?: number;
    borderRadius?: number;
  }
}

export type MultiLine = ShapeBase & StrokeProps & {
  type: "multiline";
  coords: Point[];
  variant?: "smooth";
};
export type Polygon = ShapeBase & StrokeProps & FillProps &  {
  type: "polygon";
  coords: Point[];
};
...
export type TextMeasurement = { width: number; fontHeight: number; actualHeight: number };
...
export class CanvasChart {
  ...
  get events() {
    return (this.opts?.events && this.opts.events.disabled !== true)? this.opts.events : undefined;
  }
  ...
  setView(view: Partial<ChartView>){
    ...
    const { yScaleLocked = false, yPanLocked = false, minXScale, maxXScale } = this.opts;
  ...
  getDataXY = (xCanvas: number, yCanvas: number): [number, number] => {
    const { view: { xScale, yScale, xO, yO } } = this;
    return [
      (xCanvas - xO)/xScale,
      (yCanvas - yO)/yScale 
    ];
    ...
    shapes.map(s => {
      ...
      if(s.type === "rectangle"){
        ...
        if(s.background){
          ...
          const y = coords[1] - (allLowerCase(s.text)? 1 : 1) * txtSize.actualHeight - txtPadding;
 ...
 */
 function roundRect(ctx, x: number, y: number, width: number, height: number, radius: number | { tl: number; tr: number; bl: number; br: number }) {
