...
export const getTimechartTooltipShapes = function(this: TimeChart){
  ...
  if(!xCursor || this.panning  || (yCursor ?? 0) > yMax - 14) return undefined;
    ...
    const { layers: iLayers = [], snapped_x } = this.getIntersections(xCursor) ?? {};
    if(!iLayers.length || !this.data) { //  && !("layers" in delta)
      return undefined;
    }
    ...
    const getMinOffset = () => {
      const minOffset = this.data?.dates.reduce((a, v, i, arr) => {
        if(i){
    ...
    if(dateDelta <= SECOND || tooltipDate.getMilliseconds() !== 0 && snapped_x !== undefined){
      tooltipBottomDateText = toDateStr(tooltipDate, HoursMinutesSeconds) + "." + tooltipDate.getMilliseconds();
    ...
    //   tooltipBottomDateText = toDateStr(tooltipDate, HoursMinutes);
      ...
      tooltipBottomDateText = toDateStr(tooltipDate, { ...HoursMinutes, day: "numeric", month: "short", year: "2-digit"});

    } else {
      tooltipBottomDateText = toDateStr(tooltipDate, { day: "numeric", month: "short", year: "2-digit"});
    }
    ...
    textLabels = textLabels.map((l, i)=> {
      const x = l.coords[0];
      const offset = tooltipPosition === "top"? 0 : tooltipPosition === "bottom"? freeHeight : tooltipPosition === "middle"? freeHeight * .5 : undefined; 
      let y = offset !== undefined? (offset + labelHeight * .75 + i * labelHeight) : l.coords[1];
