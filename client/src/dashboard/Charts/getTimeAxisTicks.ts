...
export function getTimeAxisTicks(args: GetTimeTicksOpts): ChartedText[] {
  const { minDate: lDate, maxDate: rDate, getX, leftX, rightX, height, measureText, getScreenXY, values } = args;
  ...
  const getLabelHHMM = (d: Date) => toDateStr(d, { hour: "2-digit", minute: "2-digit" });
  const getStartMINUTES = (d: Date) => new Date(d.getFullYear(), d.getMonth() , d.getDate(), d.getHours(), d.getMinutes());
  ...
  const incrementers: DateIncrementer[] = [
    ...
    },
    {
      ...
      id: "Weekly",
      dateDelta:  DAY * 7,
      getStart: (d: Date) => new Date(d.getFullYear(), d.getMonth() , d.getDate()),
    ...
    ...[12, 6, 3, 2, 1].map(multiplier => ({
      ...
      getStart: (d: Date) => new Date(d.getFullYear(), d.getMonth() , d.getDate(), d.getHours()),
    ...
    ...[30, 20, 15, 10, 5, 2, 1].map(multiplier => ({
      ...
      getLabel: (d: Date) => toDateStr(d, { hour: "2-digit", minute: "2-digit", second: "2-digit" }),
    })),
    ...[500, 250, 100, 50, 25, 15, 10, 5, 2, 1].map(multiplier => ({
      ...
      getLabel: (d: Date) => toDateStr(d, { hour: "2-digit", minute: "2-digit", second: "2-digit" }) + "." + d.toISOString().slice(20 ,-1),
    })), 
  ];

  const MIDTICK_STYLE: Pick<ChartedText, "type"| "font" | "fillStyle" | "textAlign"> = {
    type: "text",
    font: "12px Arial",
    fillStyle: color0,
    textAlign: "center",
  ...
  if(Number.isFinite(+lDate) && +lDate === +rDate){
    const row1 = toDateStr(lDate, { hour: "2-digit", minute: "2-digit", second: "2-digit" });
  ...
  let incs = incrementers.map(inc => {
    ...
    const v = inc.getStart(new Date(+lDate)), 
      ...
      tickWidth = measureText(getTickText({ text: inc.getLabel(new Date(2001, 1, 1)) })).width,
  ...
  const theChosenInc = incs.sort((a, b) => a.diffFromIdeal - b.diffFromIdeal)[0]!.inc;

  let midTicks: {x: number; v: number; label: string}[] = [];
  const getTickChartedText = (tick: typeof midTicks[number], i: number): ChartedText & { xRange: [number, number]; } => {
  ...
  const getProvidedTicks = (): undefined | typeof midTicks => {
    ...
    const noOverlap = chartedTicks?.every((t, i) => {
      ...
      return ![prevT, nextT].filter(isDefined).some((st: typeof t) => {
        const overlap = t.xRange[0] < st.xRange[1] && t.xRange[1] > st.xRange[0];
  ...
  if(age.seconds < 10){
    lt.text = toDateStr(lDate, { hour: "2-digit", minute: "2-digit", second: "2-digit", hourCycle: "h23" }) + "." + lDate.getMilliseconds()
    rt.text = toDateStr(rDate, { hour: "2-digit", minute: "2-digit", second: "2-digit", hourCycle: "h23" }) + "." + rDate.getMilliseconds()
  ...
  const leftTickMaxWidth = Math.max(
    measureText(lt).width,
    measureText(lb).width,
  ) + 10;
  const rightTickMaxWidth = Math.max(
    measureText(rt).width,
    measureText(rb).width,  
  ) + 10;
  const leftTickMaxX = Math.max(
    getScreenXY(leftX, 0)[0] + leftTickMaxWidth
  )
  const rightTickMinX = Math.max(
    getScreenXY(rightX, 0)[0] - rightTickMaxWidth
  )
