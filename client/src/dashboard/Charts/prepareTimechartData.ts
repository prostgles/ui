...
export const prepareTimechartData = function(this: TimeChart){
  ...
  let tcLayers: TimeChartLayer[] = (layers as TimeChartLayer[])
    .map(l => {
      ...
      const sortedParsedData = data.map(d => {
  ...
  const xScale = scaleLinear().domain([minDate, maxDate]).range([xMin, xMax]).clamp(true);
  const xScaleYLabels = scaleLinear().domain([minDate, maxDate]).range([xForYLabels, xMax]).clamp(true);
  const yScale = scaleLinear().domain([maxVal, minVal]).range([yMin, yMax]).clamp(true);
