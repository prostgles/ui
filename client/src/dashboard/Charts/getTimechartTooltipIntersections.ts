...
export const getTimechartTooltipIntersections = function(this: TimeChart, xCanvas: number): IntersectedLayers | undefined {
  let snapped_x;
  ...
  const SNAP_DISTANCE = this.lineLayers?.some(layer => layer.coords.length < 3)? 10 : 5;
      ...
      if(layer.coords.length === 1 || i < layer.coords.length - 1){
        ...
        if(xDist <= SNAP_DISTANCE){
          ...
          /* Point along line */
            ...
            renderStyle === "line" &&
           ( xPoint >= x     && xPoint < xNext || 
            xPoint >= xNext && xPoint < x)
          ){
            const xDiff = xNext - x,
              yDiff = yNext - y,
              perc = (xPoint - x)/xDiff;
