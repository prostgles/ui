...
export type TimeChartProps = {
  ...
  onExtentChanged?: (visibleData: DateExtent, viewPort: DateExtent, opts: { resetExtent: boolean }) => void;
...
export class TimeChart extends RTComp<TimeChartProps, Record<string, never>, TimeChartD> {
  ref?: HTMLDivElement;
  ...
  render(){
    ...
    return (
      <div ref={e => { if(e) this.ref = e; }}
        ...
        className={classOverride("charts-comp flex-col f-1 h-fit min-h-0 min-w-0 relative  noselect ", className)} 
        onPointerMove={onHover}
        onPointerUp={e => {
          if(this.tooltips?.snapped_x !== undefined && this.data && (Date.now() - this.lastExtentChange) > 500){
            onClick?.({ 
              dateMillis: this.data.xScale.copy().invert(this.tooltips.snapped_x),
              isMinDate: this.data.xScale.range()[0] === this.tooltips.snapped_x
            })
          }
          onHover(e);
        }}
