...
export const renderInterval = (v: AnyObject = {}, shortVersion = false, withAgoText?: boolean, excludeMillis = false) => {
  let isNegative = false;

  const startIndex = SHORT_NAMES.findIndex(([name]) => Number.isFinite(v[name]) && v[name] !== 0)
  // const endIndex = SHORT_NAMES.slice(0).reverse().find(([name]) => Number.isFinite(v[name]) && v[name] !== 0)
  const intervalText = SHORT_NAMES.map(([name, shortName], i) => {
    let val = v[name];
    if(i >= startIndex && Number.isFinite(val) && (!excludeMillis || name !== "milliseconds")){
      isNegative = val < 0;
