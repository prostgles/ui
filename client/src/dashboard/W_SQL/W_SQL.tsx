...
import type { AnyObject, SocketSQLStreamHandlers, SQLResultInfo } from "prostgles-types";

import React, { useEffect } from "react";
import Loading from "../../components/Loading";
import type { PageSize, TableColumn, TableProps } from "../../components/Table/Table";
import type { OnAddChart, Query, WindowData, WindowSyncItem } from "../Dashboard/dashboardUtils";
...
export class W_SQL extends RTComp<W_SQLProps, W_SQLState, D> {
  ...
  dataAge?: number = 0;
  autoRefresh?: any;
  onDelta = async (dp: DeltaOf<W_SQLProps>, ds: DeltaOf<W_SQLState>, dd: DeltaOf<D>) => {
        ...
        <div className="SQLHotkeysWrapper min-s-0 bg-color-0 p-1 rounded max-s-fit flex-col gap-1">
          ...
          <Btn
            ...
            onClick={async () => { 
              ...
              await dbs.users.update({ id: user.id }, { options: { $merge: [{ viewedSQLTips: true }] } });
            }} 
          >
            Ok, don't show again
          </Btn>
      ...
      >
        ...
        >
          ...
          {this.d.w && <W_SQLBottomBar 
            ...
            toggleCodeEditor={() => this.setState({ hideCodeEditor: !this.state.hideCodeEditor })}
            w={this.d.w}
...
export const Counter  = ({ from, className, title }: CounterProps) => {
  ...
  return (
    <div title={title} className={"text-2 " + className}>{[minutes, seconds].map(v => `${v}`.padStart(2, "0")).join(":")}</div>
  )
