...
import { isDateCol, isGeoCol, type ChartColumn, type ColInfo } from "../W_Table/TableMenu/getChartCols";
...
export const getChartableSQL = async ({ text, tokens, ftoken, blockStartOffset }: Pick<CodeBlock, "text" | "tokens" | "blockStartOffset" | "ftoken">, sqlHandler: SQLHandler): Promise<ChartableSQL> => {
  ...
  const { dateCols, geoCols, columns } = await getChartColsFromSql(sql, sqlHandler);
   ...
   */
  if(ftoken?.textLC !== "with" || tokens.some(t => t.textLC === "returning")) {
    return emptyResult
  }

  const firstSelectIndex = tokens.findIndex(t => !t.nestingId && t.textLC === "select");
  const firstSelectToken = tokens[firstSelectIndex]!;
  const withStatement = text.slice(0, firstSelectToken.offset - blockStartOffset).trim();
  const lastSelectStatement = cleanSql(text.slice(firstSelectToken.offset - blockStartOffset));
  ...
  if(newCols.columns.length !== columns.length || 
    newCols.columns.map(c => c.name + c.udt_name).sort().join() !== columns.map(c => c.name + c.udt_name).sort().join()
  ) {
    return emptyResult
  }

  return { text, withStatement, sql: lastSelectStatement, dateCols, geoCols, columns };
}
