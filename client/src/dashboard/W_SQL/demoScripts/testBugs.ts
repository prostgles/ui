...
export const testBugs: DemoScript = async ({ typeAuto, fromBeginning, testResult, getEditor, moveCursor, newLine, triggerSuggest, acceptSelectedSuggestion, actions, runDbSQL, runSQL }) => {
  ...
  await runDbSQL(`CREATE TABLE IF NOT EXISTS my_table (id1 serial PRIMARY KEY, name1 text);`);
  const publicSchemaPrefix = "SELECT * FROM public.my_table WHERE";
  ...
  const publicSchemaPrefixIndex = "CREATE INDEX ON public.my_table (  )";
  ...
  await runDbSQL(`DROP TABLE  my_table;`);
