...
export const loadPSQLLanguage = async (loadedSuggestions: LoadedSuggestions | undefined) => {
	...
	m.languages.getLanguages().forEach((lang) => {
		if (["sql"].includes(lang.id) && "loader" in lang) {
			...
			lang.loader = async () => {
				...
				langModule.language.operators = Array.from(new Set([...operators, ...langModule.language.operators]))

				/** Remove rule that matches $ to number */
				//@ts-ignore
				langModule.language.tokenizer.numbers = langModule.language.tokenizer.numbers?.filter(([ruleRegex], i) => !("$").match(ruleRegex));


				// langModule.language.keywords = Array.from(new Set([...STARTING_KEYWORDS, ...langModule.language.keywords]));
				const dataTypes = loadedSuggestions?.suggestions
					.flatMap(s => s.dataTypeInfo && [s.dataTypeInfo.name, s.dataTypeInfo.udt_name])
					.filter(isDefined) ?? [];
				langModule.language.keywords = Array.from(new Set([...STARTING_KEYWORDS, ...keywords, ...dataTypes]));
				langModule.language.builtinFunctions = loadedSuggestions?.suggestions
					.filter(s => s.type === "function" && s.schema === "pg_catalog" && !dataTypes.includes(s.name))
					.map(s => s.name)
				...
				langModule.language.tokenizer.strings!.push([new RegExp("\\$\\$[^$]*\\$\\$"), "string"]);
				return langModule;
			};
		}
	});
