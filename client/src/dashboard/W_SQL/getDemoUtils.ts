...
export const getDemoUtils = (w: Pick<WindowSyncItem<"sql">, "id">) => {
  ...
  const getEditors = () => {
    const editors = document.querySelectorAll<HTMLDivElement>(`[data-box-id=${JSON.stringify(w.id)}] .ProstglesSQL`);
  ...
  const triggerParamHints = () => getEditor().e.trigger("demo", "editor.action.triggerParameterHints", {});
  const acceptSelectedSuggestion = () => getEditor().e.trigger("demo", "acceptSelectedSuggestion", {});
  ...
  const sqlAction = async (type: "kill-query" | "stop-listen" | "run") => {
    ...
    const button = getEditor().editor.querySelector<HTMLButtonElement>(getCommandElemSelector(selector)); 
  ...
  const testResult = (expected: string, editorValue?: string): void => {
    ...
    const actual = editorValue ?? model?.getValue()?.replaceAll(model.getEOL(), "\n") ?? "";
    if(!expected){
