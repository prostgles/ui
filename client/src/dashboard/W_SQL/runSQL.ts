...
export async function runSQL(this: W_SQL, sort: ColumnSort[] = []) {
  ...
  try {
    ...
    if (isSelect && sort.length) {
      ...
      try {
        ...
        const orderBy = sort
          ...
          .map(s => `${(s.key as number) + 1}` + ([1, true].includes(s.asc as any) ? " ASC " : " DESC "))
          .join(", ") || " TRUE::BOOLEAN ";
    ...
    const handler = await stream.start(async packet => {
      ...
      const defaultRunningQuery = { trimmedSql: "", state: "running", started: new Date(), hashedSQL: "" };
      const { trimmedSql, hashedSQL } = runningQuery ?? defaultRunningQuery
      if(packet.type === "error"){
        this.state.handler?.stop();
        const sqlError = await parseSQLError.bind(this)({ sql: trimmedSql, err: packet.error, trimmedSql });
        this.setState({ 
          handler: undefined,
          queryEnded: Date.now(), 
          activeQuery: {
        ...
        if(fields || packet.ended){
          ...
          if(packet.info?.command){
            ...
            if(isSelect !== commandResultIsSelect){
              ...
              w.$update({ options: { lastSQL: isSelect ? trimmedSql : "" } }, { deepMerge: true });
            }
          }

          if(fields){
            cols = parseSqlResultCols.bind(this)({ fields, isSelect, rows: packet.rows, sql, trimmedSql })
          }
          ...
          if(packet.ended){
            ...
            const queriesNotPickedUpBySchemaWatchEventTrigger = [
              "create database ", "drop database ", "alter database ", "drop database if exists ",
              "create user ", "drop user ", "alter user ", "drop user if exists ", "create user if exists ", "alter user if exists "
            ];
            if(queriesNotPickedUpBySchemaWatchEventTrigger.some(query => trimmedSql.toLowerCase().replace(/\s\s+/g, " ").includes(query))){
