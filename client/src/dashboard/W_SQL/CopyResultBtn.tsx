...
export const CopyResultBtn = (props: { queryEnded: boolean; sql: SQLHandler; rows: any[][]; cols: Pick<ValidatedColumnInfo, "name" | "tsDataType" | "udt_name">[] }) => {
  ...
  const res = usePromise(async () => {
    ...
    try {
      ...
      const csv = await getSqlRowsAsCSV(rows, cols.map(c => c.name));
      ...
      const outputs: Outputs = [
        ...
        (jsonArray? { val: jsonArray,  label: "Copy as JSON Array", iconPath: mdiCodeJson, fileName: "result.json" } : undefined),
...
const getStringifiedObjects = (values: any[], cols: Pick<ValidatedColumnInfo, "name" | "tsDataType" | "udt_name">[]) => {
