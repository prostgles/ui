...
export const language = {
	...
	tokenizer: {
		root: [
			...
			[/[<>=!%&+\-*/|~^]/, 'operator']
		] satisfies monaco.languages.IMonarchLanguageRule[],
		whitespace: [[/\s+/, 'white']],
		comments: [
			[/--+.*/, 'comment'],
...
export const loadPSQLLanguage = (m: typeof monaco) => {


  m.languages.getLanguages().forEach((lang) => {
    if (["sql"].includes(lang.id) && "loader" in lang) {
      const oldLoader: () => Promise<{ language: monaco.languages.IMonarchLanguage }> = lang.loader as any;
      lang.loader = async () => {
