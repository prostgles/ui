...
export const parseSqlResultCols = function (this: W_SQL, { fields, isSelect, rows, sql, trimmedSql }: { rows: any[]; fields: any[]; isSelect: boolean; trimmedSql: string; sql: string; }) {
  ...
  const keyedRows = rows.map(r => r.reduce((a, v, i) => ({ ...a, [i]: v }), {}));
  const colsWithWidth = !_cols?.length ? [] : getColWidth(_cols, keyedRows, "idx", this.ref?.getBoundingClientRect().width);
  const cols = colsWithWidth;
  w.$update({ options: { sqlResultCols: cols, lastSQL: isSelect ? trimmedSql : "" } }, { deepMerge: true });
  ...
  if (geoCols.length) {
    this.props.myLinks.forEach(l => {
      if (!l.closed && !l.deleted && l.options.type === "map") {
        const newCols = l.options.columns.slice(0)
...
export const getFieldsWithActions = (fields: any[], isSelect: boolean) => fields.map((f, idx) => ({
  ...f,
  idx,
  key: f.name,
  subLabel: f.dataType,
