...
import { clickCommand } from "./DemoSQL";
...
export const DemoAccessControl = async () => {
  ...
  const reactHandlerKey = Object.keys(input ?? {}).find(v => v.includes("__reactEventHandlers"));
  ...
  input.closest(".SearchList")?.querySelector<HTMLInputElement>(".SearchList_Suggestions #targetUserGroups")?.click();
  await clickCommand("config.ac.edit.user.select.done");
  document.querySelector<HTMLButtonElement>(`${getCommandElemSelector("config.ac.edit.type")} [value='Custom']`)?.click();
  document.querySelector<HTMLDivElement>(`${getCommandElemSelector("config.ac.edit.dataAccess")}`)?.scrollIntoView();
