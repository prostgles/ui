...
type SyncWindow =  WindowSyncItem<"table"> | WindowSyncItem<"map"> | WindowSyncItem<"sql"> | WindowSyncItem<"timechart">

export type CrossFilters = { crossFilters: AnyObject[]; activeRowFilter?: AnyObject; all: AnyObject[] }
type CrossFilterWindow = Pick<WindowSyncItem<"table">, "id" | "table_name" | "type"> | Pick<WindowSyncItem<"map">, "id" | "table_name" | "type"> | Pick<WindowSyncItem<"timechart">, "id" | "table_name" | "type">
...
export const getJoinFilters = (
  ...
  const myLinks: GetJoinFiltersResult[] = links.map(l => {
    const lwids = [l.w1_id, l.w2_id];
    const lws = windows.filter(w => !w.closed && !w.deleted && lwids.includes(w.id));
    const otherW = lws.find(lw => lw.id !== w.id);
    const getTableFilters = (table: WindowData<"table">) => table.filter?.map(f => getFinalFilter(f)).filter(isDefined) ?? []
    if(otherW && lws.length === 2 && lwids.includes(w.id) && !previousWids.some(pw => lwids.includes(pw))){
      const activeRowFilter = otherW.id === activeRow?.window_id? activeRow.row_filter : undefined;
      const getChartFilters = (chartFilters: AnyObject[], reverseToTable: string | undefined) => {
        if(l.options.type === "map" || l.options.type === "timechart"){
          const { joinPath } = l.options;
          if (joinPath?.length || previousPath.length) {
            const parsedPath = !joinPath? [] : reverseToTable? reverseParsedPath(joinPath.slice(0), reverseToTable) : joinPath.slice(0);
            const f = { 
              $existsJoined: { 
                path: [
                  ...previousPath,
      ...
      if (w.type === "table" && w.type !== otherW.type && otherW.type === l.options.type) {
        ...
        return {
          ...
          activeRowFilter: activeRowFilter? getChartFilters([activeRowFilter], undefined).f : undefined,
        }
      ...
      /** Table to table */
        ...
        const currentParsedPath = isLTR? tablePath.slice(0) : reverseParsedPath(tablePath.slice(0), otherW.table_name);
        ...
        return {
          ...
          activeRowFilter: otherW.id === activeRow?.window_id? {
              $existsJoined: {
                path: parsedPath,
      ...
      } else if (otherW.type === "table" && l.options.type === w.type && w.type !== "table") {
        return {
          l,
          w: otherW,
          ...getChartFilters(getTableFilters(otherW), otherW.table_name),
          // activeRowFilter,
          activeRowFilter: activeRowFilter? getChartFilters([activeRowFilter], otherW.table_name).f : undefined,
        }
      } else {
  ...
  const nextLinks: typeof myLinks = 
    myLinks.flatMap(d => {
      if(d.w.type !== "table"){
        return [];
      }
      return getJoinFilters(d.w, activeRow, links, _windows, [...previousWids, w.id], [...previousPath, ...d.parsedPath])
    });
