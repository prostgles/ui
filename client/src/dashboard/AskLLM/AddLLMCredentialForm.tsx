...
export const AddLLMCredentialForm = ({ dbs }: P) => {
  ...
  const [providerName, setProviderName] = React.useState<typeof providers[number]["name"]>("OpenAI");
  ...
  const models = usePromise(async () => {
    ...
    if(providerName === "OpenAI"){
      ...
      const res = models.filter(m => m.id.startsWith("gpt-") || m.id.startsWith("o-"));
      const defaultModel = res.find(m => m.id === "gpt-4o-2024-11-20")?.id ?? res[0]?.id;
      if(defaultModel){
  ...
  const disabledInfo = provider?.name === "Custom"? (
  ...
  return <PopupMenu 
    ...
    onClickClose={false}
    button={
      <Btn variant="filled" color="action" iconPath={mdiPlus}>Add new credential</Btn>
    }
