...
export const useLLMSchemaStr = ({ db, connection, tables }: Prgl) => {
  const tableConstraints = usePromise(async () => {
    ...
    const schemas = Object.entries(connection.db_schema_filter || { public: 1 }).filter(([k, v]) => v).map(([k, v]) => k);
  ...
  const schemaStr = useMemoDeep(() => {
    const res = tables.map(t => {
      const constraints = tableConstraints?.filter(c => c.table_name === t.name) ?? [];
      return `CREATE TABLE ${t.name} (\n${
        t.columns.map(c => {
          return [
            ...
            c.references? c.references.map(r => `REFERENCES ${r.ftable} (${r.fcols})`) : "",
          ].filter(v=>v).join(" ")
        }).concat(
          constraints.map(con => con.definition)
        ).join(",\n ")
      }\n)`
    }).join(";\n");
