...
export const loadGeneratedWorkspaces = async (basicWorkspaces: WorkspaceInsertModel[], { dbs, connectionId }: Pick<Prgl, "dbs" | "connectionId">) => {
  ...
  const wspToWindow: { wspIndex: number; wIndex: number; insertedWindowId: string }[] = [];

  /** Add links for charts */
  await Promise.all(basicWorkspaces.map(async (wsp, i) => {
    await Promise.all(wsp.windows.map(async (w, wIndex) => {
      const insertedWorkspace = insertedWorkspaces[i];
      const insertedWindows = (insertedWorkspace as any).windows as DBSSchema["windows"][] | DBSSchema["windows"];
      // TODO fix bug where a single inserted window is not an array but an object
      const insertedWindow = isObject(insertedWindows) && !wIndex ? insertedWindows : insertedWindows[wIndex];
      if ((w.type === "map" || w.type === "timechart") && insertedWorkspace) {
        const insertedChart: DBSSchema["windows"] | undefined = insertedWindow;
        if (insertedChart) {
          ...
          wspToWindow.push({ wspIndex: i, wIndex, insertedWindowId: chartParentWindow.id });
          await dbs.windows.update({ id: insertedChart.id }, { parent_window_id: chartParentWindow.id });
          await dbs.links.insert({
            ...
            options: w.type === "map"? {
              ...
              "columns": [
                {
                  "name": w.date_column,
                  statType: w.y_axis === "count(*)" ? undefined : {
        ...
        wspToWindow.push({ wspIndex: i, wIndex, insertedWindowId: insertedWindow.id });
      }
    }));
  }));
  ...
  await Promise.all(insertedWorkspaces.map(async (wsp, i) => {
    ...
    const fixIds = (layout: any) => {
      if ("items" in layout) {
        ...
        const insertedWindowId = wspToWindow.find((w) => w.wspIndex === i && w.wIndex === viewIndex)?.insertedWindowId;
        layout.id = isDefined(insertedWindowId) ? insertedWindowId : layout.id;
      }
    }
    fixIds(layout);
