...
export const PALETTE = {
  c1: { get: (opacity = 1, target: "deck" | "css" = "css") => { const v = [0,   129, 167, target === "css"? opacity : Math.round(opacity * 255)];  return (target === "deck")? v : `rgba(${v.join(", ")})` as any} },
  c2: { get: (opacity = 1, target: "deck" | "css" = "css") => { const v = [240, 113, 103, target === "css"? opacity : Math.round(opacity * 255)];  return (target === "deck")? v : `rgba(${v.join(", ")})` as any} },
  c3: { get: (opacity = 1, target: "deck" | "css" = "css") => { const v = [58,  134, 255, target === "css"? opacity : Math.round(opacity * 255)];  return (target === "deck")? v : `rgba(${v.join(", ")})` as any} },
...
export type ChartOptions<CType extends ChartType = "table"> = 
  ...
  CType extends "timechart"? {    
    ...
    sqlResultCols?: (Pick<ValidatedColumnInfo, "tsDataType" | "udt_name" | "name"> & {
      idx: number;
      key: string;
      // label: string;
      subLabel: string;
...
export const isMethod = (w?: SyncDataItem<WindowData> | WindowData): w is WindowSyncItem<"method"> => {
  return w?.type === "method";
}
export const IsSQL = (w: any): w is SyncDataItem<WindowData<"sql">> => {
  return w.type === "sql";
}
export const IsTimeChart = (w?: any): w is SyncDataItem<WindowData<"timechart">> => {
...
export const windowIs = <T extends ChartType>(w: WindowData, type: T ): w is WindowData<T> => {
  return w.type === type;
}
...
export type WindowSyncItem<T extends ChartType = ChartType> = ChartsObjOfUnion<T>;// SyncDataItem<Required<WindowData<T>>, true>;
