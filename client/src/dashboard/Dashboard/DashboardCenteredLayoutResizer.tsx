...
export const DashboardCenteredLayoutResizer = () => {
  ...
  const updateCenteredLayoutWidth = useCallback(debounce((newWidth: number) => {
    localSettings.$set({
      centeredLayout: {
  ...
  return <Pan key={"wsp-centered-resize"}
    style={{
      height: "100%",
      width: "25px",
      cursor: "ew-resize",
    ...
    onPan={e => {
      ...
      const dashboardContent = dashboardNode?.querySelector<HTMLDivElement>(".ViewRenderer");
      if(dashboardNode && dashboardContent){
        const newCenteredWidth = Math.max(200, window.innerWidth - 2 * e.x);
        dashboardNode.style.setProperty(CENTERED_WIDTH_CSS_VAR, `${newCenteredWidth}px`);
