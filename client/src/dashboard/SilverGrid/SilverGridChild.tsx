...
export type SilverGridChildProps = {
  ...
  getRoot: () => HTMLElement;
  onClose?: (childKey: string, e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => void | Promise<any>;
  moveTo: (sourceId: string, targetId: string, parentType: "row" | "col" | "tab", insertBefore: boolean) => void;
  minimize?: {
    value: boolean;
    toggle: VoidFunction;
  };
...
export class SilverGridChild extends RTComp<SilverGridChildProps, SilverGridChildState, any> {
  ...
  onDelta(deltaP: Partial<SilverGridChildProps> | undefined, deltaS: Partial<SilverGridChildState> | undefined, deltaD: any): void {
    ...
    const headerRef = this.refHeader || this.ref.querySelector<HTMLDivElement>("." + header?.move);
    if(!headerRef) {
    ...
    setPan(headerRef, {
      ...
      onPan: (p) => {
        ...
        const boxes = Array.from(
            container.querySelectorAll("[data-box-type]")
          )
          .map(node => {
            const br = node.getBoundingClientRect();

            let hr, tabDist = Infinity;

            if(node.classList.contains("silver-grid-item")){
              hr = node.querySelector(":scope > .silver-grid-item-header")?.getBoundingClientRect();
              tabDist = Math.hypot(x - (hr.x + hr.width/2), y - (hr.y + hr.height/2));
            }
              
            const leftDist = Math.hypot(x - (br.x + 0.25 * br.width), y - (br.y + 0.5 * br.height)),
              rightDist = Math.hypot(x - (br.x + 0.75 * br.width), y - (br.y + 0.5 * br.height)),
              topDist = Math.hypot(x - (br.x + 0.5 * br.width), y - (br.y + 0.25 * br.height)),
              bottomDist = Math.hypot(x - (br.x + 0.5 * br.width), y - (br.y + 0.75 * br.height));
            ...
            return {
              ...
              topDist,
              bottomDist,
              minDist: Math.min(tabDist, leftDist, rightDist, topDist, bottomDist)
            }
          })
        ...
        if(
          ...
          _x -2 >= thisR.x - parentRect.x && _x + _w + 2 <= thisR.x - parentRect.x + thisR.width &&
          _y -2 >= thisR.y - parentRect.y && _y + _h + 2 <= thisR.y - parentRect.y + thisR.height
        ) return;
  ...
  get siblings() {
    ...
    return Array.from(getRoot().querySelectorAll<HTMLDivElement>('[data-box-type="item"]'));
  }
