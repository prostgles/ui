...
const getClickSelector = (path: ClickedNode[]) => {
  return path.slice(0).reverse().map(n => n.value? `[data-${n.type}=${JSON.stringify(n.value)}]` : n.type).join(" ");
}
...
const getClickPath = (el: HTMLElement | Element | EventTarget | null) => {
  ...
  const currAction = getClosestActionElement(closestButton ?? closestAnchor ?? closestListItem ?? el, true);
  const path: ClickedNode[] = [currAction ?? {
    type: el.nodeName,
    node: el,
  }];
...
const getClosestActionElement = (eventTarget: HTMLElement | Element | EventTarget | undefined, onlySelf = false): ClickedNode | undefined => {
  ...
  DATA_TEST_SELECTORS.find(selector => {
    ...
    if(closest && (!onlySelf || closest === el)){
      actionNode ??= { type: selector, node: closest, value: (closest as HTMLElement).getAttribute(attributeName) ?? "" };
      return actionNode
    }
    return undefined;
  });
