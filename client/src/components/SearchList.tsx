...
export type SearchListItem = TestSelectors & {
  ...
  checked?: boolean;
  selected?: boolean;
  onPress?: (e: React.MouseEvent<HTMLLIElement, globalThis.MouseEvent> | React.KeyboardEvent<HTMLLIElement>, term?: string) => void;
...
export type SearchListProps<M extends boolean = false> = TestSelectors & {
  ...
  onSearchItems?: (
    ...
    onPartialResult?: (searchItems: SearchListItem[], finished: boolean, cancel: Function) => any
   ...
   */
  onMultiToggle?: (items: SearchListItem[], e: React.ChangeEvent<HTMLInputElement>) => void;
  inputEl?: HTMLElement;
...
const getValueAsText = v => (["boolean", "number"].includes(typeof v)? v.toString() : v);


export default class SearchList<M extends boolean = false> extends RTComp<SearchListProps<M>, SearchListState> {
  ...
  static getMatch = (args: { ranking?: number; term: string, text: string, key?: any, subLabel?: string; matchCase?: boolean; style?: React.CSSProperties; subLabelStyle?: React.CSSProperties; rootStyle?: React.CSSProperties; }): { node: React.ReactNode; rank: number } => {
    ...
    const getNode = (r: { ranking?: number; term: string, text: string, key?: any, isSublabel?: boolean, matchCase?: boolean; style?: React.CSSProperties; }) => {
      ...
      let rank = 0,
        label = text || "";
      const noTermLabel = isSublabel? label.split("\n").slice(0, 3).join("\n") : label;
      let node = <span className={rootColorClass + ` text-ellipsis`} style={rootStyle}>{noTermLabel}</span>;
      if(term){
        ...
        if(idx > -1){
          ...
          if(isSublabel){
            ...
            const matchingLineIdx = lines.findIndex(l => matchCase? l.includes(term) : l.toLowerCase().includes(term.toLowerCase()) );
            label = lines[matchingLineIdx] ?? "";
            idx = matchCase? label.indexOf(term) : label.toLowerCase().indexOf(term.toLowerCase()) ;
    ...
    const { term, text, key, subLabel, matchCase, style, subLabelStyle, rootStyle, ranking } = args;
    const node1 = getNode({ term, text, key, matchCase, style, ranking });

    const result = node1;
    if(subLabel){
      const node2 = getNode({ term, text: subLabel, key: subLabel, isSublabel: true, matchCase, style: subLabelStyle, ranking: 1 });
      result.node = (
  ...
  onMount(){
    const { autoFocus = false, defaultSearch, defaultValue: df, id = generateUniqueID() } = this.props;
    if(autoFocus){
  ...
  onClick: EventListenerOrEventListenerObject = (e) => {
    ...
    if(variant?.startsWith("search") && this.refList && e.target){
      if(!searchClosed && !this.refList.contains(e.target as Node) && !this.refInput?.contains(e.target as Node)){
        this.setState({ searchClosed: true });
        this.cancelCurrentSearch?.();
      } else if(searchClosed && this.refInput && this.refInput.contains(e.target as Node)){
        if(term && onSearchItems && dataSignature !== this.props.dataSignature){
  ...
  onSetTerm = (searchTerm, e?: any) => {
    ...
    onType?.(searchTerm, newTerm => { searchTerm = newTerm })
        ...
        timeout: setTimeout(async () => {
          ...
          try {
            ...
            this.props.onSearchItems?.(term, { matchCase }, (searchItems, finished, cancel) => {
              ...
              if(term === this.searching?.term){
                this.setState({ searchItems, searchClosed: false, error: undefined, dataSignature, term });
                if(finished){
                  this.cancelCurrentSearch();
                }
              } else if(!this.searching && this.state.searchingItems) {
  ...
  render(){
    ...
    const getFullItem = (d: SearchListItem): ParsedListItem => {
      const match = SearchList.getMatch({ 
          matchCase, 
          ranking: typeof d.ranking === "function"? d.ranking(searchTerm) : d.ranking,
          term: searchTerm, 
          style: d.styles?.label,
          subLabelStyle: { ...styles?.subLabel, ...d.styles?.subLabel, },
          rootStyle: { ...styles?.labelRootWrapperStyle, ...d.styles?.labelRootWrapperStyle},
    ...
    const renderedItems: ParsedListItem[] = onSearchItems? searchItems : 
        dontHighlight? items :
        onSearch? items.map(getFullItem) : 
          items.map(getFullItem)
            .filter((d, i)=> d.rank !== Infinity )
            .sort((a, b) => !searchTerm? 0 : (
                ((a.rank ?? 0) - (b.rank ?? 0)) || 
    ...
    const renderedSelected = renderedItems.filter(d => !d.disabledInfo && d.checked);
    const allSelected = items.filter(d => d.checked);
    const noSearch = !onSearchItems && items.length < noSearchLimit && !searchTerm;
          ...
          {(onSearch && !this.props.items)? null : 
            ...
            renderedItems
              .map((d, i)=> { 
                ...
                const onPress: SearchListItem["onPress"] = (!d.onPress || d.disabledInfo)? undefined : e => {
                ...
                if("content" in d){
                  ...
                  rowContent = <div className="ROWINNER flex-row ai-center f-1 " style={d.styles?.rowInner}>
                    ...
                    {contentRight || null}
                    {
                      (typeof d.checked === "boolean")? <Checkbox id={id} className="f-0 no-pointer-events" checked={d.checked} style={{ marginRight: "12px" }} onChange={()=>{}} /> : null
                    }
                  </div>
                }
    ...
    const content = (
      <div className={"SearchList list-comp ta-left flex-col min-h-0 " + className}
        ...
        onKeyDown={e => {
          ...
          const lastChild = this.refList.lastChild as HTMLLIElement | null,
            ...
            previousElementSibling = activeElement.previousElementSibling as HTMLElement | null,
            nextElementSibling = activeElement.nextElementSibling as HTMLElement | null,
          ...
          if(!listNotFocused && e.key.length === 1 && !e.shiftKey && !e.ctrlKey && !e.altKey){
            this.focusInput();
            return
          }
          ...
          switch (e.key) {
            case "ArrowUp": 
              ...
              e.preventDefault();
              this.focusedRowIndex = !activeElement.parentElement?.children? -1 : Array.from(activeElement.parentElement.children).indexOf(activeElement);
            break;
            case "ArrowDown":
      ...
      >
        ...
        >
          {!hasSearch? 
            (multiSelect? <div className="pl-1 py-p5 noselect text-1p5">{allSelected.length || 0} selected</div> : null) : 
            <div
              ref={inputWrapper => {
                if(inputWrapper){
            ...
            <Checkbox 
              ...
              onChange={e => {
                ...
                const newItems = items.map(d => {
                  ...
                  const filteredItem = !searchTerm? d : renderedItems.find(_d => _d.key === d.key);
                  return {
                    ...d, 
                    checked: filteredItem? (d.disabledInfo? d.checked : checked) : d.checked
                  }
                });
