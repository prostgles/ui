...
export default class Checkbox extends React.Component<P, any> {
  ...
  render(){
    ...
    const checkbox = (
      ...
      >
        <input id={id}
          ...
          type="checkbox"
          checked={checked}
          onChange={!onChange? undefined : e => { onChange(e, e.target.checked) }}
          readOnly={readOnly}
        />
        <Icon path={mdiCheckBold} 
          size={variant === "button"? 1 : variant === "minimal"? 1 : 0.75}
          className={tickClass}
          style={tickStyle}
        />
      </div>
    ...
    return <label 
      ...
      htmlFor={id}
      className={classOverride("Checkbox flex-row-wrap ai-center pointer w-fit h-fit", className)} 
      title={title}
      {...testSel}
    >
      {checkbox}
