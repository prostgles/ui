        ...
        {cols.map((col, iCol) => {
          ...
          return (
            <div key={iCol} 
              ...
              onContextMenu={!col.onContextMenu? undefined : e => {
                if(col.onContextMenu && !e.currentTarget.classList.contains("resizing-ew")){
                  vibrateFeedback(25)
                  return col.onContextMenu(e, (e.nativeEvent.target as HTMLElement), col, popup => this.setState({ popup }));
                }
              }} 
              role="columnheader" 
              ...
              onDragOver={!onColumnReorder? undefined : e => {
                if(draggedCol && draggedCol.idx !== iCol && draggedCol.targetIdx !== iCol){
                  setDraggedCol({ ...draggedCol, targetIdx: iCol });
                }
                e.preventDefault() ;
                return false;
              }}
              onDragLeave={!onColumnReorder? undefined : e => {
                    
                if(e.target === e.currentTarget){
                  setDraggedCol({ ...draggedCol!, targetIdx: undefined });
                }
              }}
              onDragEnd={!onColumnReorder? undefined : e => {
                setDraggedCol(undefined);
              }}
              onDrop={!onColumnReorder? undefined : e => {
                ...
                if(draggedCol && droppedOnOtherColumn){
                  ...
                  if(!sourceCol) return;
                  newCols = newCols.filter(c => c.key !== sourceCol.key);
                  const targetIdx = newCols.findIndex(c => c.key === targetCol.key);
                  newCols.splice(targetIdx + (draggedCol.idx < (targetIdx + 1)? 1 : 0), 0, sourceCol)

                  onColumnReorder(newCols)
                }
                setDraggedCol(undefined);
              }}
              onClick={(!onSort || !col.sortable)? undefined : (e) => {
                if(e.button || (e.target as HTMLDivElement).classList.contains("resize-handle")) return;
                ...
                onSort((e.shiftKey? sort.filter(s => s.key !== col.key && s.key !== mySort?.key) : []).concat(newSort? [newSort] : []));
              }}
            >
              <div className={
                "flex-row fs-1 h-fit " +
                (!col.sortable? "" : (typeof mySort?.asc !== "boolean")? " sort-none " : [false].includes(mySort.asc as any)? "sort-desc" : "sort-asc") +
                (col.headerClassname || "")
                }
              >
                <div  className="flex-col min-w-0 f-shrink">
                  <div 
                    title={col.title ?? (typeof col.label === "string"? col.label : undefined)}
              ...
              {(iCol <= cols.length - 1)? 
                <Pan 
                  ...
                  onPan={(opts, ev) => {
                    ...
                    if(colh){
                      const { xDiff } = opts;
                      const w_px = Math.max(30, (opts.node as any)._w + xDiff);
                      colh.style.width = `${w_px}px`;
                      colh.style.flex = "none";

                      const tableBody = this.props.rootRef?.querySelector(`[role="rowgroup"]`);
                      const rowCols = tableBody?.querySelectorAll<HTMLElement>(`[role="row"] > *:nth-child(${iCol + 1})`);
                      rowCols?.forEach(rc => {
                  ...
                  onPress={ev => {
                    const colh = (ev.target as HTMLElement).closest(`[role="columnheader"]`);
                    if(colh){
...
export const getDraggedTableColStyle = (col: ProstglesColumn, iCol?: number, draggedCol?: TableState["draggedCol"]): React.CSSProperties => {
