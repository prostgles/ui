...
export type TableColumn = {
  ...
  onClick?: (row: AnyObject, col: TableColumn, e: React.MouseEvent) => void | any;
  ...
  getCellStyle?: (row: AnyObject, value: any, renderedVal: any) => React.CSSProperties;
  onContextMenu?: (e: React.MouseEvent, n: HTMLElement, col: TableColumn, setPopup: ((popup: TableHeaderState["popup"]) => void)) => boolean;
  width?: number;
  flex?: number;
}; 
...
export type TableProps = {
  ...
  onRowClick?: (row: AnyObject | undefined, e: React.MouseEvent<HTMLDivElement>) => any;
  rounded?: boolean;
  ...
  return (
    ...
    >
      <div role="table" className={"min-w-fit min-h-0 b b-default flex-col h-full"} style={tableStyle}>
        ...
        <div 
          ref={scrollBodyRef}
          className={classOverride("b-y b-default f-1 oy-auto ox-hidden flex-col min-w-fit " + (rawPagination === "virtual"? "" : "no-scroll-bar"), bodyClass)}
          role="rowgroup"
          onPointerUp={!onRowClick ? undefined : e => {
            ...
            !_rows.length ? <div className="text-3 p-2 noselect">No data</div> :
              _rows.map((row, iRow) => {
                ...
                return <div key={rowKey}
                  ...
                  onClick={!onRowClick ? undefined : e => {

                    /** Do not interrupt user from selecting text */
                    if (e.currentTarget.contains(e.target as Node) && !window.getSelection()?.toString().trim()) {
                      onRowClick(row, e);
                    }
                  }}
                >
                  {cols.map((col, i) => {
                    ...
                    const cellTextVal = col.onRender ? null : parseCell(row[col.key], maxCharsPerCell);

                    const cellClassName = classOverride(`text-sm leading-5 flex-col text-0 o-auto no-scroll-bar ta-left br b-gray-100 ` + //  ws-no-wrap  o-hidden to-ellipsis 
                    ...
                    return (
                      ...
                      >
                        {col.onClick ?
                          ...
                          :
                          (col.onRender ?
                            col.onRender({ row: row, value: row[col.key], renderedVal: cellText, rowIndex: actualRowIdx, prevRow: rows[actualRowIdx - 1], nextRow: rows[actualRowIdx + 1] })
                            :
                            cellTextVal
                          )
                        }
...
const parseCell = <FT extends boolean = false>(d: any, maxCharsPerCell = 100, getText?: FT): FT extends true ? string : React.ReactNode => {
  ...
  if (typeof d === "string") {
    ...
    txt = sliceText(`[${d.map(c => parseCell(c)).join(", \n")}]`, maxCharsPerCell);
...
export function closest<Num extends number>(v: number, arr: readonly Num[] | Num[]): Num | undefined {
  return arr.map(av => ({ av, diff: Math.abs(v - av) })).sort((a, b) => a.diff - b.diff)[0]?.av
}
export function closestIndexOf<Num extends number>(v: number, arr: readonly Num[] | Num[]): Num | undefined {
  return arr.map((av, i) => ({ i, av, diff: Math.abs(v - av) })).sort((a, b) => a.diff - b.diff)[0]?.i;
}

export const onWheelScroll = (parentClassname?: string): React.WheelEventHandler<HTMLElement> => (e: React.WheelEvent<HTMLElement>) => {
  if (e.shiftKey || e.ctrlKey || !e.currentTarget.contains(e.target as any)) return;

  const oFlowY = (el?: HTMLElement | null) => {
    return el && el.scrollWidth > el.clientWidth;
  };
  let maxDepth = 5;
  let el = !parentClassname ? e.currentTarget : e.currentTarget.parentElement?.closest<HTMLElement>("." + parentClassname);
  let sel;
  while (maxDepth > 0) {
    if (oFlowY(el)) {
      sel = el;
