...
export default class Loading extends RTComp<P, S> {
  ...
  onDelta(deltaP: Partial<P> | undefined, deltaS: Partial<{ ready: boolean; }> | undefined, deltaD: { [x: string]: any; } | undefined): void {
  ...
  render() {
    ...
    const spinner = <svg className="spinner f-0" width={size} height={size} viewBox="0 0 66 66" xmlns="http://www.w3.org/2000/svg">
      <circle className={"path " + (colorAnimation? " color-animation " : "")} fill="none" strokeWidth="6" strokeLinecap="round" cx="33" cy="33" r="30"></circle>
    </svg>

    if (variant === "top-bar") {
      return <div style={{ position: "absolute", zIndex: 2, top: 0, left: 0, right: 0, width: "100%", height: "6px", ...style, ...commonStyle }} className={"top-bar-loader " + className}></div>
    ...
    const rootStyle: React.CSSProperties = !message? { width: size, height: size } : {}

    return (
      <div 
        style={{ ...style, ...rootStyle, ...commonStyle }} 
        className={classOverride("Loading spinner-loader ws-nowrap flex-row gap-1 ai-center ", className)}
      >
        {spinner}
        {message}
      </div>
