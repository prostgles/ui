...
export const DraggableLI = <T extends any[]>({ children, onReorder, items, idx, ...props }: P<T>) => {
  ...
  type ListParent = null | HTMLElement & {
    _initialSize?: {
      width: string;
      height: string;
    };
  ...
  return <li 
    ...
    onDrag={!onReorder? undefined : e => {
      ...
      const getSize = (e: HTMLLIElement) => isRow? e.offsetWidth : e.offsetHeight
      ...
      const targetIndex = siblings.findIndex(s => {
        ...
        const horizontalOverlap = (e.clientX > r.x && e.clientX < r.x + r.width);
        const verticalOverlap = (e.clientY > r.y && e.clientY < r.y + r.height);
      ...
      siblings.map((c, siblingIdx)=> {
        ...
        if(isRow){
          ...
          if(offsetPx !== c.style.marginLeft) c.style.marginLeft = offsetPx;
        } else {
          // if(newPadding !== c.style.paddingTop) c.style.paddingTop = newPadding;
          if(offsetPx !== c.style.marginTop) c.style.marginTop = offsetPx;
        }
    ...
    onDragEnd={!onReorder? undefined : async e => {
      if(shouldStop(e)){
        return false;
      }
      ...
      setTimeout(() => {
        Array.from(p.children as unknown as HTMLElement[]).map((c: HTMLElement)=> {
          c.style.paddingTop = "";
          c.style.paddingLeft = "";
          c.style.marginLeft = "";
          c.style.marginTop = "";
