...
export default class FileInput extends RTComp<{
  ...
  */
    ...
    if(isViewerMode){
      if(!this.resizeObserver){
        ...
        this.resizeObserver = new ResizeObserver(entries => {
          const isOverflowing = this.refWrapper!.scrollWidth > this.refWrapper!.offsetWidth;
  ...
  renderMedia = (params: {
    ...
    const file: { type: string; name?: string; url: string; isLocalFile?: boolean; } = ("url" in f) ? { ...f, type: f.content_type } : {
      name: f.name,
      type: f.data.type,
    ...
    if (url) {
      ...
      if (type.startsWith("image")) {
        ...
        return <Chip key={i} value={file.isLocalFile? file.name : url} leftIcon={{ path: mdiFileOutline }} onDelete={onDelete} />
      }
    }
    ...
    >
      ...
      >
        ...
        {!onClick ? null : <div className={(isImageOrVideo? "" : " media-onclick-cover  ") + " absolute w-full h-full pointer"} style={{ zIndex: 2 }} onClick={e => {
          e.stopPropagation();
          e.preventDefault();
          onClick();
        }}></div>}
        ...
        {focused? null :!name ? <a href={url} target="_blank" className="p-5 f-0">{url}</a> : 
          <div
            ...
            style={{
              ...
              background: isImageOrVideo? "linear-gradient(to bottom, rgb(255 255 255 / 0%) 0%,#00000075 70%)" : "white",
            }}
          >{name}</div>}
      </div>
  ...
  render() {
    ...
    if (focusedFile) {
      ...
      popup = <Popup 
        ...
        onKeyDown={!isViewerMode? undefined : e => {
          if (e.key === "ArrowRight") {
            const index = (focusedFile.index < media.length - 1) ? (focusedFile.index + 1) : 0;
            this.setState({
              focusedFile: {
                file: media[index]!,
                index,
            ...
            const index = focusedFile.index ? (focusedFile.index - 1) : (media.length - 1);
            this.setState({
              focusedFile: {
                file: media[index]!,
                index,
    ...
    const showLeftChevron = Boolean(showChevrons && this.refWrapper?.scrollLeft);
    const showRightChevron = Boolean(showChevrons && 
      (!this.refWrapper?.scrollLeft ||
      this.refWrapper.scrollWidth - 20 > (this.refWrapper.scrollLeft + this.refWrapper.clientWidth))
    );

    const scrollItem = (val: 1 | -1) => {
      if(this.refWrapper){
        /** first child will be chevron */
        const mediaSize = (this.refWrapper.children[1] as HTMLDivElement).offsetWidth || minSize;
        ...
        <div className={"relative flex-col min-w-0 min-h-0 "} >
          ...
          <div className={(isViewerMode? " f-1 flex-row o-auto min-w-0 min-h-0 o-auto " : " flex-row-wrap gap-p25 ")}
            ...
            onScroll={e => {
              ...
              const w: any = window
              if(!w.__lastMediaScroll || w.__lastMediaScroll < Date.now() - 100){
                this.forceUpdate();
          ...
          >
            ...
            {showLeftChevron && <div 
              ...
              style={{ ...chevronCommonProps.style, left: 0, background: "linear-gradient( -90deg, transparent, white)" }} 
              onClick={scrollPrev}
            >
            ...
            {media.map((mediaItem, i) => {
              return this.renderMedia({
                ...
                onClick: () => { this.setState({ focusedFile: { file: mediaItem, index: i } }) },
                style: !isViewerMode? {} : {
                  width: "250px",
                  height: "250px",
                  minWidth: `${minSize}px`,
            ...
            {showRightChevron && <div 
              className={chevronCommonProps.className} 
              style={{ ...chevronCommonProps.style,  right: 0, background: "linear-gradient( 90deg, transparent, white)" }} 
              onClick={scrollNext}
            >
