...
export const ProgressBar = ({ message, value, totalValue, variant }: P) => {
  ...
  const progressBar = <div className={isBarchart? "shadow" : ""}
    style={{ 
      ...
      ...(isBarchart? {
        ...
        width: "200px",
        animation: perc > -1? undefined : "indeterminateAnimation 1s infinite linear",
      }),
    }}
  ></div>

  // const text = !status.loading?.loaded? "Preparing..." : `Uploaded ${bytesToSize(status.loading.loaded ?? 0)}`;
  return <div className="ProgressBar flex-col gap-p25">
    {progressBar}
...
export const CellBarchart = ({ min, max, value, message, barColor = MINI_BARCHART_COLOR, textColor, style, className, ...divProps }: CellBarchartProps) => { 
  
  const delta = +max - +min;
  const valDelta = +value - +min;
  const perc = (+value > +min ? Math.round(100 * valDelta/delta) : -1);
  const height = 8
  return <div { ...divProps} className={classOverride("ProgressBar flex-col gap-p25", className)} style={style}>
    ...
    ></div>
    <div className={"text-1 ta-left"} style={textColor? { color: textColor } : undefined}>{message}</div>
  </div>
}