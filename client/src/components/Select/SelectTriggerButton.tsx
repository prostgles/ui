...
type P<O extends OptionKey, Multi extends boolean = false, Optional extends boolean = false>  = Omit<SelectProps<O, Multi, Optional>, "fullOptions"> & { 
...
export const SelectTriggerButton = <O extends OptionKey, Multi extends boolean = false, Optional extends boolean = false> (props: P<O, Multi, Optional>) => {
  ...
  const noOtherOption = !options.length || options.length === 1 && value === options[0];
  ...
  const triggerButton = <Btn 
    ...
    onKeyDown={noOtherOption? undefined : e => {
      
      if(e.key.length === 1 && !popupAnchor){
        const maxBtnWidth = e.currentTarget.getBoundingClientRect().width;
        setState({ popupAnchor: e.currentTarget, fixedBtnWidth: maxBtnWidth, defaultSearch: e.key });
