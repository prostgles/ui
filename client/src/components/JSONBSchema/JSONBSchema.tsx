...
import { JSONBSchemaAllowedOptions, JSONBSchemaAllowedOptionsMatch } from "./JSONBSchemaAllowedOptions";
...
export const JSONBSchema = <S extends Schema>({ style, className = "", value, schema, onChange, setHasErrors, ...otherProps }: P<S>) => {
  ...
  if(JSONBSchemaAllowedOptionsMatch(schema)){
    node = <JSONBSchemaAllowedOptions value={localValue} schema={schema} onChange={setLocalValue} {...otherProps} />
  } else if(JSONBSchemaPrimitiveMatch(schema)){
    node = <JSONBSchemaPrimitive value={localValue} schema={schema} onChange={setLocalValue} {...otherProps} />
  } else if(JSONBSchemaOneOfTypeMatch(schema)){
    //@ts-ignore
    node = <JSONBSchemaOneOfType value={localValue as any} schema={schema as any} onChange={setLocalValue} {...otherProps} /> 
  } else if(JSONBSchemaObjectMatch(schema)){
    node = <JSONBSchemaObject value={localValue} schema={schema} onChange={setLocalValue} {...otherProps} />
  } else if(JSONBSchemaRecordMatch(schema)){
    node = <JSONBSchemaRecord value={localValue} schema={schema} onChange={setLocalValue} {...otherProps} />
  } else if(JSONBSchemaArrayMatch(schema)){
    node = <JSONBSchemaArray value={localValue} schema={schema} onChange={setLocalValue} {...otherProps} />
  } else if(JSONBSchemaLookupMatch(schema)){
    return <JSONBSchemaLookup value={localValue} schema={schema} onChange={setLocalValue} {...otherProps} />
  }

  if(node){
    const isDisabled = isObject(schema) && schema.optional && localValue === undefined;
    const styleDisabled = (!isDisabled || otherProps.isNested)? {} : { opacity: .5 };
    return <div style={{ ...style, ...styleDisabled}} className={`JSONBSchema flex-row ${className}`}>
      {node}
    </div>
  }

  return <>Schema not suitable for JSONBSchema.tsx: {JSON.stringify(schema, null, 2)}</>
}


export const JSONBSchemaA = (p: JSONBSchemaCommonProps & { schema: any; onChange: (newValue: any) => void;}) => {
