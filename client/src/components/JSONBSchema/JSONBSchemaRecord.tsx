...
export const JSONBSchemaRecordMatch = (s: JSONB.JSONBSchema): s is Schema => isObject(s.record);
export const JSONBSchemaRecord = ({ value, schema, onChange, ...oProps } : P) => {

  return <div className="JSONBSchemaRecord flex-col gap-1">
    {isObject(value) && getKeys(value).map(propName => {
      ...
      return <div key={propName} className="flex-row gap-1">
        ...
        <FormFieldDebounced 
          ...
          onChange={newPropName => {
            onChange({ ...omitKeys(value, [propName]), [newPropName]: value[propName] })
          }} 
        />
        <JSONBSchema 
