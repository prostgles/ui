...
export const JSONBSchemaObjectMatch = (s: JSONB.JSONBSchema): s is JSONB.ObjectType => isObject(s.type);
export const JSONBSchemaObject = ({ value: rawValue, schema, onChange, isNested = false, ...oProps } : P) => {

  const value = isObject(rawValue)? rawValue : undefined;
  const optionalProps = Object.entries(schema.type)
    .filter(([k, v]) => 
      isObject(v) && v.optional && value?.[k] === undefined
    )  
    .map(([k]) => k);
  ...
  if(schema.optional && value === undefined && !showOptional && isNested){
    return <div className={"JSONBSchemaObject flex-col gap-p5 as-end " + (oProps.className ?? "")} style={oProps.style}>
  ...
  const requiredPropNames = getKeys(objectSchema).filter(k => !getSchemaFromField(objectSchema[k]!).optional);
  const optionalPropNames = getKeys(objectSchema).filter(k => getSchemaFromField(objectSchema[k]!).optional);

  return <div className={"JSONBSchemaObject flex-row-wrap gap-1 " + (oProps.className ?? "")} style={oProps.style}>
    ...
    .map(propName => {
      ...
      return <div key={propName} className="flex-row gap-1">
        <JSONBSchema 
