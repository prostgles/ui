...
export const JSONBSchemaLookupMatch = (s: JSONB.JSONBSchema): s is Schema => isObject(s.lookup);
export const JSONBSchemaLookup = ({ value: rawValue, schema, onChange, db, tables, ...oProps } : P) => {
  ...
  if(lookup.type === "schema" || lookup.type === "data-def"){
    ...
    const colFilter = isObject(filter) && !isEmpty(filter) && (filter.udt_name || filter.tsDataType)? pickKeys(filter, ["tsDataType", "udt_name"], true) : undefined;
    ...
    const fullOptions = needsCol? 
      matchingTables.flatMap(t => {
          return t.columns
            .filter(c => !colFilter || areEqual(colFilter, c, getKeys(colFilter)))
            .map(c => ({ 
    ...
    const selectedValue = isObject(rawValue)? [rawValue.table, rawValue.column].join(delimiter) : rawValue;
    
    const selector = <Select label={schema.title} 
      value={selectedValue} 
      ...
      onChange={opts => {
        if(needsCol){
          const [table, column] = (typeof opts === "string")? opts.split(delimiter) : [];
            ...
            schema={{ 
              ...
              type: {
                isFullRow: {
                  title: "Full row",
                  optional: true, 
                  description: "If true then the full row object will be passed to the function",
                  type: {
                    displayColumns: { type: "string[]", optional: true, allowedValues: tableCols },
                  }, 
                },
                searchColumns: { 
                  title: "Search columns", 
                  type: "string[]",
                  description: "Columns used for searching the value. By default all columns will be used", 
                  optional: true, 
                ...
                showInRowCard: {
                  ...
                  optional: true, 
                  description: " If true then a button will be shown in the row edit card to display this action",
                  type: {
                    actionLabel: { type: "string", optional: true, title: "Button label" }
                  }
                }
              }
            } as any}  
  ...
  if(lookup.isFullRow){
    if(!isObject(rawValue)){
      ...
      defaultValue = Object.values(pickKeys(rawValue, lookup.isFullRow.displayColumns)).join(", ")
  ...
  const error = oProps.showErrors && !isCompleteJSONB(defaultValue || undefined, schema)? "Required" : undefined;
