...
type Schema = Extract<{ oneOfType: Required<JSONB.OneOf>["oneOfType"] }, JSONB.OneOf>; 
 ...
 */
export const JSONBSchemaOneOfTypeMatch = (s: JSONB.JSONBSchema): s is Schema => !!(s.oneOfType?.length);
export const JSONBSchemaOneOfType = ({ value, schema, onChange, ...oProps } : P) => {
  ...
  const commonRequiredPropertyNames = firstSchemaKeys.filter(propName => { 
    ...
    return !fPropS.optional && s.oneOfType.every(subSchema => {
      ...
      return propS && (propS.type === fPropS.type || propS.enum && fPropS.enum);
    })
  });
  ...
  const getOneOfSchemaIndex = (value: any): number => {
    return s.oneOfType.findIndex(ss => 
      isObject(value) && 
  ...
  return <div className="JSONBSchemaOneOf flex-row-wrap gap-1"> 
    {n}
  </div>
}
