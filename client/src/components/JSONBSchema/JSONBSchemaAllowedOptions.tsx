 ...
 * }
 */
const getFullOptions = (s: JSONB.JSONBSchema): { fullOptions: FullOption[]; isMulti: boolean; } | undefined => {
  if(s.allowedValues){
    return { 
    ...
    return {
      fullOptions, isMulti: false
    }
  }

  return undefined
}
export const JSONBSchemaAllowedOptionsMatch = (s: JSONB.JSONBSchema): s is JSONB.BasicType => !!getFullOptions(s);
export const JSONBSchemaAllowedOptions = ({ value, schema, onChange, showErrors } : P) => {

  const o = getFullOptions(schema);
  if(!o) {
    return <>Could not render JSONBSchemaAllowedOptions schema: {JSON.stringify(schema)}</>
  }

  const error = showErrors && !isCompleteJSONB(value, schema)? "Required" : undefined;

  return <FormField name={schema.title} 
