    ...
    if(e.key === "Enter"){
      ...
      if(!suggestions?.length){
        if(["ArrowDown"].includes(e.key) && !value && onSuggest){
          ...
          const selF = f.find(f => f.type === "n" && f.idx <= selStart && f.idx + f.len >= selStart);

          if(selF){
            this.inputSelStart = selStart;
            let removeMinusAtIndex = -1;
            const up = e.key === "ArrowUp";
            let newValue = f.map(f => {
              if(f.idx === selF.idx){
                const step = e.altKey? 0.1 : e.ctrlKey? 10 : 1;
                const incrDecimals = e.altKey? 1 : 0;
                const isNegative = value[selF.idx-1] === "-";
                const increment = (up? (isNegative? -1 : 1) : (isNegative? 1 : -1)) * step;
                const val = +(selF.val as string).split(".").map((v, i, arr) => {
                  const newVal = i < arr.length -1? v : +v + increment;
                  return newVal;
                }).join(".");
                
                let res = val.toFixed(selF.decimalPlaces || incrDecimals).padStart(selF.len, "0");
                ...
                if(selF.decimalPlaces && res.endsWith("." + new Array(selF.decimalPlaces).fill("0").join(""))){
            ...
            if(removeMinusAtIndex > -1){
              newValue = newValue.substring(0, removeMinusAtIndex) + newValue.substring(removeMinusAtIndex+1);
            }
            inpt.value = newValue;
            this.onChange(inpt);
        ...
        let newSugIdx = Number.isFinite(activeSuggestionIdx)? Math.max(0, activeSuggestionIdx + increment) : 0;
