...
export default class FormField extends React.Component<FormFieldProps, FormFieldState> {
  ...
  componentWillUnmount(){
    this.mounted = false;
    if(this.textArea) this.textArea.removeEventListener("input", this.resize, false);
  }
  componentDidUpdate(prevProps: FormFieldProps){
    ...
    if(prevProps.value !== this.props.value) {
      ...
      if(this.inputRef &&  (/text|password|search|tel|url/).test(this.inputRef.type) && this.cursorPosition !== undefined){
      ...
      timeout: setTimeout(async ()=>{
        if(this.mounted && this.changing){
          ...
          if(onSuggest){
            ...
            this.setState({
              suggestions: (suggestions.length === 1 && suggestions[0] === value)? undefined : suggestions
            });
          }
  ...
  render(): React.ReactNode {
    ...
    let extraInptStyle = {};
    if(type === "color" && value){
      wrapperStyle = { ...wrapperStyle, backgroundColor: value || "white", cursor: "pointer" };
    ...
    const inputProps: React.DetailedHTMLProps<React.InputHTMLAttributes<HTMLInputElement>, HTMLInputElement> = {
      ...
      onKeyDown: (type === "number")? e => {
        const numLockAlert = !e.getModifierState("NumLock") && e.nativeEvent.code.startsWith("Numpad") && e.key !== e.nativeEvent.code.slice(0, "Numpad".length);        //(NUMLOCK_KEYS.includes(e.key) || !e.currentTarget.value && ["ArrowRight", "ArrowLeft"].includes(e.key) )
    ...
    if(!readOnly && !disabledInfo && onChange && (nullable || optional) && !hideClearButton){
      ...
      if(nullable && optional){
        rightIcons2 = <Select 
          ...
          options={[rawValue !== null? "NULL" : undefined, rawValue !== undefined? "undefined" : undefined].filter(isDefined)} 
          onChange={v => {
            onChange(v === "NULL"? null : undefined)
          }} 
        />
      } else if(rawValue !== undefined && optional || rawValue !== null && nullable) {
        rightIcons2 = (
          <Btn 
        ...
        >
          ...
          <div className={" flex-row f-1 ai-center min-w-0 gap-p25 "}>
            ...
            >
              ...
              {(hint || error || numLockAlert) && <div className={"flex-col jc-center "}>
                {numLockAlert && <InfoRow variant="naked" className="font-10" iconSize={.75}>NumLock is off</InfoRow>}
                {error? <ErrorComponent error={error} style={{ padding: 0 }} findMsg={true} />  : null}
