...
export default class Chip extends React.Component<ChipProps> {
  ...
  render() {
    const { className = "", subValues, label, variant = "default", onDelete, leftIcon, color = "gray", ...divProps } = this.props;
    
    const asHeader = variant === "header";
    const labelNode = typeof label !== "string" ? null : <span className={"text-1 "} style={asHeader? { fontSize: "14px", fontWeight: 400 } : { fontWeight: 400 }}>{label}: </span>; 
    return <div
      {...divProps}
      style={{
        ...(!asHeader && {
      ...
      className={classOverride(`chip-component flex-row ws-pre-wrap ai-center chip lg ${color} variant-${variant}  ${asHeader? "text-ellipsis" : ""}`, className)}
    >
      {leftIcon? (leftIcon.onClick? 
        <Btn 
          iconPath={leftIcon.path} 
          onClick={leftIcon.onClick} 
          className={classOverride("mr-p5 round", leftIcon.className)} 
          size="medium" 
          style={{ padding: 0, background: "transparent", color: "black", ...leftIcon.style }} 
          variant="filled" 
        /> : 
        <Icon 
          path={leftIcon.path} 
      ...
      <FlexCol className={`gap-p25 ` }>
        ...
        {!!subValues?.length && subValues.map((subValue, i) => <span key={i} className=" f-1 text-ellipsis" style={{ opacity: .7, padding: "2px" }}>{subValue}</span>)}
      </FlexCol>
