...
export const NonHTTPSWarning = ({ dbs, auth }: Required<AppState>["prglState"]) => {
  ...
  if (
    ...
    return <InfoRow color="danger" iconPath={mdiAlertBox} className="m-p5 bg-color-0">
      ...
      {canUpdateUsers && <Btn onClickPromise={async () => {
        dbs.users.update({ id: authUser?.id }, { options: { $merge: [{ hideNonSSLWarning: true }] } });
