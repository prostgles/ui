...
const ChatWrapped:FunctionComponent<P> = (props) => {
  ...
  const [chat, setChat] = useState<ChatData>({ id: 2, other_user: "John W", avatar_url: "/worker_avatar.png" });
  // console.log(props);
      ...
      if(db && user){
        if(chat_id){
          ...
          if(!chat){
            ...
            setChat({
              id: chat_id,
              other_user: get(chat, "other_participants.0.first_name") + " " + get(chat, "other_participants.0.last_name"),
              avatar_url: "/worker_avatar.png" 
            })

            if(msgSub) await msgSub.unsubscribe();
            msgSub = (
              await db.messages.subscribe(
                { chat_id }, 
                { 
                  select: { "*": 1, media: { local_url: 1, content_type: 1, final_name: 1 } }, orderBy: { sent: 1 } 
                }, (messages: any[]) => {
                  setMessages(
                    messages.map(m => 
                      ({ 
  ...
  const onSend = async (message?: string, media?: any, mediaName?: string, mediaContentType?: string) => {
