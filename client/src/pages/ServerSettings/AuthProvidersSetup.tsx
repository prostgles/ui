...
export type AuthProvidersConfig = Extract<DBSSchema["global_settings"]["auth_providers"], { website_url: string }>;
...
export const AuthProviderSetup = ({ dbs, dbsTables }: Pick<Prgl, "dbs" | "dbsTables">) => {
  const globalSettingsTable = dbsTables.find(t => t.name === "global_settings");
  const authColumn = globalSettingsTable?.columns.find(c => c.name === "auth_providers");
  const { data: global_settings } = dbs.global_settings.useSubscribeOne();
  const { data: userTypes } = dbs.user_types.useFind();
  const updateAuth = async (auth: Partial<DBSSchema["global_settings"]["auth_providers"]>) => {
    await dbs.global_settings.update(
      {}, 
      { 
        auth_providers: !auth? undefined : {
          website_url: global_settings?.auth_providers?.website_url ?? window.location.origin,
  ...
  if(!globalSettingsTable || !authColumn){
    return <InfoRow>
      Could not find global_settings table or authColumn.
  ...
  return <FlexCol className="p-1 f-1">
    <FlexCol className="p-1 gap-2">
      ...
      <Select 
        ...
        fullOptions={userTypes?.map(ut => ({ key: ut.id, subLabel: ut.description ?? "" })) ?? []}
        onChange={async (default_user_type) => {
          if(default_user_type === "admin"){
            const result = window.confirm("Warning: You are setting the default user type to 'admin'. This means that new users will be granted the highest level of access!");
            if(!result) return;
          }
          updateAuth({
            ...auth_providers,
