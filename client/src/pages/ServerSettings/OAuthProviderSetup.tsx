...
import { FacebookIcon, GithubIcon, GoogleIcon, MicrosoftIcon } from "../Login/SocialIcons";
...
type P = AuthProviderProps & {
  provider: keyof Omit<AuthProviderProps["authProviders"], "email" | "website_url" | "created_user_type">;
}
export const OAuthProviderSetup = ({ dbs, authProviders, disabledInfo, contentClassName, provider }: P) => {
  ...
  const msftAuthOpts = localAuth?.authOpts && "prompt" in localAuth.authOpts? localAuth.authOpts : undefined;
  ...
  >
    <SwitchToggle
      ...
      disabledInfo={!localAuth?.clientID && !localAuth?.clientSecret && "Must provide a client ID and secret"}
