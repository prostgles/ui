      ...
      type === "Connection URI" ?
        <>
          <FormFieldDebounced 
            ...
            required={true}
            value={c.db_conn} 
            onChange={(db_conn: string) => { updateConnection({ db_conn }) }}
          />
        </> :
        <>
          ...
          <FormField 
            id="u" 
            value={c.db_user} 
            fullOptions={suggestions?.users?.map(({ rolname, rolsuper }) => ({ key: rolname, subLabel: rolsuper ? "Superuser" : "" }))} 
            label="User" 
            type="text" 
            autoComplete="off" 
    ...
    {type !== "Prostgles" && <>
      ...
      {!isForStateDB && <>
        ...
        <SwitchToggle
          ...
          onChange={disable_realtime => { updateConnection({ disable_realtime: !disable_realtime }) }}
        />
        {!c.disable_realtime &&
          <InfoRow>
