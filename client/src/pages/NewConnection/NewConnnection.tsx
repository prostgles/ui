...
export const getSqlErrorText = (e: any) => {
  ...
  if (isObject(e) && "txt" in e) {
    objDetails = ["table", "column"].map(k => [k, e[k]] satisfies [string, any]).filter(d => typeof d[1] === "string");
  }

  /** SQL handler error */
  const text = get(e, "err.err_msg") || get(e, "err.constraint") || get(e, "err.txt") | get(e, "err.message") || e.err_msg || e;
  if (typeof text === "string" && isObject(e.err)) {
    objDetails = ["schema", "table", "column"].map(k => [k, e.err[k]] satisfies [string, any]).filter(d => typeof d[1] === "string");
  }
  if (objDetails.length) {
    return `${objDetails.map(([key, val]) => `${key}: ${val}`).concat([text]).join("\n")}`
  }
  return text;
}

export type Connection = Omit<DBSchemaGenerated["connections"]["columns"], "user_id">;
...
class NewConnection extends RTComp<NewConnectionProps, NewConnectionState> {
  ...
  testConnection = async () => {
    ...
    try {
      ...
      this.setState({ 
        status: "OK" + ((res as any).isSSLModeFallBack? ". (sslmode=prefer fallback)" : ""), 
        statusOK: true, 
      });
    } catch (err: any) {
      this.setState({ 
  ...
  render() {
    const { 
      nameErr, connection: c, status, statusOK, conNotFound, mode, 
      error, originalConnection: origCon, dropDatabase 
    } = this.state;
    ...
    if (!dbsMethods.createConnection || !dbsMethods.deleteConnection) {
      return <InfoRow color="warning" className="mt-2">You do not have sufficient privileges to access this</InfoRow>
    }

    if (conNotFound) {
      return <FlexCol className="m-auto">
    ...
    const updateConnection = async (con: Partial<Connection>) => {
      ...
      let { connection, warning } = ((newData.db_host || newData.db_user || newData.db_name) || newData.db_conn || mode === "edit" || mode === "insert" ) ?
        await dbsMethods.validateConnection!(newData)
          .catch(error => {
      ...
      const wasEdited = this.state.wasEdited || Object.keys(con).join("") !== "type";
      if (!wasEdited) {
        connection = {
          ...connection,
          db_conn: ""
    ...
    >
      ...
      <div className={"flex-col bg-color-0  min-h-0 " + (contentOnly ? "" : " card p-1 ")} style={{ maxWidth: "100%" }}>
        ...
        >
          {mode === "clone" && origCon && <InfoRow color="action">Cloning connection: <strong>{getServerInfo(origCon)}</strong></InfoRow>}
          {showTitle && <h2 className="m-0 p-0 mb-1">
        ...
        <ErrorComponent error={error} style={{ background: "white", padding: "1em" }} withIcon={true} />
          ...
          <Btn 
            ...
            onClickMessage={async (e, setMsg) => {
              try {
                ...
                setMsg({ loading: 1 })
                if (c.name && (await dbs!.connections.findOne({ name: c.name, "id.<>": conn.id }))) {
                  this.setState({ nameErr: "Already exists. Chose another name" });
                  setMsg({ loading: 0 })
                  return
                }
