...
export class Connections extends RTComp<PrglState, S> {
  ...
  onDelta = async () => {
    ...
    if (user && !this.loaded) {
      ...
      this.userSub ??= await dbs.users.subscribeOne({ id: user.id }, {}, user => {
        this.setState({ user })
      });
    }
  }
  ...
  render() {
    ...
    const renderedConnections = connections.filter(c => showStateConn || !c.is_state_db);
    ...
    if (isAdmin) {
      ...
      const sameServer = (_s1: ServerUser, _s2: ServerUser, sameUser = true): boolean => {
        ...
        if(sameUser) return [s1.db_host, s1.db_port, s1.db_user].join() === [s2.db_host, s2.db_port, s2.db_user].join()
        return [s1.db_host, s1.db_port].join() === [s2.db_host, s2.db_port].join()
      };
      ...
      serverUserGroupings = serverUserGroupings.sort((a, b) => 
        Math.max(...b.conns.map(c => + new Date(c.created!))) - Math.max(...a.conns.map(c => + new Date(c.created!)))
      );
    ...
    return (
      <div className="CONNECTIONS flex-col  f-1 w-full min-h-0" >
        ...
        <div className="flex-row as-center w-full gap-p5 mt-1 p-p5  max-w-800">
          ...
          {!connections.length && <InfoRow color="info" className=" f-1 w-full">No connections available/permitted</InfoRow>}
          {isAdmin &&
            <Btn href="/new-connection"
              asNavLink={true}
