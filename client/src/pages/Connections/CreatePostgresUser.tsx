...
export const CreatePostgresUser = ({ newPgUser, setNewPgUser, connectionName, newUserPasswordError, newUsernameError }: P) => {
  ...
  return <>
    <SwitchToggle 
      ...
      onChange={create => {
        setNewPgUser({ 
          ...newPgUser,
          ...(create && !newPgUser.name && connectionName && { name: `${connectionName}_owner` }),
          create, 
        });
      }}
    />
    {newPgUser.create && <>
      ...
      {newPgUser.permissions.type === "custom" && <>
        <div>
          Ticked commands will be allowed on all tables in the public schema
        </div>
        <FlexRowWrap>
...
export const useCreatePostgresUser = ({ connId, runConnectionQuery }: Args) => {
  const [newPgUser, setNewPgUser] = useState<NewPostgresUser & { create: boolean; }>({ 
    name: "", 
    password: "", 
    create: false,
    permissions: {
  ...
  const newUsernameError = usePromise(async () => {
    if(!connId || !runConnectionQuery || !newUserName || !newPgUser.create) return undefined;
  ...
  const newUserPasswordError = newPgUser.create && !newPgUser.password? "Password is required" : undefined;
