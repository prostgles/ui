...
export async function checkAutomaticBackup(this: BackupManager, con: Connections) {

  const AUTO_INITIATOR = "automatic_backups";
  const dbConf = await this.dbs.database_configs.findOne(getDatabaseConfigFilter(con) as any)
  ...
  const dump = async () => {
    ...
    if (bkpConf.err) await this.dbs.database_configs.update({ $existsJoined: { connections: { id: con.id } } }, { backups_config: { ...bkpConf, err: null } });
    ...
    if (currentBackup) {
      shouldDump = false;
    } else if (bkpConf.frequency === "hourly" && (!lastBackup || new Date(lastBackup.created) < new Date(Date.now() - HOUR))) {
      shouldDump = true;
    } else if (hourIsOK() && bkpConf.frequency === "daily" && (!lastBackup || new Date(lastBackup.created) < new Date(Date.now() - 24 * HOUR))) {
      shouldDump = true;
    } else if (dowIsOK() && hourIsOK() && bkpConf.frequency === "weekly" && (!lastBackup || new Date(lastBackup.created) < new Date(Date.now() - 7 * 24 * HOUR))) {
      shouldDump = true;
    } else if (dateIsOK() && dowIsOK() && hourIsOK() && bkpConf.frequency === "monthly" && (!lastBackup || new Date(lastBackup.created) < new Date(Date.now() - 28 * 24 * HOUR))) {
      shouldDump = true;
    }
  ...
  if (!bkpConf.cloudConfig?.credential_id) {
    ...
    if (space.err) {
      if (bkpConf.err !== space.err) {
        await this.dbs.database_configs.update({ id: dbConf!.id }, { backups_config: { ...bkpConf, err: space.err } });
      }
    } else {
      await dump();
    }
