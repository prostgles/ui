...
export const login: Required<Auth<DBSchemaGenerated, SUser>>["login"] = async (loginParams, db, _db: DB, clientInfo) => {
  ...
  if(loginParams.type === "provider"){
    ...
    const { onSuccess, ip } = await startLoginAttempt(db, clientInfo, { auth_type: "provider", auth_provider: provider });
    onSuccess();
  ...
  const { onSuccess, ip } = await startLoginAttempt(db, { ip_address, ip_address_remote, user_agent, x_real_ip }, { auth_type: "login", username });
  try {
...
const createSession = async ({ db, ip, user, user_agent }: CreateSessionArgs) => {
  const activeSession = await getActiveSession(db, { type: "login-success", filter: { user_id: user.id, type: "web", user_agent: user_agent ?? "" }});
  if(!activeSession){
    const globalSettings = await db.global_settings.findOne();
    const DAY = 24 * 60 * 60 * 1000;
    const expires = Date.now() + (globalSettings?.session_max_age_days ?? 1) * DAY;
    return await makeSession(user, { ip_address: ip, user_agent: user_agent || null, type: "web" }, db, expires)
  }
