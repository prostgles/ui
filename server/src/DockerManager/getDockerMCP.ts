...
export const getDockerMCP = async (
  ...
  const maybeDockerManager = await tryCatchV2(async () => {
    const { dockerManager, tools } = await getDockerMCPTools(dbs);
    return { dockerManager, tools };
  });
  ...
  const databaseQueryDescription =
    ...
    : [
        `To run queries against the database you need to POST JSON body parameters to ${maybeDockerManager.data.dockerManager.api_url}`,
  ...
  return {
    ...
    toolSchemas,
    tools: maybeDockerManager.data!.tools,
  };
};
