...
export const createAgenticWorkflowPrompt = [
  ...
  // `IMPORTANT: given that you will not have access to the runtime logs and some of the tools you must test any orchestrator tools you are not sure about before creating the final workflow to ensure it works on first try.`,
  `Preflight contract: `,
  `1) Generated workflows are code the user may run later; the assistant will not automatically see production runtime logs.`,
