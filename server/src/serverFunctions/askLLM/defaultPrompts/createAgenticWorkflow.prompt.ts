...
export const createAgenticWorkflowPrompt = [
  ...
  `IMPORTANT: given that you will not have access to the runtime logs ${getProstglesMCPFullToolName("prostgles-ui", "run_typescript_in_nodejs")} is a tool you can use to quickly check if your logic works as expected. Use it when interacting with external datasources or packages that you are not 100% sure of. This helps ensuring the workflow_function_definition code works as expected on first try.`,
