...
FROM (
  VALUES 
    ...
    ('Sexual Health', 'Trojan-Enz Condoms Lubricated Latex, 12 CT', 'HSA/FSA EligibleAmerica''s No. 1 condom trusted for over 80 years. Helps reduce the risk of pregnancy and sexually transmitted diseases. If used properly, latex condoms will help to reduce the risk of transmission of HIV infection (AIDS) and many other sexually transmitted diseases. Triple tested - Trojan quality. Made from premium quality latex to help reduce the risk. Jelly-type water soluble lubricant for comfort and sensitivity. Special reservoir end for extra safety. Each condom is electronically tested to help ensure reliability. Always insist on Trojan, America''s No. 1 condom. Trojan Brand Latex Condoms, when used properly, are highly effective against pregnancy - although no contraceptive can guarantee 100%25 effectiveness. Any use of Trojan Brand Latex Condoms for other than vaginal intercourse can increase the potential of damage to the condom. Trojan Brand Latex Condoms, when used properly, may help reduce the risk of catching or spreading many sexually transmitted diseases (STDs) such as syphilis, gonorrhea, chlamydia infections, genital herpes, and AIDS; however, they cannot eliminate the risk. For maximum benefits, it is important to follow the instructions for use printed on the inner side of this package. Failure to do so may result in the loss of the benefits of a condom. During intimate contact, lesions and various body fluids can transmit STDs. Therefore, a new condom should be applied each and every time before any such contact occurs. Made in USA.This item is FSA (Flexible Spending Account) eligible.', '17.99 USD'),
...
COPY "london_restaurants.geojson" (id, "type", lat, lon, name) FROM PROGRAM $$
  curl -d "[out:csv(::id, ::type, ::lat, ::lon, name; false)];(node(51.31087184032102,-0.33782958984375,51.723200166800346,0.053558349609375)[amenity=pub];); out;"  -X POST http://overpass-api.de/api/interpreter
$$;

UPDATE "london_restaurants.geojson"
SET geometry = st_point(lon, lat, 4326);

/* Create 1k restaurants and 100k users in London */
WITH raddr as (
  SELECT name, st_centroid(geometry)::GEOGRAPHY AS geog, 'restaurant' as type
...
iaddr as (
  ...
  SELECT name, 'London', 'UK', 'England', 'A1 9AB',  geog
  FROM raddr
  RETURNING *
), 
ires AS (
  ...
  FROM iaddr a
  INNER JOIN raddr r ON a.geog = r.geog 
  RETURNING *
), 
uadd as (
  INSERT INTO addresses (street, city, state, postal_code, country, geog)
  SELECT 'West street', 'London', 'UK', 'England', 'A1 9AB',
...
uins AS (
  ...
  SELECT email, pwd, first_name, last_name, phone_number, type,  now() + (random() * "interval"('1 day'))
  FROM new_users
  RETURNING *
), 
uains AS (
...
/* Create 500 riders */
INSERT INTO users (email, password, first_name, last_name, phone_number, type, created_at)
SELECT email, 'pwd', first_name, last_name, phone_number, 'rider', now() + (random() * "interval"('1 day'))
FROM random_fake_contacts;
...
SELECT DISTINCT ON (t.rnum) user_id, address_id, restaurant_id, riders.id as deliverer_id, 'preparing', 0, 0, random() * 100, now() + (random() * interval '1 hour')
FROM (
  SELECT  vu.id as user_id, vu.address_id, r.id as restaurant_id, rnum
  FROM ( 
    SELECT *, row_number() OVER( order by random()) as rnum
    FROM v_users 
    WHERE type = 'customer'
  ...
  ON st_distance(vu.geog, r.geog) < 5000
) t
LEFT JOIN ( 
  SELECT *, row_number() OVER( order by random()) as rnum
  FROM users
...
setInterval(() => {
    db.sql(`UPDATE addresses 
SET geog = st_point(
  st_x(geog::GEOMETRY) + (CASE WHEN random() < .5 THEN 1 ELSE -1 END ) * random()/1e4, 
  st_y(geog::GEOMETRY) + (CASE WHEN random() < .5 THEN 1 ELSE -1 END ) * random()/1e4, 
  4326
) `)
}, 2000);
...
*/
-- INSERT INTO users (email, password, first_name, last_name, phone_number, type)
-- VALUES('manager@restauranti.com', 'password', 'Luigi', 'Ascot', '098756432', 'restaurant_manager');

-- INSERT INTO addresses (street, city, state, postal_code, country)
-- VALUES('123 west str', 'london', 'london', 'CV1 2AB', 'UK');

-- INSERT INTO restaurants (name, manager_id, address, address_id)
-- VALUES('restauranti', 1, '123 west road', 1);

-- INSERT INTO menu_items (restaurant_id, name, category, price, description)
-- VALUES(1, 'eggs', 'breakfast', 5, 'fried eggs');

-- INSERT INTO menu_items (restaurant_id, name, category, price, description)
-- VALUES(1, 'salad', 'extras', 2, 'salad');



-- INSERT INTO users (email, password, first_name, last_name, phone_number, type)
-- VALUES('johndoe@yahoo.com', 'password', 'John', 'Doe', '012756432', 'rider');
