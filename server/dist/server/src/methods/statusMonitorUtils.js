...
exports.killPID = exports.getServerStatus = exports.execPSQLBash = exports.IGNORE_QUERY = void 0;
const utils_1 = require("../../../commonTypes/utils");
const ConnectionManager_1 = require("../ConnectionManager/ConnectionManager");
exports.IGNORE_QUERY = "prostgles-status-monitor-query";
const execPSQLBash = (db, connId, command) => {
    const tblName = JSON.stringify(`prostgles_shell_${connId}`);
    return db.any(`
    /* ${utils_1.QUERY_WATCH_IGNORE}  */
    /* ${exports.IGNORE_QUERY} */
    ...
    `, { command }).then(vals => vals.map(v => Object.values(v)[0]));
};
exports.execPSQLBash = execPSQLBash;
const getServerStatus = async (db, connId) => {
    const getBashQuery = async (vars) => {
        ...
        const resArr = (await (0, exports.execPSQLBash)(db, connId, command))[0]?.split(varDelimiter);
    ...
    const cpu_mhz = (await (0, exports.execPSQLBash)(db, connId, `lscpu | grep "MHz"`)).join("\n");
    const cpu_cores_mhz = (await (0, exports.execPSQLBash)(db, connId, `cat /proc/cpuinfo | grep "MHz" | sed 's/^.*: //'`)).join("\n");
    const disk_spaceStr = (await (0, exports.execPSQLBash)(db, connId, `df -h ${data_directory} | grep "/"`)).join("\n");
...
//# sourceMappingURL=statusMonitorUtils.js.map