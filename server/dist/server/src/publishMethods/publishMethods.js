...
exports.runConnectionQuery = exports.getSampleSchemas = exports.checkIf = exports.is = exports.publishMethods = void 0;
...
const getInstalledPrograms_1 = require("../BackupManager/getInstalledPrograms");
...
const publishMethods = async (params) => {
    ...
    const adminMethods = {
        ...
        getDBSize: async (conId) => {
            const c = index_1.connMgr.getConnection(conId);
            const size = await c.prgl.db.sql("SELECT pg_size_pretty( pg_database_size(current_database()) ) ", {}, { returnType: "value" });
            return size;
        },
        getIsSuperUser: async (conId) => {
            const c = index_1.connMgr.getConnection(conId);
            return (0, Prostgles_1.getIsSuperUser)(c.prgl._db);
        },
        ...
        getPsqlVersions: () => {
            return (0, getInstalledPrograms_1.getInstalledPrograms)(_dbs);
        },
        createConnection: async (con, sampleSchemaName) => {
            const res = await (0, upsertConnection_1.upsertConnection)(con, user.id, dbs, sampleSchemaName);
            return res;
        },
        ...
        deleteConnection: async (id, opts) => {
            try {
                return dbs.tx(async (t) => {
                    ...
                    await index_1.connMgr.prgl_connections[id]?.methodRunner?.destroy();
                    await index_1.connMgr.prgl_connections[id]?.onMountRunner?.destroy();
                    await index_1.connMgr.prgl_connections[id]?.tableConfigRunner?.destroy();
                    if (opts?.dropDatabase) {
                        if (!con?.db_name)
                            throw "Unexpected: Database name missing";
                        const cdb = await (0, ConnectionManager_1.getCDB)(con.id, undefined, true);
            ...
            `);
                        ...
                        const acdb = await (0, ConnectionManager_1.getCDB)(con.id, { database: anotherDatabaseName.datname }, true);
        ...
        getStatus: (connId) => (0, getPidStats_1.getStatus)(connId, dbs),
        runConnectionQuery: exports.runConnectionQuery,
        getSampleSchemas: exports.getSampleSchemas,
        getCompiledTS: async (ts) => {
            return (0, connectionManagerUtils_1.getCompiledTS)(ts);
        },
        killPID: statusMonitorUtils_1.killPID,
        setOnMountAndTableConfig: async (connId, changes) => {
            const { dbConf } = await getConnectionAndDbConf(connId);
            const newDbConf = await dbs.database_configs.update({ id: dbConf.id }, changes, { returning: "*", multi: false });
            if (!newDbConf)
                throw "Unexpected: newDbConf missing";
            if ("on_mount_ts" in changes || "on_mount_ts_disabled" in changes) {
                await index_1.connMgr.setOnMount(connId, newDbConf.on_mount_ts, newDbConf.on_mount_ts_disabled);
            }
            if ("table_config_ts" in changes || "table_config_ts_disabled" in changes) {
                await index_1.connMgr.setTableConfig(connId, newDbConf.table_config_ts, newDbConf.table_config_ts_disabled);
            }
        },
        getForkedProcStats: async (connId) => {
            const prgl = index_1.connMgr.getConnection(connId);
            return {
                server: {
                    // cpu: os.cpus(),
                    mem: os.totalmem(),
    ...
    const userMethods = !user.id ? {} : {
        ...
        create2FA: async () => {
            ...
            const hashedRecoveryCode = (0, authUtils_1.getPasswordHash)(user, recoveryCode);
...
const getSampleSchemas = async () => {
    const path = (0, electronConfig_1.getRootDir)() + `/sample_schemas`;
    const files = fs_1.default.readdirSync(path).filter(name => !name.startsWith("_"));
    return files.map(name => {
        const schemaPath = `${path}/${name}`;
        if (fs_1.default.statSync(`${schemaPath}`).isDirectory()) {
            ...
            return {
                ...
                workspaceConfig: workspaceConfigStr ? (0, connectionManagerUtils_1.getEvaledExports)(workspaceConfigStr).workspaceConfig : undefined,
            };
        }
        return {
            name,
...
exports.getSampleSchemas = getSampleSchemas;
const runConnectionQuery = async (connId, query, args) => {
    const cdb = await (0, ConnectionManager_1.getCDB)(connId);
    return cdb.any(query, args);
};
exports.runConnectionQuery = runConnectionQuery;
