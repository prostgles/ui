...
async function pgDump(conId, credId, { options: o, destination, credentialID, initiator = "manual_backup" }) {
    ...
    const { fileMgr } = await (0, utils_1.getFileMgr)(this.dbs, credId);
    if (!credId) {
    ...
    let backup_id;
    const uri = (0, utils_1.getConnectionUri)(con);
    ...
    const dumpCommand = dumpAll ? {
        ...
        opts: (0, utils_1.addOptions)(["-d", uri], [
            [o.clean, "--clean"],
    ...
    try {
        ...
        const backup = await this.dbs.backups.insert({
            ...
            dump_command: (0, pipeFromCommand_1.envToStr)(ENV_VARS) + dumpCommand.command + " " + dumpCommand.opts.join(" "),
        ...
        const destStream = fileMgr.uploadStream(backup_id, content_type, async (loadingRaw) => {
            /** S3 is adding some extra fields while total is missing */
            const loading = (0, PubSubManager_1.pickKeys)(loadingRaw, ["total", "loaded"]);
        ...
        proc = (0, pipeFromCommand_1.pipeFromCommand)({
            ...dumpCommand,
            envVars: ENV_VARS,
            ...
            onStdout: !o.keepLogs ? undefined : async ({ chunk: _dump_logs, pipedLength }, isStdErr) => {
                ...
                const dump_logs = (0, utils_1.makeLogs)(_dump_logs, currBkp?.dump_logs, currBkp?.created);
