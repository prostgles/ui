  ...
  links: {
    ...
    columns: {
      ...
      options: 
       ...
       |  {  type: "timechart";  dataSource?: |  {  type: "sql";  sql: string;  withStatement: string; } |  {  type: "table";  joinPath?: (  {  table: string;  on: (  Record<string, any> )[]; } )[]; } |  {  type: "local-table";  localTableName: string;  smartGroupFilter?: |  {  $and: any[]; } |  {  $or: any[]; }; };  smartGroupFilter?: |  {  $and: any[]; } |  {  $or: any[]; };  joinPath?: (  {  table: string;  on: (  Record<string, any> )[]; } )[];  localTableName?: string;  sql?: string;  title?: string;  groupByColumn?: string;  otherColumns?: (  {  name: string;  label?: string;  udt_name: string; } )[];  columns: (  {  name: string;  colorArr: number[];  statType?: {  funcName: "$min" | "$max" | "$countAll" | "$avg" | "$sum";  numericColumn: string; }; } )[]; }
      user_id: string;
      w1_id: string;
      w2_id: string;
