...
export type DBGeneratedSchema = {
 ...
 |  {  title: string;  optional?: boolean;  type: "custom";  dataType: "string" | "number" | "boolean" | "Date"; }>;   timeOutInSeconds: number;   newTables?: (  {  name: string;  columns: (  {  name: string;  dataType: string; } )[]; } )[];   orchestrationTools?: | undefined
 |  Partial<Record<string,  Partial<Record<string, 1>>>>;  };
      definition_override?: null | {    agentDefinitions?: Partial<Record<string,  {  prompt?: string;  modelName?: string;  maxCostUSD?: number;  maxIterations?: number;  tools?: Partial<Record<string,  Partial<Record<string, 1>>>>;  maxTokens?: number;  temperature?: number; }>>;   timeOutInSeconds?: number;  };
      id?: number;
      name: string;
      user_id: string;
    };
...
export type GeneratedFunctionSchema = { 
 ...
 |  {  type: "end";  streamId: string; };  }) => Promise<(undefined | string)>;
  ...
  "startAgenticWorkflow": (args: {    chatId: number;   messageId: string;   workflowId: number;   name: string;   workflowTs: string;   timeOutInSeconds: number;   executionMode: "series" | "parallel";   databaseAccessDefinitions?: |  {  mode: "execute_sql_with_rollback" | "execute_sql_with_commit"; }
