...
export type GeneratedFunctionSchema = { 
 ...
 |  {  type: "end";  streamId: string; };  }) => Promise<(undefined | string)>;
  ...
  "startAgenticWorkflow": (args: {    chatId: number;   name: string;   workflowTs: string;   timeOutInSeconds: number;   databaseAccessDefinitions?: |  {  mode: "custom";  tablePermissions: Partial<Record<string,  Partial<Record<"select" | "insert" | "update" | "delete", boolean>>>>; }
 |  {  mode: "run_commited_sql" | "run_readonly_sql"; };   toolDefinitions: Record<string,  {  mcpServerName: string;  toolNames: string[];  configId?: number; }>;   agentDefinitions: Record<string,  {  prompt: string;  modelName?: string;  maxCostUSD?: number;  maxIterations?: number;  allowedToolDefinitionNames?: string[];  maxTokens?: number;  temperature?: number;  outputSchema: Record<string,  | "string" | "number" | "boolean" | "unknown" | "string[]" | "number[]" | "boolean[]" | "unknown[]" |  {  optional?: boolean;  type: "string" | "number" | "boolean" | "unknown" | "string[]" | "number[]" | "boolean[]" | "unknown[]"; } |  {  optional?: boolean;  arrayOfType: Record<string,  | "string" | "number" | "boolean" | "unknown" | "string[]" | "number[]" | "boolean[]" | "unknown[]" |  {  optional?: boolean;  type: "string" | "number" | "boolean" | "unknown" | "string[]" | "number[]" | "boolean[]" | "unknown[]"; }>; }>; }>;  }) => Promise<({} & { state: ("error" | "finished" | "build-error" | "timed-out" | "aborted"); name: string; command: string; log: Array<({} & { type: ("error" | "stdout" | "stderr"); text: string })>; exitCode: number; runDuration: number; buildDuration: number })>;
