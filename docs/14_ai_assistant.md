...
_Note: AI providers are configured by administrators in Server Settings > LLM Providers_
...
- **MCP tools allowed**: Opens the MCP tools menu for the current chat.
  ...
  - **MCP tools**: List of available MCP tools for the current chat. Each tool represents a specific functionality or integration.
    ...
    - **Enable/Disable MCP server**: Enables or disables the selected MCP server for all chats.
...
- **Prompt settings**: Opens the prompt settings for the current chat, allowing you to manage the prompt template and other related settings.
